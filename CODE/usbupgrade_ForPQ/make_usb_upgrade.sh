...
function func_init_common_env(){
    ...
    SCRIPT_DIR=$ANDROID_BUILD_TOP/scripts
    IMAGE_DIR=$ANDROID_BUILD_TOP/images
    OUTPUT_DIR=$ANDROID_BUILD_TOP/UsbBin


    IMAGE_EXT=".img"
    FILE_PART_READ_CMD="filepartload"
    USB_IMG=$OUTPUT_DIR/$OUTPUT_IMG
    SCRIPT_FILE=$OUTPUT_DIR/usb_upgrade.txt
    ...
    TEMP_IMG=$OUTPUT_DIR/usb_temp.bin
    PADDED_BIN=$OUTPUT_DIR/padded.bin
    PAD_DUMMY_BIN=$OUTPUT_DIR/dummy_pad.
}

# rm some exist files, create PAD_DUMMY_BIN
function func_pre_process(){

    rm ./images/tvconfig.img
    rm ./images/tvdatabase.img
		
    ./tools/mktvconfig.sh
    ./tools/mktvdatabase.sh
...
function func_post_process(){
    ...
    if [ "$READBACK_VERIFY" = "1" ]; then
        ...
        CRC_VALUE=`./tools/crc -a $SCRIPT_FILE.temp | grep "CRC32" | awk '{print $3;}'`
...
function func_finish_script(){
    ...
    printf "setenv factory_poweron_mode \n" >> $SCRIPT_FILE
    printf "setenv UARTOnOff off\n" >> $SCRIPT_FILE
    printf "setenv db_table 0\n" >> $SCRIPT_FILE
    printf "setenv EOSTEK_PQUPGRADE_Ver "$VER"\n" >> $SCRIPT_FILE
    printf "setenv verify n\n" >> $SCRIPT_FILE
    printf "saveenv\n" >> $SCRIPT_FILE
    printf "printenv\n" >> $SCRIPT_FILE
...
function func_mk_usb_image(){
   ...
   #func_print_selection;
    ...
    dd if=$USB_IMG of=$OUTPUT_DIR/out.bin bs=16 count=1;
    cat $OUTPUT_DIR/out.bin >>$USB_IMG
    rm -rf $OUTPUT_DIR/out.bin
    # ----------------------- copy the first 16 bytes to last end ------------------

}

rm -r $OUTPUT_DIR/*
mkdir $OUTPUT_DIR/usbflash
VER=$1

sed -i "s/PQVer =.*/PQVer = $VER;/g" ./tvconfig/config/sys.ini

OUTPUT_IMG="usbflash/638Upgrade.bin"
func_mk_usb_image;

OUTPUT_IMG="usbflash/PQUpgrade.bin"
func_mk_usb_image;
echo
echo "###################################################################"
echo
