...
import com.heran.launcher2.util.ReadView;
...
public class NewsMainFragment extends PublicFragment {
    ...
    private static final int UPDATE_NEWS_RIGTHT = 0;

    private static final int UPDATE_NEWS_LEFT = 1;
    ...
    public NewsMainFragment(HomeActivity context, MainViewHolder mHolder) {
        super();
    ...
    private void show(int i) {
        ...
        aq.id(news_img).image(list.get(loop).articleList.get(i).imagePath, true, true, 100, 0, null, 0,
                AQuery.INVISIBLE);
    ...
    private void updateNewsInfo(int loop, int num1, int num2, int num3, int num4) {
        ...
        if (num1 <= list.get(loop).articleList.size() - 1) {
            btn1_isOk = true;
            aq.id(newsphoto01).image(list.get(loop).articleList.get(num1).imagePath, true, true, 100, 0, null, 0,
                    AQuery.INVISIBLE);
        ...
        if (num2 <= list.get(loop).articleList.size() - 1) {
            btn2_isOk = true;
            aq.id(newsphoto02).image(list.get(loop).articleList.get(num2).imagePath, true, true, 100, 0, null, 0,
                    AQuery.INVISIBLE);
        ...
        if (num3 <= list.get(loop).articleList.size() - 1) {
            btn3_isOk = true;
            aq.id(newsphoto03).image(list.get(loop).articleList.get(num3).imagePath, true, true, 100, 0, null, 0,
                    AQuery.INVISIBLE);
        ...
        if (num4 <= list.get(loop).articleList.size() - 1) {
            btn4_isOk = true;
            aq.id(newsphoto04).image(list.get(loop).articleList.get(num4).imagePath, true, true, 100, 0, null, 0,
                    AQuery.INVISIBLE);
            ...
            if (focus) {
                ...
                switch (v.getId()) {
                    ...
                    case R.id.index_homebtn_icon02:
                        liveIcon.setImageResource(R.drawable.index_icon_02_select);
                        break;
                ...
                homeIcon.setImageResource(R.drawable.index_icon_01_unselect);
                liveIcon.setImageResource(R.drawable.index_icon_02_unselect);
    ...
    class MyBtnNewsMainIconClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.index_homebtn_icon01:
                    recData = HistoryRec.block[7] + ',' + HistoryRec.block8Action[1] + ',' + "首頁" + ',' + "" + ',' + ""
                    ...
                    break;
                case R.id.index_homebtn_icon02:
                    recData = HistoryRec.block[7] + ',' + HistoryRec.block8Action[1] + ',' + "多媒體" + ',' + "" + ',' + ""
                            + ',' + HistoryRec.getCurrentDateTime();
                    HistoryRec.writeToFile(recData);
                    recData = "";
                    ...
                    break;
                case R.id.index_homebtn_icon03:
                    recData = HistoryRec.block[7] + ',' + HistoryRec.block8Action[1] + ',' + "app" + ',' + "" + ',' + ""
                            + ',' + HistoryRec.getCurrentDateTime();
                    HistoryRec.writeToFile(recData);
                    recData = "";
                    ...
                    break;
                case R.id.index_homebtn_icon04:
                    recData = HistoryRec.block[7] + ',' + HistoryRec.block8Action[1] + ',' + "設置" + ',' + "" + ',' + ""
                            + ',' + HistoryRec.getCurrentDateTime();
                    HistoryRec.writeToFile(recData);
                    recData = "";
                    ...
                    break;
                case R.id.index_homebtn_icon05:
                    recData = HistoryRec.block[7] + ',' + HistoryRec.block8Action[1] + ',' + "會員登入" + ',' + "" + ','
                            + "" + ',' + HistoryRec.getCurrentDateTime();
                    HistoryRec.writeToFile(recData);
                    recData = "";
                ...
                switch (v.getId()) {
                    case R.id.newsItem04: {
                        ...
                        if (keyCode == KeyEvent.KEYCODE_DPAD_RIGHT) {
                            Log.d(TAG, "list : " + (list.get(0).articleList.size() - 1));
                            Log.d(TAG, "newsIndex04 : " + (newsIndex04 + 4));
                            if (newsIndex04 <= list.get(0).articleList.size() - 1) {
    ...
    public class GetNewsCategory extends Thread {
        ...
        public void run() {
            ...
            try {
                ...
                InputStream inputStream = HttpUtils.getXML(path, "news");
        ...
        public void run() {
            ...
            try {
                ...
                for (int i = 0; i < list.size(); i++) {
                    ...
                    InputStream inputStream = HttpUtils.getXML(path + list.get(i).categoryID, "news");
                    list.get(i).articleList = SaxService.readNewsArticleXML(inputStream);

                    for (int j = 0; j < list.get(i).articleList.size(); j++) {
                        Log.d(TAG, "j = " + j + "ContentUrl = " + list.get(i).articleList.get(j).contentUrl);
                    }
                }
                mHandler.removeMessages(UPDATE_NEWS_RIGTHT);
                mHandler.sendEmptyMessage(UPDATE_NEWS_RIGTHT);
