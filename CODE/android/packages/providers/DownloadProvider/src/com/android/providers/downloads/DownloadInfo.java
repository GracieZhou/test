...
public class DownloadInfo {
    ...
    private boolean isReadyToDownload() {
        ...
        switch (mStatus) {
            ...
            case Downloads.Impl.STATUS_INSUFFICIENT_SPACE_ERROR:
                ...
                return false;
                // EosTek Patch Begin
           // case Downloads.Impl.STATUS_UDISK_NOT_FOUND_ERROR:
               // if (!TextUtils.isEmpty(mFileName)
                      //  && (mFileName.startsWith(Constants.M805_UDISK_PATH) || mFileName
                     //           .startsWith(Constants.UDISK_PATH))) {
    ...
    public boolean startDownloadIfReady(ExecutorService executor) {
        synchronized (this) {
            ...
            if (isReady && !isActive) {
                // EosTek Patch Begin
                int runningTask = ((ThreadPoolExecutor) executor).getActiveCount();
                if (runningTask < ((DownloadService) mContext).getDownloadNumber()) {
                    // EosTek Patch End
                ...
                mSubmittedTask = executor.submit(mTask);
                    ...
                    if (mStatus != Impl.STATUS_PENDING) {
                        mStatus = Impl.STATUS_PENDING;
                        ContentValues values = new ContentValues();
                        values.put(Impl.COLUMN_STATUS, mStatus);
                        mContext.getContentResolver().update(getAllDownloadsUri(), values, null, null);
                    }
