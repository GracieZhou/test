...
// EosTek Patch Begin
#define OEM_BOOTANIMATION_FILE "/data/local/bootanimation.zip"
...
// Eostek Patch Begin
#define SYSTEM_BOOTVIDEOADVERT_FILE "/system/media/video/video.ts"
// Eostek Patch End
...
BootAnimation::BootAnimation() : Thread(false), mZip(NULL), mVideo(false)
{
...
bool BootAnimation::android()
{
    ...
    GLint xc = (mWidth  - mAndroid[0].w);
    GLint yc = 0;

    char property[PROPERTY_VALUE_MAX] = {0};
    property_get("persist.bootanimation.size", property, "0");
    int size = 1;
    if (atoi(property) > 1) {
        size = atoi(property);
        for (int i = 0; i < 2; i++) {
            mAndroid[i].w *= size;
            mAndroid[i].h *= size;
            if (mAndroid[i].w > mWidth)
                mAndroid[i].w = mWidth;
            if (mAndroid[i].h > mHeight)
                mAndroid[i].h = mHeight;
        }
...
// MStar Android Patch Begin
// Boot video issue, "mstar.videoadvert.finished" is
//  0 means that video does not finish
//  1 means that video has finished
//  no default value
//
// When there has a video file to be played, we has cleared screen
// , so users can see video behind screen.
// Bootanimation will wait for video to finish, before it exits.
// After bootanimation exits, users can see launcher UI.
//
// Return Value
// true means that a video file has been played
// false means that no video
