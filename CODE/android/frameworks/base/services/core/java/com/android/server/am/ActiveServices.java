            ...
            if (type == ActivityThread.SERVICE_DONE_EXECUTING_START) {
                ...
                if (res == Service.START_STICKY_COMPATIBILITY) {
                    r.callStart = false;
					                ...
					                // EosTek Patch Begin
                if (mAm.preventServiceRestart(r.packageName)) {
                    if (ActivityManagerService.localLOGV) {
                        Slog.d(TAG, "reset stopIfKilled of [" + r.packageName + "] from " + r.stopIfKilled + " to true");
                    }
                    r.stopIfKilled = true;
                }
                // EosTek Patch End
    ...
    final void killServicesLocked(ProcessRecord app, boolean allowRestart) {
        ...
        for (int i=app.services.size()-1; i>=0; i--) {
            ...
            if (sr.app != app && sr.app != null && !sr.app.persistent) {
                // EosTek Patch Begin
                if (mAm.preventServiceRestart(sr.app.processName)) {
        ...
        for (int i=app.services.size()-1; i>=0; i--) {
            ...
            if (allowRestart && sr.crashCount >= 2 && (sr.serviceInfo.applicationInfo.flags
                ...
                // EosTek Patch Begin
                if (mAm.preventServiceRestart(sr.packageName)) {
                    Slog.d(TAG, "bringDownService [" + sr.packageName + "]");
                    bringDownServiceLocked(sr);
                    if (!allowRestart) {
                        app.services.clear();
                    }
                ...
                Slog.d(TAG, "scheduleServiceRestartLocked [" + sr.packageName + "] stopIfKilled : " + sr.stopIfKilled);
                // EosTek Patch End
