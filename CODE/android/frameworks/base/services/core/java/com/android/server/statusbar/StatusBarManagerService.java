...
public class StatusBarManagerService extends IStatusBarService.Stub {
    ...
    private void updateUiVisibilityLocked(final int vis, final int mask) {
        // EosTek Patch Begin
        final boolean enabled = SystemProperties.getInt("ro.build.scifly", 0) == 1;
        if (enabled) {
            if (mask != 0xffffeeee) {
                Slog.i(TAG, String.format("vis : %s, mask : %s", Integer.toHexString(vis), Integer.toHexString(mask)));
                return;
            }
        }
        // EosTek Patch End
