...
public class InputMethodManagerService extends IInputMethodManager.Stub
    ...
    private void resetAllInternalStateLocked(final boolean updateOnlyWhenLocaleChanged,
        ...
        if (!updateOnlyWhenLocaleChanged
            ...
            if (!updateOnlyWhenLocaleChanged) {
                ...
                // Eostek Android Patch Begin
                // resetDefaultImeLocked(mContext);
                // If the locale is changed, to reset the Corresponding ime
                // ,example zhTW&enUS -->ZHUYIN,zhCN-->PINYIN,frFR-->FRENCH
                resetDefaultIme();
                // Eostek Android Patch End
            }
            updateFromSettingsLocked(true);
    ...
    private void resetDefaultIme() {
        String inputmethod = mContext.getResources().getString(R.string.default_ime);
        if (mMethodList != null) {
            final int N = mMethodList.size();
            for (int i = 0; i < N; i++) {
                if (mMethodList.get(i).getId().equals(inputmethod)) {
                    Slog.d(TAG, "<<<<<<mMethodList<<<<<<<<" + mMethodList.get(i).getId());
                    setSelectedInputMethodAndSubtypeLocked(mMethodList.get(i), NOT_A_SUBTYPE_ID, false);
                }
            }
        }
    }
