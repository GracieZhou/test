...
class MountService extends IMountService.Stub
    ...
    // MStar Android Patch Begin
    // whether the external storage is emulated
    private static final String ENV_EMULATED_STORAGE_TARGET = "EMULATED_STORAGE_TARGET";
    private boolean mExternalStorageEmulated = false;

    private StorageVolume mSecondStorageTemplate = null;

    // How many times to do unmount action when shutdown
    private static Integer shutdownCount = 0;
    class ISOFileMountInfo {
        private String isoFilePath;
        private String isoFileMountPath;
        private IISOActionListener observer;
        private int nonce;

        public ISOFileMountInfo(String isoFile, String mountPath, IISOActionListener token, int n) {
            isoFilePath = isoFile;
            isoFileMountPath = mountPath;
            observer = token;
            nonce = n;
        }
        ...
        public String getFileMountPath() {
            return isoFileMountPath;
        }

        public IISOActionListener getISOActionListener() {
            return observer;
        }
    ...
    private ArrayList<ArrayList<ISOFileMountInfo>> ISOFileMountInfos =
            new ArrayList<ArrayList<ISOFileMountInfo>>();
    // MStar Android Patch End
    ...
    // EosTek Patch Begin
    final private HashSet<String> mUsbAsecMountSet = new HashSet<String>();
    // EosTek Patch End
    ...
    // MStar Android Patch Begin
    PMGuardThread pmGuardThread = null;
    // Protect pm against a lot of media mount/unmount events
    ...
    class PMGuardThread extends Thread {
        private static final int MEDIA_NOEVENT = 0;
        private static final int MEDIA_IDLE_EVENT = 1;
        public static final int MEDIA_UNMOUNT_EVENT = 2;
        public static final int MEDIA_MOUNT_EVENT = 3;
        // EosTek Patch Begin
        public static final int MEDIA_USB_MOUNT_EVENT = 4;
        public static final int MEDIA_USB_UNMOUNT_EVENT = 5;

        private Integer currentUsbMediaEvent = MEDIA_IDLE_EVENT;
        private int pmUsbMediaStatus = MEDIA_UNMOUNT;
        // EosTek Patch End
        public static final int MEDIA_UNMOUNT = 0;
        public static final int MEDIA_MOUNT = 1;

        private Integer currentMediaEvent = MEDIA_IDLE_EVENT;

        private int pmMediaStatus = MEDIA_UNMOUNT;

        private Boolean finishUnumountApksOnSD = true;

        public void registerMediaEvents(int event) {
            if (event != MEDIA_UNMOUNT_EVENT && event != MEDIA_MOUNT_EVENT ) {
                return;
            }

            synchronized (currentMediaEvent) {
                if (pmMediaStatus == MEDIA_MOUNT) {
                    /* pm has known media mounted, the next event must be a media unmount event.
                     * immediately handle the event.
                     */
                    if (event == MEDIA_UNMOUNT_EVENT) {
                        pmMediaStatus = MEDIA_UNMOUNT;
                        currentMediaEvent = MEDIA_IDLE_EVENT;

                        synchronized(finishUnumountApksOnSD) {
                            finishUnumountApksOnSD = false;
                        }
                        mPms.updateExternalMediaStatus(false,true);
                    } else {
                         //It is impossible to go here.
                         Slog.e(TAG,"pm media status is mounted, but the next event is not a media unmount event!");
                    }
                } else {
                    //register the current event
                    currentMediaEvent = event;
                }
            }
        }

        // EosTek Patch Begin
        public void registerUsbMediaEvents(int event) {

            if (sInstallOnUsbDisabled) {
                return;
            }
            if (event != MEDIA_USB_UNMOUNT_EVENT && event != MEDIA_USB_MOUNT_EVENT) {
                return;
            }

            synchronized (currentUsbMediaEvent) {
                if (pmUsbMediaStatus == MEDIA_MOUNT) {
                    /* pm has known media mounted, the next event must be a media unmount event.
                     * immediately handle the event.
                     */
                    if (event == MEDIA_USB_UNMOUNT_EVENT) {
                        pmUsbMediaStatus = MEDIA_UNMOUNT;
                        currentUsbMediaEvent = MEDIA_IDLE_EVENT;

                        synchronized(finishUnumountApksOnSD) {
                            finishUnumountApksOnSD = false;
                        }
                        Slog.d(TAG, "==>updateUsbExternalMediaStatus(false,true)");
                        mPms.updateUsbExternalMediaStatus(false,true);
                    } else {
                         //It is impossible to go here.
                         Slog.e(TAG,"pm usb media status is mounted, but the next event is not a media unmount event!");
                    }
                } else {
                    //register the current event
                    currentUsbMediaEvent = event;
                }
            }
        }
        // EosTek Patch End

        public void notifyFinishUnumountApksOnSD() {
            synchronized(finishUnumountApksOnSD) {
                finishUnumountApksOnSD = true;
            }
        }
        ...
        public void run() {
            while (true) {
                synchronized (currentMediaEvent) {
                    if (pmMediaStatus == MEDIA_UNMOUNT) {
                        if (currentMediaEvent == MEDIA_IDLE_EVENT) {
                            // idle, nothing to do
                        } else if (currentMediaEvent == MEDIA_NOEVENT) {
                            // after a media mount event occur, no media event occur while the thread sleep
                            pmMediaStatus = MEDIA_MOUNT;
                            currentMediaEvent = MEDIA_IDLE_EVENT;

                            mPms.updateExternalMediaStatus(true,false);
                        } else if (currentMediaEvent == MEDIA_UNMOUNT_EVENT) {
                            /* media unmount event occur, but pm has known media
                             * unmounted, nothing to do.
                             * Reset currentMediaEvent.
                             */
                            currentMediaEvent = MEDIA_IDLE_EVENT;
                        } else if (currentMediaEvent == MEDIA_MOUNT_EVENT){
                            // media mount event occur
                            //assume that no event will come
                            currentMediaEvent = MEDIA_NOEVENT;
                        } else {
                            //unknown event
                        }
                    } else {
                        // handle in registerMediaEvents()
                    }
                }

                // EosTek Patch Begin
                synchronized (currentUsbMediaEvent) {
                    if (pmUsbMediaStatus == MEDIA_UNMOUNT) {
                        if (currentUsbMediaEvent == MEDIA_IDLE_EVENT) {
                            // idle, nothing to do
                        } else if (currentUsbMediaEvent == MEDIA_NOEVENT) {
                            // after a media mount event occur, no media event occur while the thread sleep
                            pmUsbMediaStatus = MEDIA_MOUNT;
                            currentUsbMediaEvent = MEDIA_IDLE_EVENT;

                            Slog.d(TAG, "==>updateUsbExternalMediaStatus(true,false)");
                            mPms.updateUsbExternalMediaStatus(true,false);
                        } else if (currentUsbMediaEvent == MEDIA_USB_UNMOUNT_EVENT) {
                            /* media unmount event occur, but pm has known media
                             * unmounted, nothing to do.
                             * Reset currentMediaEvent.
                             */
                            currentUsbMediaEvent = MEDIA_IDLE_EVENT;
                        } else if (currentUsbMediaEvent == MEDIA_USB_MOUNT_EVENT){
                            // media mount event occur
                            //assume that no event will come
                            currentUsbMediaEvent = MEDIA_NOEVENT;
                        } else {
                            //unknown event
                        }
                    } else {
                        // handle in registerUsbMediaEvents()
                    }
                }
                // EosTek Patch End
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case H_UNMOUNT_PM_UPDATE: {
                    ...
                    if (!mUpdatingStatus && primary != null && primary.getPath().equals(ucb.path)) {
    ...
    private void updatePublicVolumeState(StorageVolume volume, String state) {
        ...
        if (volume.isPrimary() && !volume.isEmulated()) {
            if (Environment.MEDIA_UNMOUNTED.equals(state)) {
                ...
                pmGuardThread.registerMediaEvents(PMGuardThread.MEDIA_MOUNT_EVENT);
            }
        // EosTek Patch Begin
        } else if (path.startsWith(PackageManager.USB_STORAGE_PATH_PREFIX)) {
            if (Environment.MEDIA_MOUNTED.equals(state)) {
                Slog.d(TAG, "==>MEDIA_USB_MOUNT_EVENT");
                pmGuardThread.registerUsbMediaEvents(PMGuardThread.MEDIA_USB_MOUNT_EVENT);
            } else if (Environment.MEDIA_UNMOUNTED.equals(state)) {
                Slog.d(TAG, "==>MEDIA_USB_UNMOUNT_EVENT");
                pmGuardThread.registerUsbMediaEvents(PMGuardThread.MEDIA_USB_UNMOUNT_EVENT);
            }
        // EosTek Patch End
        }
        // MStar Android Patch End
            ...
            if (action != null) {
                ...
                if (action.equals(Intent.ACTION_MEDIA_REMOVED) || action.equals(Intent.ACTION_MEDIA_BAD_REMOVAL)){
                    if (volume.isPrimary()) {
                        ...
                        // EosTek Patch Begin
                    } else if (volume.getPath().startsWith(PackageManager.USB_STORAGE_PATH_PREFIX)) {
                        synchronized(mUsbAsecMountSet) {
                            mUsbAsecMountSet.clear();
                        }
                        // EosTek Patch End
                    }
    ...
    // EosTek Patch Begin
    private static String getMountState(String mountPoint) {
        String state = "";
        String oneLine = "";
        ...
        try {
            ...
            while ((oneLine = br.readLine()) != null) {
                ...
                if (oneLine.contains(mountPoint) && splits.length > 4) {
                    state = splits[3].substring(0, 2);
                    break;
                }
            }
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
        ...
        if (newState == VolumeState.Init) {
            ...
            // EosTek Patch Begin
            // comment: add state for mounted as ro fs
            if ("ro".equals(getMountState(volume.getPath()))) {
                Log.i(TAG, volume.getPath() + " was mounted as ro fs.");
                updatePublicVolumeState(volume, Environment.MEDIA_MOUNTED_READ_ONLY);
            } else {
                updatePublicVolumeState(volume, Environment.MEDIA_MOUNTED);
            }
            // EosTek Patch End
            ...
            // MStar Android Patch Begin
            synchronized(ISOFileMountInfos) {
                ArrayList<ISOFileMountInfo> infoList;
                ISOFileMountInfo info;

                for (int i=0;i < ISOFileMountInfos.size();++i) {
                    infoList = ISOFileMountInfos.get(i);
                    info = infoList.get(0);
                    if (info.getISOFilePath().startsWith(path)) {
                        int length = infoList.size();
                        for (int j=length-1;j >= 0;--j) {
                            info = infoList.get(j);
                            IISOActionListener token = info.getISOActionListener();
                            if (token !=  null) {
                                try {
                                    token.onISOEvent(info.getISOFilePath(),info.getNonce(),IISOActionListener.UNMOUNTED);
                                } catch (RemoteException e) {
                                    Slog.i(TAG, "Couldn't send unmount notification for  ISO: "
                                            + info.getISOFilePath());
                                }
                            }
                        }
                        ISOFileMountInfos.remove(infoList);
                        --i;
                    }
                }
            }
    ...
    private int doUnmountVolume(String path, boolean force, boolean removeEncryption) {
        ...
        // MStar Android Patch Begin
        StorageVolume primary = getPrimaryPhysicalVolume();
        boolean isPrimaryVolume = primary != null && primary.getPath().equals(path);
        if (isPrimaryVolume) {
    ...
    private void createEmulatedVolumeForUserLocked(UserHandle user) {
        ...
        if (isExternalStorageEmulated()) {
            ...
            // when external storage is not emulated, use "/storage/emulated/"" as default
            path = new File("/storage/emulated/" + user.getIdentifier());
        }
    ...
    private void removeVolumeLocked(StorageVolume volume) {
        ...
        // MStar Android Patch Begin
        // physical primary volume can not be deleted
        if (volume == null || (!volume.isEmulated() && volume.isPrimary())) {
            return;
        }
        // MStar Android Patch End
    ...
    public MountService(Context context) {
        ...
        // MStar Android Patch Begin
        pmGuardThread = new PMGuardThread();
        pmGuardThread.start();
        // MStar Android Patch End
    ...
    public void shutdown(final IMountShutdownObserver observer) {
        // MStar Android Patch Begin
        boolean waitUnmount = false;
        boolean waitForChecking = true;
        ArrayList<ShutdownCallBack> shutdownCallBacks = new ArrayList<ShutdownCallBack>();
        validatePermission(android.Manifest.permission.SHUTDOWN);

        Slog.i(TAG, "Shutting down");
        while (waitForChecking) {
            waitForChecking = false;
            synchronized (mVolumesLock) {
        ...
        if (!waitUnmount) {
            ...
            int size = shutdownCallBacks.size();
            for(int i = 0; i < size; ++i) {
                ShutdownCallBack ucb = shutdownCallBacks.get(i);
    ...
    public boolean isExternalStorageEmulated() {
        // MStar Android Patch Begin
        return mExternalStorageEmulated;
        // MStar Android Patch End
    }
    ...
    public String[] getSecureContainerList() {
        ...
        // MStar Android Patch Begin
        final StorageVolume primary = getPrimaryPhysicalVolume();
        boolean primaryVolumeMounted = false;
        if (primary != null) {
            try {
                primaryVolumeMounted = Environment.MEDIA_MOUNTED.equals(getVolumeState(primary.getPath()));
            } catch (IllegalStateException e) {
            }
        }
        if (!primaryVolumeMounted) {
            String [] res = new String[0];
            synchronized (mAsecMountSet) {
                res=mAsecMountSet.toArray(res);
            }
            return res;
        }
        // MStar Android Patch End
    ...
    // EosTek Patch Begin
    public String[] getUsbSecureContainerList() {
        validatePermission(android.Manifest.permission.ASEC_ACCESS);
        waitForReady();

        try {
            // EosTek Patch Begin
            String[] result = NativeDaemonEvent.filterMessageList(
                    mConnector.executeForList("asec", "listusb"), VoldResponseCode.AsecListResult);
            if (result == null || result.length == 0) {
                String [] res = new String[0];
                synchronized (mUsbAsecMountSet) {
                    res=mUsbAsecMountSet.toArray(res);
                }
                return res;
            }
            return result;
    ...
    public int createSecureContainer(String id, int sizeMb, String fstype, String key,
        ...
        return createSecureContainer(id, sizeMb, fstype, key, ownerUid, external, false);
    }

    public int createSecureContainer(String id, int sizeMb, String fstype, String key,
            int ownerUid, boolean external, boolean isUsb) {
    // EosTek Patch End
        ...
        try {
            ...
            mConnector.execute("asec", "create", id, sizeMb, fstype, new SensitiveArg(key),
                    ownerUid, external ? "1" : "0", isUsb ? "1" : "0");
            // EosTek Patch End
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
        }
    ...
    public int finalizeSecureContainer(String id) {
        return finalizeSecureContainer(id, false);
    }

    public int finalizeSecureContainer(String id, boolean isUsb) {
    // EosTek Patch End
        validatePermission(android.Manifest.permission.ASEC_CREATE);
        warnOnNotMounted();
        ...
        try {
            // EosTek Patch Begin
            mConnector.execute("asec", "finalize", id, isUsb ? "1" : "0");
            // EosTek Patch End
            /*
    ...
    public int fixPermissionsSecureContainer(String id, int gid, String filename) {
        return fixPermissionsSecureContainer(id, gid, filename, false);
    }

    public int fixPermissionsSecureContainer(String id, int gid, String filename, boolean isUsb) {
    // EosTek Patch End
        validatePermission(android.Manifest.permission.ASEC_CREATE);
        warnOnNotMounted();
        ...
        try {
            // EosTek Patch Begin
            mConnector.execute("asec", "fixperms", id, gid, filename, isUsb ? "1" : "0");
            // EosTek Patch End
            /*
    ...
    public int destroySecureContainer(String id, boolean force) {
        return destroySecureContainer(id, force, false);
    }

    public int destroySecureContainer(String id, boolean force, boolean isUsb) {
    // EosTek Patch End
        ...
        if (rc == StorageResultCode.OperationSucceeded) {
            ...
            if (isUsb) {
                synchronized (mUsbAsecMountSet) {
                    if (mUsbAsecMountSet.contains(id)) {
                        mUsbAsecMountSet.remove(id);
                    }
                }
            } else {
                synchronized (mAsecMountSet) {
	...
	public int mountSecureContainer(String id, String key, int ownerUid, boolean readOnly) {
		return mountSecureContainer(id, key, ownerUid, readOnly, false);
	}
    public int mountSecureContainer(String id, String key, int ownerUid, boolean readOnly, boolean isUsb) {
    // EosTek Patch End
    ...
    public int unmountSecureContainer(String id, boolean force) {
        return unmountSecureContainer(id, force, false);
    }

    public int unmountSecureContainer(String id, boolean force, boolean isUsb) {
    // EosTek Patch End
        validatePermission(android.Manifest.permission.ASEC_MOUNT_UNMOUNT);
        waitForReady();
        warnOnNotMounted();
    ...
    public boolean isSecureContainerMounted(String id) {
        ...
        synchronized (mAsecMountSet) {
            result = mAsecMountSet.contains(id);
        }

        if (!result) {
            synchronized (mUsbAsecMountSet) {
                result = mUsbAsecMountSet.contains(id);
            }
        }
    ...
    public int renameSecureContainer(String oldId, String newId) {
        return renameSecureContainer(oldId, newId, false);
    }

    public int renameSecureContainer(String oldId, String newId, boolean isUsb) {
    // EosTek Patch End
        ...
        synchronized (mUsbAsecMountSet) {
             ...
             * changed while active, we must ensure both ids are not currently mounted.
             */
            if (mUsbAsecMountSet.contains(oldId) || mUsbAsecMountSet.contains(newId)) {
                return StorageResultCode.OperationFailedStorageMounted;
            }
        }
        // EosTek Patch End
        ...
        try {
            // EosTek Patch Begin
            mConnector.execute("asec", "rename", oldId, newId, isUsb ? "1" : "0");
            // EosTek Patch End
        } catch (NativeDaemonConnectorException e) {
            rc = StorageResultCode.OperationFailedInternalError;
        }
    ...
    public String getSecureContainerPath(String id) {
        return getSecureContainerPath(id, false);
    }

    public String getSecureContainerPath(String id, boolean isUsb) {
    // EosTek Patch End
        validatePermission(android.Manifest.permission.ASEC_ACCESS);
        waitForReady();
        warnOnNotMounted();
        ...
        try {
            // EosTek Patch Begin
            event = mConnector.execute("asec", "path", id, isUsb ? "1" : "0");
            // EosTek Patch End
    ...
    public void finishMediaUpdate() {
        ...
        // MStar Android Patch Begin
        pmGuardThread.notifyFinishUnumountApksOnSD();
        // MStar Android Patch End
    }
    ...
    public StorageVolume[] getVolumeList() {
        ...
        synchronized (mVolumesLock) {
            ...
            for (StorageVolume volume : mVolumes) {
                ...
                // MStar Android Patch Begin
                if (!isExternalStorageEmulated() && volume.isEmulated()) {
                    continue;
                }
                // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    @Override
    public boolean mountISO(String filename, IISOActionListener token, int nonce) {
        if (filename == null) {
        ...
        try {
            mConnector.execute("iso","mount",filename);
            String mountPath = getISOFileMountPath(filename);
            if (mountPath == null) {
                return false;
            }

            ISOFileMountInfo newInfo = new ISOFileMountInfo(filename,mountPath,token,nonce);
            synchronized(ISOFileMountInfos) {
                int size = ISOFileMountInfos.size();
                for (int i = 0;i < size;++i) {
                    ArrayList<ISOFileMountInfo> infoList = ISOFileMountInfos.get(i);
                    int length = infoList.size();
                    ISOFileMountInfo info;
                    String path;
                    ...
                    for (int j = 0;j < length;++j) {
                        info = infoList.get(j);
                        path = info.getFileMountPath();
                        if(filename.startsWith(path)) {
                            infoList.add(newInfo);
                            if (token != null) {
                                try {
                                    token.onISOEvent(newInfo.getISOFilePath(),newInfo.getNonce(),IISOActionListener.MOUNTED);
                                } catch (RemoteException e) {
                                    Slog.i(TAG, "Couldn't send unmount notification for  ISO: "
                                            + newInfo.getISOFilePath());
                                }
                            }
                            return true;
                        }
                ...
                ArrayList<ISOFileMountInfo> newInfoList = new ArrayList<ISOFileMountInfo>(1);
                newInfoList.add(newInfo);
                ISOFileMountInfos.add(newInfoList);
                if (token != null) {
                    try {
                        token.onISOEvent(newInfo.getISOFilePath(),newInfo.getNonce(),IISOActionListener.MOUNTED);
                    } catch (RemoteException e) {
            ...
            Slog.e(TAG,"Can not mount the iso file "+filename);
            return false;
        }
    }
    ...
    public String getISOFileMountPath(String filename) {
        ...
        final NativeDaemonEvent event;
        try {
            event = mConnector.execute("iso", "path", filename);
            event.checkCode(VoldResponseCode.AsecPathResult);
            return event.getMessage();
        } catch (NativeDaemonConnectorException e) {
            int code = e.getCode();
    ...
    public boolean unmountISO(String filename, boolean force) {
        boolean res = doUnmountISO(filename,force);
        if (res == false) {
            return res;
        }
    ...
    private boolean doUnmountISO(String filename, boolean force) {
        if (filename == null) {
            throw new IllegalArgumentException("filename cannot be null");
        }
        ...
        try {
            final Command cmd = new Command("iso", "unmount", filename);
            if (force) {
                cmd.appendArg("force");
            }
            mConnector.execute(cmd);
            ...
            Slog.e(TAG,"Can not unmount the iso file "+filename);
            return false;
        }
    }
    ...
    public String[] getMountedISOFileList() {
        ...
        try {
            return NativeDaemonEvent.filterMessageList(
                    mConnector.executeForList("iso", "list"), VoldResponseCode.AsecListResult);
        } catch (NativeDaemonConnectorException e) {
            return new String[0];
        }
    }
    ...
    public boolean mountSamba(String host, String shareDirectory, String mountPoint, String userName,
                              String password, boolean readOnly, boolean exec) {
        if (host == null) {
            throw new IllegalArgumentException("host cannot be null");
        }
        if (shareDirectory == null) {
            throw new IllegalArgumentException("shareDirectory cannot be null");
        }
        if (mountPoint == null) {
            throw new IllegalArgumentException("mountPoint cannot be null");
        }
        if (userName == null) {
        ...
        int index = shareDirectory.length()-1;
        for(; index >= 0; --index) {
            char ch = shareDirectory.charAt(index);
            if (ch != '/') {
                break;
            }
        }
        if (index != -1) {
            shareDirectory = shareDirectory.substring(0,index+1);
        }

        Command cmd = new Command("samba","mount", host.trim(), shareDirectory.trim(), mountPoint,
                                  userName,password,
                                  readOnly ? "ro " : "rw ",
                                  exec ? "exec" : "noexec");
        try {
            ...
            Slog.e(TAG,"Can not mount samba, host:"+host+" share directory:"+shareDirectory);
            return false;
        }
    }
    ...
    public boolean unmountSamba(String mountPoint, boolean force) {
        ...
        try {
            Command cmd = new Command("samba","unmount",mountPoint);
            if (force) {
                cmd.appendArg("force");
            }
            mConnector.execute(cmd);
            ...
            Slog.e(TAG,"Can not unmount samba, mountPoint:"+mountPoint);
            return false;
        }
    }
    ...
    public String getVolumeLabel(String mountPoint) {
        ...
        try {
            final String[] r = NativeDaemonEvent.filterMessageList(
                    mConnector.executeForList("volume", "label", mountPoint),
                    VoldResponseCode.VolumeListResult);

            return r[0];
        } catch(Exception e) {
            Slog.e(TAG,"Error when get volume label!");
        }
