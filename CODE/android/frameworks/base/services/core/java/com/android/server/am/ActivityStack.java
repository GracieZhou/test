...
final class ActivityStack {
    ...
    // MStar Android Patch Begin
    static final String MALI_ALLOC_STRATEGY_SCRIPT="/system/bin/mali_alloc_strategy.sh";
    static final String MALI_ALLOC_STRATEGY_PKS_PATH="/system/etc/mali_alloc_strategy_packages.xml";
    static final String OPAQUE_ACTIVITY_PKGS_PATH = "/system/etc/opaque_activity_packages.xml";
    final ArrayList<String> mOpaqueActivityPkgs = new ArrayList<String>();
    final ArrayList<String> mMaliAllocStrategyPkgs = new ArrayList<String>();
    boolean mMaliAllocStrategyPkgsParsed = false;
    boolean mMaliAllocStrategyChange = false;
    // MStar Android Patch End
    ...
    ActivityStack(ActivityStackSupervisor.ActivityContainer activityContainer) {
        ...
        // MStar Android Patch Begin
        parsePackagesList("utf-8", OPAQUE_ACTIVITY_PKGS_PATH, mOpaqueActivityPkgs);
        // MStar Android Patch End
    }
    ...
    void minimalResumeActivityLocked(ActivityRecord r) {
        ...
        // MStar Android Patch Begin
        //mStackSupervisor.dumpRecentTasks();
        mStackSupervisor.doCpuDvfsScaling();
        mStackSupervisor.doBenchmarkAccelerate();
        maliAllocStrategy(true, r);
        checkWillStartOpaqueActivity(true,r);
        // MStar Android Patch End
    }
    ...
    private void completePauseLocked(boolean resumeNext) {
        ...
        // MStar Android Patch Begin
        maliAllocStrategy(false, null);
        checkWillStartOpaqueActivity(false,null);
        // MStar Android Patch End
    ...
    final boolean resumeTopActivityInnerLocked(ActivityRecord prev, Bundle options) {
        ...
        // MStar Android Patch Begin
        if (next.compat != null && !next.compat.supportsScreen()) {
            CompatibilityInfo.setDefaultDinmen(next.compat.forceTo4k2kSize());
        }
        // MStar Android Patch End
    ...
    final void startActivityLocked(ActivityRecord r, boolean newTask,
        ...
        // EosTek Patch Begin
        if (mService.buildWithScifly()) {
            mService.recordApkFootprints(r.intent, r.packageName);
            //performKillProcessesLocked(r.packageName);
        }
        // EosTek Patch End
    }
            ...
            for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
                ...
                if (r.app == app) {
                    ...
                    if ((!r.haveState && !r.stateNotNeeded) || r.finishing) {
                        ...
                        remove = true;
                        // EosTek Patch Begin
                    } else if (!r.finishing && mService.buildWithScifly()) {
                          Slog.w(TAG, "Force removing " + r + ": app died, no saved state");
                          EventLog.writeEvent(EventLogTags.AM_FINISH_ACTIVITY, r.userId, System.identityHashCode(r),
                                  r.task.taskId, r.shortComponentName, "proc died without state saved");

                          removeActivityFromHistoryLocked(r,null);
                          // EosTek Patch End
    ...
    // MStar Android Patch Begin
    private boolean parsePackagesList(String xmlEncoding, String xmlPath, ArrayList<String> pkgList) {
        ...
        try {
            parser.setInput(new FileInputStream(new File(xmlPath)), xmlEncoding);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
        try {
            while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
                if (parser.getEventType() == XmlPullParser.START_TAG) {
                    if (parser.getName().equalsIgnoreCase("item")) {  // get tag name
                        String pkg = parser.nextText();  // get tag text
                        pkgList.add(pkg);
                        Slog.v(TAG, "parsePackagesList>> adding pkg=" + pkg);
                    }
                }
                parser.next();
            }
    ...
    private void maliAllocStrategy(boolean startHome, ActivityRecord r) {
        boolean maliAllocStrategy = false;
        if (true == SystemProperties.getBoolean("mstar.mali.alloc.strategy", false)) {
            if (false == mMaliAllocStrategyPkgsParsed ) {
                if (false == parsePackagesList("utf-8", MALI_ALLOC_STRATEGY_PKS_PATH, mMaliAllocStrategyPkgs)) {
                    return;
                } else {
                    mMaliAllocStrategyPkgsParsed = true;
                }
            }
        }

        if (startHome && mTaskHistory.size() == 1) {
            if (mMaliAllocStrategyPkgs.contains(r.packageName)) {
                maliAllocStrategy = true;
            } else {
                maliAllocStrategy = false;
                Slog.v(TAG,"unmatched pkg name=" + r.packageName);
            }
        }

        if (mTaskHistory.size() > 1) {
            ActivityRecord next = topRunningActivityLocked(null);
            if ((next != null) && mMaliAllocStrategyPkgs.contains(next.packageName)) {
                maliAllocStrategy = true;
            } else {
                maliAllocStrategy = false;
            }
        ...
        try {
            String cmd;
            if (maliAllocStrategy) {
                cmd = "/system/bin/sh " + MALI_ALLOC_STRATEGY_SCRIPT + " true";
            } else {
                cmd = "/system/bin/sh " + MALI_ALLOC_STRATEGY_SCRIPT + " false";
            }
            if (maliAllocStrategy != mMaliAllocStrategyChange) {
                Runtime.getRuntime().exec(cmd);
                mMaliAllocStrategyChange = maliAllocStrategy;
                Slog.i(TAG, cmd + " set memory location done");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return ;
    ...
    private void checkWillStartOpaqueActivity(boolean startHome, ActivityRecord r) {
        boolean bOpaqueActivity = false;
        if (startHome && mTaskHistory.size()==1) {
            if (mOpaqueActivityPkgs.contains(r.packageName)) {
                bOpaqueActivity = true;
                Slog.v(TAG,"matched opaque pkg name=" + r.packageName);
            }
        }

        if (mTaskHistory.size()>1) {
            ActivityRecord next = topRunningActivityLocked(null);
            if ((next!=null)&&mOpaqueActivityPkgs.contains(next.packageName)) {
                bOpaqueActivity = true;
                Slog.v(TAG,"matched opaque pkg name=" + next.packageName);
            }
        }

        if (bOpaqueActivity) {
            SystemProperties.set("mstar.EnterOpaqueUI", "true");
            Slog.v(TAG,"set mstar.EnterOpaqueUI true");
        } else {
            SystemProperties.set("mstar.EnterOpaqueUI", "false");
            Slog.v(TAG,"set mstar.EnterOpaqueUI false");
        }
    }
    // MStar Android Patch End
	// EosTek Patch Begin
    private void performKillProcessesLocked(String pkg) {
        synchronized (mService) {
            if (pkg == null) {
                return;
            }
            ...
            mService.getMemoryInfo(memory);
            if (localLOGV) {
                Slog.d(TAG, "lowMemory : " + memory.lowMemory + " availMem : " + memory.availMem + " threshold : "
                        + memory.threshold + " totalMem : " + memory.totalMem);
            }

            // 2. memory is low or less than 80%, then call kill app
            if (memory.totalMem == 0) {
                if (memory.lowMemory) {
                    Slog.d(TAG, "clean memory for startup scifly app.");
                    mService.killAllBackgroundApksLocked();
                }
            } else {
                double rate = (double) memory.availMem / memory.totalMem;
                if (memory.lowMemory || 100 * rate <= 20.0) {
                    Slog.d(TAG, "clean memory for startup scifly app because of low memory.");
                    mService.killAllBackgroundApksLocked();
                }
            }
        }
