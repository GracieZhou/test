...
public final class PowerManagerService extends SystemService
    ...
    // MStar Android Patch Begin
    private static final boolean enable_str = SystemProperties.getBoolean("mstar.str.enable", false);
    // Check every 100ms
    private static final int CHECK_SUSPEND_TIMEOUT_PERIOD = 100;
    // Timeout is 60 * 100ms = 6s
    private static final int CHECK_SUSPEND_TIMEOUT = 60;
    private int mCheckSuspendTimeout = 0;
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    // Message: Sent when checking ready for suspend.
    private static final int MSG_CHECK_READY_FOR_SUSPEND = 4;
    // MStar Android Patch End
    ...
    private static native void nativeUpdateStrState(boolean sleep);
    // MStar Android Patch End
    ...
    // Eostek Patch Begin
    private static final String SCREENSAVER_SLEEP_TIME = "screensaver_sleep_time";

    private static final int START_DREAM_MSG = 0x123;

    private int mScreenSaverTime = 0;

    private boolean isMediaLock = false;
    ...
    public void onBootPhase(int phase) {
        synchronized (mLock) {
            if (phase == PHASE_BOOT_COMPLETED) {
                ...
                // MStar Android Patch Begin
                /* For not boot from STR */
                if (true == SystemProperties.getBoolean("mstar.pvr.standby.recording", false)) {
                    Slog.i(TAG, "BootCompleted setprop mstar.str.suspending 1 + goToSleep");
                    if (enable_str == true) {
                        SystemProperties.set("mstar.str.suspending", "1");
                    }
                    goToSleepNoUpdateLocked(now,PowerManager.GO_TO_SLEEP_REASON_APPLICATION, 0, Process.SYSTEM_UID);
                }
                // MStar Android Patch End
            }
        }
    ...
    private void userActivityFromNative(long eventTime, int event, int flags) {
        ...
        if (mScreenSaverTime != 0) {
            mHandler.removeMessages(START_DREAM_MSG);
            mHandler.sendEmptyMessageDelayed(START_DREAM_MSG, mScreenSaverTime);
            Log.v(TAG, "userActivityFromNative send Message START_DREAM_MSG");
        }
        // Eostek Patch end
    }
    ...
    private boolean wakeUpNoUpdateLocked(long eventTime, int uid) {
        ...
        try {
            switch (mWakefulness) {
               ...
               // MStar Android Patch Begin
                if (true == SystemProperties.getBoolean("mstar.pvr.standby.recording", false)) {
                    TvManager.getInstance().getPvrManager().setPvrRecordStandByOnOff(false);
                    TvManager.getInstance().getPvrManager().setIsBootByRecord(true);
                }
                // MStar Android Patch End
    ...
    private boolean goToSleepNoUpdateLocked(long eventTime, int reason, int flags, int uid) {
        ...
        if (eventTime < mLastWakeTime
                ...
                || !mBootCompleted || !mSystemReady) {
            // MStar Android Patch Begin
            if (enable_str && !mBootCompleted)  {
                Slog.d(TAG, "Break STR suspend because of mBootCompleted = false");
        ...
        try {
            ...
            // MStar Android Patch Begin
            if (true == SystemProperties.getBoolean("mstar.pvr.standby.recording", false)) {
                    TvManager.getInstance().getPvrManager().setPvrRecordStandByOnOff(true);
                    TvManager.getInstance().getPvrManager().setIsBootByRecord(true);
            }
            // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    private void checkReadyForSuspend() {
        mHandler.removeMessages(MSG_CHECK_READY_FOR_SUSPEND);
        // Make sure screen off broadcast done
        if ((true == SystemProperties.getBoolean("mstar.str.screenoffintentdone", false))
                // Make sure activity sleep ready
                && (true == SystemProperties.getBoolean("mstar.str.activityready", false))
                // Make sure bootanim show
                && (0 == SystemProperties.getInt("service.bootanim.exit", 1))
                || (mCheckSuspendTimeout >= CHECK_SUSPEND_TIMEOUT)) {

            Slog.d(TAG, "[STR check point] Ready to Suspend with timeout = " + mCheckSuspendTimeout);

            SystemProperties.set("mstar.str.screenoffintentdone", "false");
            SystemProperties.set("mstar.str.activityready", "false");
            mCheckSuspendTimeout = 0;

            AudioManager audioManager = (AudioManager)mContext.getSystemService(Context.AUDIO_SERVICE);
            if (audioManager != null) {
                audioManager.setParameters("playback_status=suspend");
                audioManager.setParameters("capture_status=suspend");
            }
            ...
            if (BluetoothAdapter.getDefaultAdapter() != null) {
                if (BluetoothAdapter.getDefaultAdapter().getState() == BluetoothAdapter.STATE_TURNING_ON
                    ...
                    Slog.i(TAG, "---begin close bt");
                    int timeout = 0;
                    BluetoothAdapter.getDefaultAdapter().disable();
                    while ((BluetoothAdapter.getDefaultAdapter().getState() != BluetoothAdapter.STATE_OFF) && timeout < 100) {
                        Slog.i(TAG, "---enterStrMode---waiting bt closing");
                        SystemClock.sleep(100);
                        timeout++;
                    }
                    Slog.i(TAG, "---end close bt");
                    BluetoothAdapter.getDefaultAdapter().setBtSuspend(true);
                }
            }
            SystemProperties.set("mstar.lvds-off", "1");
            if (false == SystemProperties.getBoolean("mstar.pvr.standby.recording", false)) {
                nativeUpdateStrState(true);
            }
        } else {
            Message msg = mHandler.obtainMessage(MSG_CHECK_READY_FOR_SUSPEND);
            msg.setAsynchronous(true);
            mHandler.sendMessageDelayed(msg, CHECK_SUSPEND_TIMEOUT_PERIOD);
            mCheckSuspendTimeout++;
        }
    }

    private void updateStrState(boolean bSleep) {
        if (bSleep) {
            Slog.d(TAG, "goToSleep mstar.str.suspending");
            SystemProperties.set("mstar.str.suspending", "1");
            Message msg = mHandler.obtainMessage(MSG_CHECK_READY_FOR_SUSPEND);
            msg.setAsynchronous(true);
            mHandler.sendMessage(msg);
        } else {
            /* Make sure that we wake up from supernova */
            if (2 == SystemProperties.getInt("mstar.str.suspending", 0)) {

                SystemProperties.set("mstar.lvds-off", "0");

                nativeUpdateStrState(false);

                AudioManager audioManager = (AudioManager) mContext.getSystemService(Context.AUDIO_SERVICE);
                if (audioManager != null) {
                    audioManager.setParameters("playback_status=resume");
                    audioManager.setParameters("capture_status=resume");
                }

                if (BluetoothAdapter.getDefaultAdapter() != null) {
                    if (BluetoothAdapter.getDefaultAdapter().getBtSuspend()) {
                        Slog.i(TAG,"enable BT....");
                        BluetoothAdapter.getDefaultAdapter().enable();
                        Slog.i(TAG,"enable BT....Done");
                        BluetoothAdapter.getDefaultAdapter().setBtSuspend(false);
                    }
                }
            }
        }
    ...
    private void handleSandman() { // runs on handler thread
        ...
        // Eostek Patch Begin
        try {
            int tmpValue = Settings.System.getInt(mContext.getContentResolver(), SCREENSAVER_SLEEP_TIME);
            if (tmpValue == 0) {
                if (tmpValue != mScreenSaverTime) {
                    mScreenSaverTime = 0;
                    mHandler.removeMessages(START_DREAM_MSG);
                    Log.v(TAG, "disable screensaver");
                }
            } else {
                if (tmpValue != mScreenSaverTime && !isMediaLock) {
                    mScreenSaverTime = tmpValue;
                    mHandler.removeMessages(START_DREAM_MSG);
                    mHandler.sendEmptyMessageDelayed(START_DREAM_MSG, mScreenSaverTime);
                    Log.v(TAG, "handleSandman send Message START_DREAM_MSG " + mScreenSaverTime);
                }
            }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_SCREEN_BRIGHTNESS_BOOST_TIMEOUT:
                    ...
                    break;
                // MStar Android Patch Begin
                case MSG_CHECK_READY_FOR_SUSPEND:
                    checkReadyForSuspend();
                    break;
                // MStar Android Patch End
				
                // Eostek Patch Begin
                case START_DREAM_MSG:
                    Log.v(TAG, "(mWakeLockSummary & WAKE_LOCK_STAY_AWAKE)=="
                            + (mWakeLockSummary & WAKE_LOCK_STAY_AWAKE));
                    if ((mWakeLockSummary & WAKE_LOCK_STAY_AWAKE) != 0) {
                        if (mScreenSaverTime != 0) {
                            mHandler.removeMessages(START_DREAM_MSG);
                            mScreenSaverTime = 0;
                            Log.v(TAG, "WAKE_LOCK_STAY_AWAKE,remove START_DREAM_MSG");
                        }
                    } else {
                        // start dream
                        mDreamManager.startDream(true);
                        final int uid = Binder.getCallingUid();
                        napInternal(System.currentTimeMillis(), uid);
                        Log.v(TAG, "handleMessage START_DREAM_MSG startDream");
                    }
                    break;
               // Eostek Patch End
            }
    ...
    private final class BinderService extends IPowerManager.Stub {
        ...
        public void reboot(boolean confirm, String reason, boolean wait) {
            ...
            // EosTek Patch Begin
            /*
             * if the current power mode is secondary,set the power to
             * direct,and set the htvstandbystate to 1,the mboot will reset the
             * power mode to secondary when start recovery
             */
