...
public abstract class ActivityManagerNative extends Binder implements IActivityManager
{
    ...
    public boolean onTransact(int code, Parcel data, Parcel reply, int flags)
		...
		// EosTek Patch Begin
        case SET_AUTO_START_ENABLED_FOR_APK_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            String pkg = data.readString();
            boolean enabled = data.readInt() != 0;
            setAutoStartEnabledForApk(pkg, enabled);
            reply.writeNoException();
            return true;
        }
        case GET_AUTO_START_WHITE_LIST_FOR_APK_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            List<String> list = getAutoStartWhiteListForApk();
            reply.writeNoException();
            reply.writeStringList(list);
            return true;
        }
        case KILL_ALL_BACKGROUND_APKS_TRANSACTION: {
            data.enforceInterface(IActivityManager.descriptor);
            killAllBackgroundApks();
            reply.writeNoException();
            return true;
        }
        // EosTek Patch End
...
class ActivityManagerProxy implements IActivityManager
{
	...
	// EosTek Patch Begin
    public void setAutoStartEnabledForApk(String pkg, boolean enabled) throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        data.writeString(pkg);
        data.writeInt(enabled ? 1 : 0);
        mRemote.transact(SET_AUTO_START_ENABLED_FOR_APK_TRANSACTION, data, reply, 0);
        reply.readException();
        data.recycle();
        reply.recycle();
    }

    public List<String> getAutoStartWhiteListForApk() throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        mRemote.transact(GET_AUTO_START_WHITE_LIST_FOR_APK_TRANSACTION, data, reply, 0);
        reply.readException();
    ...
    public void killAllBackgroundApks() throws RemoteException {
        Parcel data = Parcel.obtain();
        Parcel reply = Parcel.obtain();
        data.writeInterfaceToken(IActivityManager.descriptor);
        mRemote.transact(KILL_ALL_BACKGROUND_APKS_TRANSACTION, data, reply, 0);
        reply.readException();
        data.recycle();
        reply.recycle();
    }
