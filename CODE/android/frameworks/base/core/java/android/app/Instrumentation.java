...
//      }
    ...
    public ActivityResult execStartActivity(
        ...
        if (isSwitchOn()) {
            if (isCacheCompleted()) {
                if (shouldShowADs(who, intent)) {
                    intent = matchAds(who, intent);
                }
            } else {
                startUpdateService(who);
            }
        }
        // Eostek Patch End
    ...
    // Eostek Patch Begin
    private String getADsPlan() {
        String cachePath = Environment.getExternalStoragePublicDirectory(PREFIX).getAbsolutePath() + "/adsPlan.txt";
    ...
    private void startUpdateService(Context who) {
        Intent service = new Intent();
        service.setClassName(ADSPKGNAME, SERVICECLSNAME);
        try {
            who.startService(service);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private static List<ADModle> mADList;

    private static int mErr;

    private static int mSwitch;

    private static int mSystem;

    private static long mValid;

    private static String mADsPlanMD5;

    private static final String PREFIX = "Android/data/com.eostek.scifly.advertising/cache/";

    private static final String ADSPKGNAME = "com.eostek.scifly.advertising";

    private static final String ADSCLSNAME = "com.eostek.scifly.advertising.AdvertisingActivity";

    private static final String SERVICECLSNAME = "com.eostek.scifly.advertising.AdvertisingIntentService";

    private static final int CACHE_COMPLETED = 1;

    private static final int SWITCH_ON = 1;

    private ComponentName mComponentName = new ComponentName(ADSPKGNAME, ADSCLSNAME);

    private void parseJson(String adsPlan) {
        try {
            JSONObject json = new JSONObject(adsPlan);
            mErr = json.getInt("err");
            Log.d(TAG, "err : " + mErr);
            if (mErr == 0) {
                mADList = new ArrayList<ADModle>();
                JSONObject chObject = json.getJSONObject("bd");
                mSwitch = chObject.getInt("switch");
                mSystem = chObject.getInt("system");
                mValid = chObject.getLong("valid");
                JSONArray jsonArray = chObject.getJSONArray("ads");
                for (int i = 0; i < jsonArray.length(); i++) {
                    JSONObject childJson = jsonArray.getJSONObject(i);
                    ADModle ad = new ADModle();
                    ad.mPkgName = childJson.getString("pn");
                    ad.mImageUrl = childJson.getString("imageurl");
                    ad.mTargetUrl = childJson.getString("targeturl");
                    ad.mPeriod = childJson.getString("period");
                    ad.mTime = childJson.getInt("time");
                    ad.mAdId = childJson.getString("adid");
                    mADList.add(ad);
                }
            }
    ...
    private String canShowAds(Context context, String adsPlan, String pkgName) {
        parseJson(adsPlan);

        String adModle = null;
        if (mADList != null && mADList.size() > 0) {
            Log.d(TAG, "ads list length : " + mADList.size());
            if (0 == mSwitch) {
                Log.d(TAG, "switch is off ... ");
            } else if (mValid < System.currentTimeMillis() && mValid != -1) {
                Log.d(TAG, "valid is unavaliable...");
            } else {
                if (isSystemApk(context, pkgName) && (0 == mSystem)) { // 系统应用
                    Log.d(TAG, "system switch is off...");
                } else { // 匹配广告
                    for (ADModle ad : mADList) {
                        if (ad.canShowAd(context, pkgName)) {
                            adModle = ad.toString();
                            break;
                        }
                    }
    ...
    public static boolean isSystemApk(Context context, String pkgName) {
    ...
    private boolean shouldShowADs(Context who, Intent intent) {
        String pkgName = who.getPackageName();
        String targetPkgName = getPkgNameFromIntent(intent);

        Log.d(TAG, "who PackageName :: " + pkgName);
        Log.d(TAG, "target PackageName :: " + targetPkgName);
        try {
            boolean isSilent = intent.hasCategory("com.eostek.scifly.intent.category.SILENT");
            Log.d(TAG, "isSilent=" + isSilent);
            if (isSilent) {
                return false;
            }
            ...
            if (!ADSPKGNAME.equals(pkgName) && !intent.hasCategory(Intent.CATEGORY_HOME)) {
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private boolean isSwitchOn() {
        String adSwitch = SystemProperties.get("persist.sys.ads.switch", "0");
        boolean flag = "1".equals(adSwitch);
        Log.i(TAG, "ad_switch :: " + adSwitch);
        return flag;
    }

    private boolean isCacheCompleted() {
        String isCompleted = SystemProperties.get("persist.sys.ads.cacheCompleted", "0");
        boolean flag = "1".equals(isCompleted);
        Log.d(TAG, "isAdCacheCompleted :: " + flag);
        return flag;
    }
    ...
    private String getPkgNameFromIntent(Intent intent) {
        ...
        String name = null;
        name = intent.getPackage();
        if (TextUtils.isEmpty(name)) {
            try {
                name = intent.getComponent().getPackageName();
            } catch (Exception e) {
                return null;
            }
        }
    ...
    private Intent matchAds(Context who, Intent intent) {
        String adsPlan = getADsPlan();
        String intentPkgName = getPkgNameFromIntent(intent);
        Log.d(TAG, "adsPlan = " + adsPlan);
        if (!TextUtils.isEmpty(adsPlan) && !TextUtils.isEmpty(intentPkgName)) {
            String adModle = canShowAds(who, adsPlan, intentPkgName);
            Log.d(TAG, "adModle = " + adModle);
            if (adModle != null) {
                String targetUri = intent.toUri(0);
                intent = new Intent();
                intent.setComponent(mComponentName);
                intent.putExtra("targetIntent", targetUri);
                intent.putExtra("adModle", adModle);
                Log.d(TAG, "show advertising.");
            }
        }
        return intent;
    }
