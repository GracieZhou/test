...
public class NtpTrustedTime implements TrustedTime {
    ...
    public boolean forceRefresh() {
        ...
        if (client.requestTime(mServer, (int) mTimeout)) {
			...
			// EosTek Patch Begin
            if (LOGD) Log.d(TAG, "Failed get time from ntpserver");
            // update the system time while auto time enabled
            int autoTime = Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.AUTO_TIME, 0);
            if (autoTime == 0) {
                return false;
            }
            ...
            try {
                final URL url = new URL("http://" + mServer);
                urlConnection = url.openConnection();
                urlConnection.setConnectTimeout(5000);
                urlConnection.connect();
            } catch (Exception e) {
                Log.w(TAG, "connected to " + mServer + " failed. ", e);
                return false;
            }

            // openConnection will not return null
            ...
            if (Math.abs(timestamp - systemCurrentTime) > 5 * 1000) {
                ...
                if (timestamp / 1000 < Integer.MAX_VALUE) {
                    ...
                    if (LOGD) {
                        Log.d(TAG, "SystemClock.setCurrentTimeMillis = " + timestamp);
                    }
                    return true;
                }
            }
