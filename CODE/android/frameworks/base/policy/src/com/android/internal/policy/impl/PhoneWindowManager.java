...
// EosTek Patch Begin
// Heran VirtualMouseManager
import android.app.Notification;
import android.app.NotificationManager;
import android.app.Notification.Builder;
import scifly.virtualmouse.VirtualMouseManager;
// EosTek Patch End
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    // MStar Android Patch Begin
    static final int SHORT_PRESS_POWER_SHUT_DOWN = 4;

    static final boolean enable_str = SystemProperties.getBoolean("mstar.str.enable", false);
    static public final String ACTIVATE_STANDBY_MODE_NONE = "none";
    static public final String ACTIVATE_STANDBY_MODE_WAKE = "wake";
    static public final String ACTIVATE_STANDBY_MODE_SLEEP = "sleep";
    static String mActivatedStandbyMode = ACTIVATE_STANDBY_MODE_NONE;
    // MStar Android Patch End
	...
	// EosTek Patch Begin
	private static final int KEYCODE_TV_REMOTE_MODE =350;
	private static final int KEYCODE_MACHINE_REMOTE_MODE =351;
	private static final int KEYCODE_KARAOKE_MIC_VOLUME_DOWN = 321;
	// Heran VirtualMouseManager
	private static final int MSG_VIRTUAL_MOUSE_DISAPPEAR = 15;
    VirtualMouseManager mVirtualMouse;
    boolean mVirtualMouseEnabled = false;
    // EosTek Patch End
    ...
    private class PolicyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
				...
				// EosTek Patch Begin
				// Heran VirtualMouseManager
				case MSG_VIRTUAL_MOUSE_DISAPPEAR:
					disappearNotification(mContext, 50);
					break;
				// EosTek Patch End
            }
        }
    ...
    private Dialog mShutdownDialog = null;

    public void showShutdownDialog() {
        mShutdownDialog = new Dialog(mContext, com.android.internal.R.style.FullscreenDialog);
        if (null != mShutdownDialog) {
            mShutdownDialog.setContentView(com.android.internal.R.layout.shutdown_message_layout);
            mShutdownDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
            mShutdownDialog.show();
        }
    }

    public void removeShutdownDialog() {
        if (null != mShutdownDialog) {
            mShutdownDialog.dismiss();
        }
    }

    Runnable mShowShutdowViewnRunnable = new Runnable() {
        @Override
        public void run() {
            showShutdownDialog();

            if (localLOGV)
                Slog.d(TAG, "mShowShutdowViewnRunnable ...");
        }
    };

    Runnable mRemoveShutdownViewRunnable = new Runnable() {
        @Override
        public void run() {
            if (localLOGV)
                Slog.d(TAG, "mRemoveShutdownViewRunnable ...");
            removeShutdownDialog();
            mPowerManager.goToSleep(mEventTime, PowerManager.GO_TO_SLEEP_REASON_POWER_BUTTON,
                    PowerManager.GO_TO_SLEEP_FLAG_NO_DOZE);
        }
    };

    private boolean mMyPowerKeyHandled = false;
    private void handlePowerKey(long eventTime) {
        mEventTime = eventTime;
        if(!mMyPowerKeyHandled) {
            mHandler.post(mShowShutdowViewnRunnable);
            mHandler.postDelayed(mRemoveShutdownViewRunnable, 1000);
            mMyPowerKeyHandled = true;
        }
    }
   ...
   // Eostek Patch Begin
    private boolean mHomeLongPressHandled = false;
    private static final Intent mTaskSwitchIntent = new Intent();
    static {
    ...
    private void handleLongPressOnHome() {
        if (mLongPressOnHomeBehavior != LONG_PRESS_HOME_NOTHING) {
            ...
            if (mLongPressOnHomeBehavior == LONG_PRESS_HOME_RECENT_SYSTEM_UI) {
                // Eostek Patch Begin 
                // do not show original tasks.2014-09-19
                // toggleRecentApps();
                // Eostek Patch End
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        // MStar Android Patch Begin
        // register for CU broadcasts
        filter = new IntentFilter();
        filter.addAction("com.mstar.tv.tvplayer.ui.intent.action.TV_CI_CU_STATUS_QUERY");
        context.registerReceiver(mCuStatusReceiver, filter);
        // MStar Android Patch End
	...
	// EosTek Patch Begin
                // Go home!
                if (!mHomeLongPressHandled) {
                    launchHomeFromHotKey();
                }
                mHomeLongPressHandled = false;
                // EosTek Patch End
                return -1;
            }
		...
		// EosTek Patch Begin
		else if (keyCode == KEYCODE_TV_REMOTE_MODE){
			 if (down) {
			    String str = mContext.getResources().getString(R.string.tv_remote_mode);
                showNotification(mContext, str, 50, R.drawable.pic_virtual_mouse);
			    Message msg = Message.obtain(mHandler, MSG_VIRTUAL_MOUSE_DISAPPEAR);
				mHandler.sendMessage(msg);
                return 0;
            }
		} 
		else if (keyCode == KEYCODE_MACHINE_REMOTE_MODE){
			 if (down) {
                String str = mContext.getResources().getString(R.string.machine_remote_mode);
                showNotification(mContext, str, 50, R.drawable.pic_virtual_mouse);
			    Message msg = Message.obtain(mHandler, MSG_VIRTUAL_MOUSE_DISAPPEAR);
				mHandler.sendMessage(msg);
                return 0;
			...
			// Heran VirtualMouseManager 
        else if (keyCode == MKeyEvent.KEYCODE_MSTAR_SIZE) {
            Log.d(TAG, "KEYCODE_MSTAR_SIZE " + down);
            if (down) {
                if (mVirtualMouse == null) {
                    mVirtualMouse = VirtualMouseManager.getInstance();
                }
                if (mVirtualMouse != null) {
                    mVirtualMouse.toggle();
                    mVirtualMouseEnabled = mVirtualMouse.isVirtualMouseEnabled();
                    String string = null;
                    if (mVirtualMouseEnabled) {
                        string = mContext.getResources().getString(R.string.virtual_mouse_enable);
                        showNotification(mContext, string, 50, R.drawable.pic_virtual_mouse);
						Message msg = Message.obtain(mHandler, MSG_VIRTUAL_MOUSE_DISAPPEAR);
						mHandler.sendMessage(msg);
                    } else {
                        string = mContext.getResources().getString(R.string.virtual_mouse_disable);
                        showNotification(mContext, string, 50, R.drawable.pic_virtual_mouse);
						Message msg = Message.obtain(mHandler, MSG_VIRTUAL_MOUSE_DISAPPEAR);
						mHandler.sendMessage(msg);
                    }
        ...
        // dispatch keyevent to virtual mouse
        if (mVirtualMouseEnabled) {
            Log.d(TAG, "dispatch event to virtual mouse");
            if (mVirtualMouse.dispatchKeyEvent(event)) {
                return -1;
            }
        }
        // EosTek Patch End
   ...
   // MStar Android Patch Begin
   // Process Key First for Wake up When StandBy Record
    private int keyTranslateForStandByRecord(KeyEvent event,int policyFlags,boolean down) {
        // no boot by record or screen is on,  use previous key
        int keyCode = event.getKeyCode();
        final boolean isWakeKey = (policyFlags & (WindowManagerPolicy.FLAG_WAKE	)) != 0;

        //TV_POWER, bypass and release resource
        if (keyCode == KeyEvent.KEYCODE_TV_POWER) {
            if (true == SystemProperties.getBoolean("mstar.pvr.standby.recording", false)) {
               try {
                   //release Resource when down
                   if (down) {
                       TvManager.getInstance().getAllResourcesForce(false);
                   }
               } catch (Throwable e) {
                   e.printStackTrace();
               }
        ...
        //POWER Key, Change state to Sleep if StandBy Record
        if (keyCode == KeyEvent.KEYCODE_POWER && !down && mActivatedStandbyMode == ACTIVATE_STANDBY_MODE_WAKE) {
            if (true == SystemProperties.getBoolean("mstar.pvr.standby.recording",false)) {
                mActivatedStandbyMode = ACTIVATE_STANDBY_MODE_SLEEP;
            }
            return keyCode;
        }
        //POWER & WAKE-UP Key, Change state to wake and key translate to TV_POWER
        if ((keyCode == KeyEvent.KEYCODE_POWER || isWakeKey)&& mActivatedStandbyMode !=ACTIVATE_STANDBY_MODE_WAKE) {
            if (true == SystemProperties.getBoolean("mstar.pvr.standby.recording",false)) {
                Log.d(TAG, "keyCode=" + keyCode);
                Log.i(TAG, "suspending = " + SystemProperties.getInt("mstar.str.suspending", 0));
                if (down) {
                    if (enable_str) {
                        SystemProperties.set("mstar.str.suspending", "2");
                    }
                } else {
                    mActivatedStandbyMode = ACTIVATE_STANDBY_MODE_WAKE;
                }
                return enable_str ? KeyEvent.KEYCODE_TV_POWER : keyCode;
            } else {
                return keyCode;
            }
         }
    ...
    // MStar Android Patch Begin
    /** Check if CI current opccupied tuner for upgraing or OP tuning via content provider
     * , it will ingore key here and throw a toast
     */
    private boolean allowCIBlockKeys(int keyCode, boolean isKeydown) {
        boolean ciOccupiedTuner = false;
        Cursor cursor = null;

        if (keyCode == KeyEvent.KEYCODE_POWER) {
            try {
                cursor = mContext.getContentResolver().query(Uri.parse("content://mstar.tv.usersetting/systemsetting"),
                                                             new String [] {"bCiOccupiedTuner"}, null, null, null);
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            }
            if (cursor != null && cursor.moveToFirst()) {
                try {
                    ciOccupiedTuner = cursor.getInt(cursor.getColumnIndexOrThrow("bCiOccupiedTuner")) == 0 ? false : true;
                } catch (IllegalArgumentException e) {
                    e.printStackTrace();
                    ciOccupiedTuner = false;
                }
                cursor.close();
            }

            if (ciOccupiedTuner == true) {
                if (isKeydown) {
                    openNotifyMessage(mContext.getString(com.android.internal.R.string.str_ci_under_upgrading));
                }
                return true;
            }
        }
        ...
        if (keyCode != KeyEvent.KEYCODE_POWER) {
            if (mIsInCuProgress == true) {
                if (isKeydown) {
                    openNotifyMessage(mContext.getString(com.android.internal.R.string.str_ci_under_progress));
                }
                return true;
            }
        }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        keyCode = keyTranslateForStandByRecord(event, policyFlags, down);

        if (enable_str == true) {
            if (1 == SystemProperties.getInt("mstar.str.suspending", 0)) {
                Log.d(TAG, "STR forbid keyCode=" + keyCode + ",down=" +down);
                return 0;
            }
            if (2 == SystemProperties.getInt("mstar.str.suspending", 0)) {
                //Fake key for STR power on
                if (keyCode == KeyEvent.KEYCODE_TV_POWER) {
                    Log.d(TAG, "STR dc on down=" + down);
                    keyCode = KeyEvent.KEYCODE_POWER;
                    // EosTek Patch Begin
                    mMyPowerKeyHandled = false;
                    // EosTek Patch End
                } else {
        ...
        if ((allowCIBlockKeys(keyCode, down) == true)) {
            return 0;
        }
        // MStar Android Patch End
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
                if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
					...
					if (down) {
						...
						if(mAudioManager != null && mAudioManager.isMasterMute()){
							TvAudioManager.getInstance().setAudioSpdifOutMode(EnumSpdifType.E_PCM.ordinal());
						}else{
							TvAudioManager.getInstance().setAudioSpdifOutMode(EnumSpdifType.E_OFF.ordinal());
						}
					}
				}
                if (down) {
            ...
            // MStar Android Patch Begin
            // FIXME: Ingre this flow at first to enable TV Sleep functionality flow
    ...
    // MStar Android Patch Begin
    BroadcastReceiver mCuStatusReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("com.mstar.tv.tvplayer.ui.intent.action.TV_CI_CU_STATUS_QUERY")) {
                synchronized (mLock) {
                    mIsInCuProgress = intent.getBooleanExtra("isInCuProgress", false);
                }
            }
        }
    };
	...
	void launchHomeFromPowerKey() {
	    ...
	    try {
			...
			android.provider.Settings.System.putString(mContext.getContentResolver(), "closeTvRecord",  curTimeStr);
	...
	//EosTek Patch Begin
    private void showNotification(Context context, String str,int id,int resId) {
        NotificationManager notificationManager = (NotificationManager) context
                .getSystemService(Context.NOTIFICATION_SERVICE);
        //cancel the notification when next time show
        notificationManager.cancel(id); 
        
        Notification notification = new Notification(resId, str, System.currentTimeMillis());
        notification.tickerText = str;
        
        notification.flags = Notification.FLAG_AUTO_CANCEL;
        notification.setLatestEventInfo(context, str, str, null);

        notificationManager.notify(id, notification);
    }
	
	private void disappearNotification(Context context,int id) {
		NotificationManager notificationManager = (NotificationManager) context
                .getSystemService(Context.NOTIFICATION_SERVICE);
        //cancel the notification when next time show
        notificationManager.cancel(id); 
