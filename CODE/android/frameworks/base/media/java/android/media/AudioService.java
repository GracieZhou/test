...
public class AudioService extends IAudioService.Stub {
   ...
   /** @hide Maximum volume index values for audio streams */
    ...
    private void restoreMasterVolume() {
        ...
        if (mUseMasterVolume) {
		    //EosTek Patch Begin
			float defVolume = 0.20f;
            // MStar Android Patch Begin
            float volume = Settings.System.getFloatForUser(mContentResolver,
                    Settings.System.VOLUME_MASTER, defVolume, UserHandle.USER_CURRENT);
            AudioSystem.setMasterVolume(volume);
			Settings.System.putFloat(mContentResolver, "eostek_restore_volume", defVolume);
            // MStar Android Patch end
			//EosTek Patch End
        }
    }
    ...
    // MStar Android Patch Begin
    /** @see AudioManager#setBluetoothMicOn()
      Only for Chat Mode AudioSystem.FOR_MEDIA will use BT-MIC
      and AudioSystem.DEVICE_OUT_BLUETOOTH_SPK will be active
      Second param will be set only for Chat Mode
    */
    public void setBluetoothMicOn(boolean on,boolean bi_directional){
        if (!checkAudioSettingsPermission("setBluetoothMicOn()")) {
            return;
        }
        mForcedUseForComm = on ? AudioSystem.FORCE_BT_MIC_IN: AudioSystem.FORCE_NONE;
        Log.d(TAG, "setBluetoothMicOn() on:"+on+", mForcedUseForComm:"+mForcedUseForComm);
        if( bi_directional) {
            AudioSystem.setDeviceConnectionState(AudioSystem.DEVICE_OUT_BLUETOOTH_SPK,
               on ? AudioSystem.DEVICE_STATE_AVAILABLE : AudioSystem.DEVICE_STATE_UNAVAILABLE,
                "00:00:00:00:00:00");
            sendMsg(mAudioHandler, MSG_SET_FORCE_USE, SENDMSG_QUEUE,
                AudioSystem.FOR_MEDIA, mForcedUseForComm, null, 0);
        }
    ...
    /** @see AudioManager#isBluetoothMicOn() */
    public boolean isBluetoothMicOn() {
        return (mForcedUseForComm == AudioSystem.FORCE_BT_MIC_IN);
    }
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    private void makeA2dpInputDeviceAvailable(String address) {
        Log.d(TAG, "makeA2dpInputDeviceAvailable (SINK)");
        // enable A2DP before notifying A2DP connection to avoid unecessary processing in
        // audio policy manager
        setBluetoothA2dpOnInt(true); // keep for now
        AudioSystem.setDeviceConnectionState(AudioSystem.DEVICE_IN_BLUETOOTH_A2DP,
                AudioSystem.DEVICE_STATE_AVAILABLE,
                address);
        // Reset A2DP suspend state each time a new source is connected
        AudioSystem.setParameters("A2dpSuspended=false");
    ...
    // MStar Android Patch Begin
    private void makeA2dpInputDeviceUnavailableNow(String address) {
        Log.d(TAG, "makeA2dpInputDeviceUnavailableNow (SINK)");
        AudioSystem.setDeviceConnectionState(AudioSystem.DEVICE_IN_BLUETOOTH_A2DP,
                AudioSystem.DEVICE_STATE_UNAVAILABLE,
                address);
