...
// EosTek Patch Begin
import android.provider.MediaStore.Documents;
import android.provider.MediaStore.Documents.DocumentColumns;
// EosTek Patch End
...
public class MediaScanner
{
    ...
    private static boolean mDisableScan = false;
    ...
    // EosTek Patch Begin
    // comment : the uri for documents insertion
    private Uri mDocumentsUri;
    // EosTek Patch End
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
        ...
        private ContentValues toValues() {
            ...
            if (!mNoMedia) {
                ...
                // EosTek Patch Begin
                else if (MediaFile.isDocumentType(mFileType)) {
                    map.put(FileColumns.MIME_TYPE, FileColumns.MEDIA_TYPE_DOCUMENT);
                }
                // EosTek Patch End
            }
            return map;
        ...
        private Uri endFile(FileEntry entry, boolean ringtones, boolean notifications,
            ...
            // update database
            // EosTek Patch Begin
            // comment : ignore other non-type files on all storages
            boolean isAudio = MediaFile.isAudioFileType(mFileType);
            boolean isVideo = MediaFile.isVideoFileType(mFileType);
            boolean isImage = MediaFile.isImageFileType(mFileType);
            boolean isPlayList = MediaFile.isPlayListFileType(mFileType);
            boolean isDrm = MediaFile.isDrmFileType(mFileType);
            boolean isDocument = MediaFile.isDocumentType(mFileType);
            
            if (!isVideo && !isAudio && !isImage && !isPlayList && !isDrm && !isDocument)
                return null;
			// EosTek Patch End
            ...
            if (rowId == 0) {
                ...
                if (!MediaScanner.isNoMediaPath(entry.mPath)) {
                    ...
                    // EosTek Patch Begin
                    else if (MediaFile.isDocumentType(fileType)) {
                        mediaType = FileColumns.MEDIA_TYPE_DOCUMENT;
                    }
                    // EosTek Patch End
    ...
    private void initialize(String volumeName) {
        ...
        // EosTek Patch Begin
        mDocumentsUri = Documents.getContentUri(volumeName);
        // EosTek Patch End
    ...
    // MStar Android Patch Begin
    public static void stopMediaScanner() {
        mDisableScan = true;
        Log.v(TAG, "-----set mDisableScan = true;----- ");
    }

    public static void startMediaScanner() {
        mDisableScan = false;
        Log.v(TAG, "-----set mDisableScan = false;----- ");
    }
    // Mstar Android Patch End
    ...
    // MStar Android Patch Begin
    private native void enableScan();
    private native void disableScan();
    // MStar Android Patch End
