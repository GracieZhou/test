...
MediaScanner::MediaScanner()
    : mLocale(NULL), mSkipList(NULL), mSkipIndex(NULL), mDoScan(true) {
...
MediaScanResult MediaScanner::processDirectory(
    ...
    // Mstar Android Patch Begin
    if (mDoScan == false) {
        return MEDIA_SCAN_RESULT_USER_ABORT;
    }
    // Mstar Android Patch End
...
// Mstar Android Patch Begin
// Set scan flag true or false
void MediaScanner::enableScan() {
    mDoScan = true;
    ALOGV("-----MediaScanner::enableScan()-----");
}

void MediaScanner::disableScan() {
    mDoScan = false;
    ALOGV("-----MediaScanner::disableScan()-----");
}
// Mstar Android Patch End
...
MediaScanResult MediaScanner::doProcessDirectory(
    ...
    while ((entry = readdir(dir))) {
        // Mstar Android Patch Begin
        result = doProcessDirectoryEntry(path, pathRemaining, client, noMedia, entry, fileSpot);
        if (result == MEDIA_SCAN_RESULT_ERROR
            || result == MEDIA_SCAN_RESULT_USER_ABORT) {
            break;
        }
        // Mstar Android Patch End
    }
...
MediaScanResult MediaScanner::doProcessDirectoryEntry(
    ...
    // EosTek Patch Begin
    // comment : ignore all hidden dirs & files
#ifndef SCIFLY    
    // ignore "." and ".."
    ...
    if (type == DT_DIR) {
        ...
        if (result == MEDIA_SCAN_RESULT_ERROR) {
            return MEDIA_SCAN_RESULT_ERROR;
        // Mstar Android Patch Begin
        } else if (result == MEDIA_SCAN_RESULT_USER_ABORT) {
            return MEDIA_SCAN_RESULT_USER_ABORT;
        }
