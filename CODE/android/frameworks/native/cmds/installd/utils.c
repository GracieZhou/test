...
static int validate_path(const dir_rec_t* dir, const char* path) {
    ...
    size_t dir_len = 0;
    if(!dir) {
        // indicate this is a usb device.
        char *temp = strstr(path, "/.asec/");
        if (temp) {
            dir_len = temp - path + 7;
            ALOGI("find %s in %s, dir_len=%d", temp, path, (int)dir_len);
        } else {
            ALOGE("invalid apk path '%s' (bad asec)\n", path);
            return -1;
        }
    } else {
        dir_len = dir->len;
...
int validate_apk_path(const char *path)
{
	...
	// EosTek Patch Begin
	} else if (!strncmp(path, "/mnt/usb", 8)) {
		dir = NULL;
	// EosTek Patch End
    } else {
        return -1;
