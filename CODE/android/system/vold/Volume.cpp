...
int Volume::mountVol() {
    ...
    for (i = 0; i < n; i++) {
        ...
        mkdir(getMountpoint(), 0074);

        if (isNtfsFS) {
            if (Ntfs::doMount(devicePath, getMountpoint(), false, false, AID_MEDIA_RW, AID_MEDIA_RW, permMask)) {
        ...
        if (isFatFs) {
            // EosTek Patch Begin
            // comment: change the fat storage's uid,gid and umask
            permMask = 0002;
            // EosTek Patch End
            
            if (Fat::doMount(devicePath, getMountpoint(), false, false, false,
                // EosTek Patch Begin
                AID_SYSTEM, AID_SDCARD_RW, permMask, true)) {
        ...
        if (isExtFs) {
            if (Extfs::doMount(devicePath, getMountpoint(), false, false, AID_MEDIA_RW, AID_MEDIA_RW, permMask)) {
        ...
        if (isExfatFs) {
            if (Exfat::doMount(devicePath, getMountpoint(), false, false, false, AID_MEDIA_RW, AID_MEDIA_RW, permMask)) {
