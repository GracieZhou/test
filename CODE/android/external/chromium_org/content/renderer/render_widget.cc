...
void RenderWidget::OnHandleInputEvent(const blink::WebInputEvent* input_event,
  ...
  if (WebInputEvent::isKeyboardEventType(input_event->type)) {
    ...
    if (key_event.nativeKeyCode != AKEYCODE_TAB
        && key_event.windowsKeyCode != VK_RETURN) {
    // MStar Android Patch End
      ime_event_guard_maybe.reset(new ImeEventGuard(this));
    }

    // MStar Android Patch Begin
    // Press Enter key to pop up soft keyboard
    if (key_event.windowsKeyCode == VK_RETURN){
        if (input_event->type == WebInputEvent::KeyDown
            || input_event->type == WebInputEvent::RawKeyDown) {
          OnShowImeIfNeeded();
        }
    }
    // MStar Anroid Patch End
  ...
  if (WebInputEvent::isKeyboardEventType(input_event->type)) {
    ...
    // press enter key to start soft keyboad when input Element in focused
    const WebKeyboardEvent& key_event =
        *static_cast<const WebKeyboardEvent*>(input_event);
    if (key_event.windowsKeyCode == VK_RETURN) {
      if (input_event->type == WebInputEvent::KeyDown
          || input_event->type == WebInputEvent::RawKeyDown) {
       OnShowImeIfNeeded();
       // don't prevent default here otherwise the WebKit can't transport 
       // the KeyDown event to the page.
       // prevent_default = true;
      }
    } else if (key_event.nativeKeyCode == AKEYCODE_DPAD_CENTER &&
  ...
  if (!WebInputEvent::isTouchEventType(input_event->type)
    && (input_event->type != WebInputEvent::Char || !suppress_next_char_events_)) {
  // MStar Android Patch End
    suppress_next_char_events_ = false;
  ...
  // MStar Android Patch Begin
  if (WebInputEvent::isTouchEventType(input_event->type) && ack_result == INPUT_EVENT_ACK_STATE_NOT_CONSUMED) {
      if (!processed && webwidget_)
          processed = webwidget_->handleInputEvent(*input_event);
  }
  // MStar Android Patch End
...
void RenderWidget::showImeIfNeeded() {
    // MStar Android Patch Begin
    // After the focus switches to a text box, the soft keyboard does not pop up
