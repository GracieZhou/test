...
void HTMLFormElement::submitImplicitly(Event* event, bool fromImplicitSubmissionTrigger)
{
	...
	// only HTMLButtonElemnt with SUBMIT type and HTMLInputElemnt with SUMMIT or IMAGE input type 
	// are "canBeSuccessfulSubmitButton".Then those "canBeSuccessfulSubmitButton" which is not 
	// disable form control are "isSuccessfulSubmitButton".At last,only HTMLInputElemnt with 
	// TEXTFIELD("text" type is also treat as this) input type are "canTriggerImplicitSubmission".

	// Note that: we close this code just in order to not submit the form even if there is only
	// one <input> element embeded in <form> element.( Maybe we can close this function instead? )
    /*
