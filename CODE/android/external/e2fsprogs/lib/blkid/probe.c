...
// EosTek Patch Begin
static const char *no_label_exfat = "NO NAME        ";
...
static __u32 get_next_cluster(struct blkid_probe * probe,
                 const struct exfat_super_block *efs, __u32 cluster)
{
	__u32 *next;
	blkid_loff_t fat_offset;

	fat_offset = (blkid_le32(efs->fat_blocknr)) << efs->blocksize_bits  + (blkid_loff_t) cluster * sizeof(cluster);
	next = (__u32 *) get_buffer(probe, fat_offset, sizeof(__u32));
	if (!next)
		return 0;
	return blkid_le32(*next);
}

static struct exfat_chunk_label * search_exfat_label(struct blkid_probe *probe,const struct exfat_super_block *efs)
{
	__u32 next_cluster;
	...
	next_cluster= blkid_le32(efs->rootdir_clusnr);
	start_data_sect = blkid_le32(efs->clus_blocknr);
	next_sect_off = (next_cluster - 2) * (__u64)(1 <<(efs->block_per_clus_bits+efs->blocksize_bits));
	next_off = (start_data_sect) *(__u64)(1 << efs->blocksize_bits) + next_sect_off;

	for (;;) {
		entry = (__u8 *) get_buffer(probe, next_off, EXFAT_ENTRY_SIZE);
		if (!entry)
			return NULL;
		if (entry[0] == 0x00)
			return NULL;
		if (entry[0] == 0x83)
			return (struct exfat_chunk_label *) entry;
		next_off += EXFAT_ENTRY_SIZE;
		if (next_off % ((__u64)(1 <<(efs->block_per_clus_bits+efs->blocksize_bits)))== 0) {
			next_cluster = get_next_cluster(probe, efs, next_cluster);
			if (next_cluster < EXFAT_FIRST_DATA_CLUSTER)
				return NULL;
			if (next_cluster > EXFAT_LAST_DATA_CLUSTER)
				return NULL;
			next_sect_off = (next_cluster - 2) * (__u64)(1 <<(efs->block_per_clus_bits+efs->blocksize_bits));
			next_off = (start_data_sect) *(__u64)(1 << efs->blocksize_bits) + next_sect_off;
		}
...
static int probe_exfat(struct blkid_probe *probe,
		      ...
		      unsigned char *buf)
{
	struct exfat_super_block *efs = (struct exfat_super_block *) buf;
	__u64 nr_sectors;
	int i=0, zero = 1;

	//vertify whether this TYPE
	for (i = 0; i <13; i++)	{
		if (efs->unused1[i]) {
			zero = 0;
			break;
	...
	if (efs->blocksize_bits < EXFAT_MIN_BLKBITS ||
	    EXFAT_MAX_BLKBITS < efs->blocksize_bits) {
		return 1;
	}
	if ((efs->blocksize_bits + efs->block_per_clus_bits) > EXFAT_MAX_CLUS_BITS) {
		return 1;
	}

	nr_sectors = blkid_le64(efs->nr_sectors);

	if (!nr_sectors) {
		return 1;
	}
	if (blkid_le32(efs->fat_blocknr) < EXFAT_SB_BLOCKS) {
		return 1;
	}
	if (!blkid_le32(efs->fat_block_counts))	{
		return 1;
	}
	if (blkid_le32(efs->clus_blocknr) < EXFAT_SB_BLOCKS) {
		return 1;
	}
	if (!blkid_le32(efs->total_clusters) ||
	    (blkid_le32(efs->total_clusters) << efs->block_per_clus_bits) > nr_sectors) {
		return 1;
	}
	if (blkid_le32(efs->rootdir_clusnr) < EXFAT_START_ENT) {
		return 1;
	}

	if (blkid_le16(efs->signature) != BOOT_SIG) {
		return 1;
	}

	//get exfat "UUID"
	char  uuid_str[9];
	sprintf(uuid_str, "%08X", blkid_le32(efs->serial_number));
	blkid_set_tag(probe->dev, "UUID", uuid_str, 0);

	//get exfat "LABEL"
	struct exfat_chunk_label * label=NULL;
	label = search_exfat_label(probe,efs);
	if (label != NULL) {
		char utf8_label[128];
		unicode_16le_to_utf8(utf8_label, sizeof(utf8_label), label->name, label->len * 2);
		blkid_set_tag(probe->dev, "LABEL", utf8_label, 0);
	} else {
		blkid_set_tag(probe->dev, "LABEL", no_label_exfat, EXFAT_CHUNK_NAME_SIZE);
	}
...
/*  type     kboff   sboff len  magic			probe */
  ...
  { "exfat",     0,    0x3,  8, "EXFAT   ",             probe_exfat},
