...
public class Controller
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        boolean vipMode = Device.isVipMode(mActivity);
        if (!vipMode) {
            inflater.inflate(R.menu.browser_simple, menu);
        } else {
    ...
    public void onCreateContextMenu(ContextMenu menu, View v,
        ...
        boolean vipMode = Device.isVipMode(mActivity);
        if (!vipMode) {
            inflater.inflate(R.menu.browsercontext_simple, menu);
        } else {
            ...
            case WebView.HitTestResult.IMAGE_TYPE:
                ...
                if (vipMode) {
                    menu.findItem(R.id.download_context_menu_id).setOnMenuItemClickListener(
                            new Download(mActivity, extra, webview.isPrivateBrowsingEnabled(), webview.getSettings()
                                    .getUserAgentString()));
                }
    ...
    private boolean handleVirtualMouseKey(KeyEvent event) {
        ...
        if ((event.getKeyCode() >= KeyEvent.KEYCODE_DPAD_UP
            && event.getKeyCode() <= KeyEvent.KEYCODE_DPAD_RIGHT) || event.getKeyCode() == KeyEvent.KEYCODE_ENTER || event.getKeyCode() == KeyEvent.KEYCODE_DPAD_CENTER) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                WebView w = getCurrentTopWebView();
                if (w == null) {
                    return false;
            ...
            case KeyEvent.KEYCODE_PROG_GREEN:
                if (event.getRepeatCount() == 0) {
                    ...
                    if (Device.isVipMode(mActivity)) {
                        ...
                        mActivity.startService(mTaskSwitchIntent);
                    }
                }
                return true;
             // EosTek Patch End
...
//          case KeyEvent.KEYCODE_Z:    // unused
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        switch(keyCode) {
            case KeyEvent.KEYCODE_BACK:
             ...
             // EosTek Patch Begin
            case KeyEvent.KEYCODE_PROG_GREEN:
                    if ((!isLongPressKey)&&Device.isVipMode(mActivity)) {
                        Intent intent = new Intent(Intent.ACTION_MAIN);
                        intent.setClassName("com.eostek.scifly.devicemanager",
                                "com.eostek.scifly.devicemanager.DeviceManagerActivity");
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN &&
            event.getKeyCode() == KeyEvent.KEYCODE_PROG_YELLOW &&
...
//		    return true;
//		}
		if(event.getKeyCode() == MKeyEvent.KEYCODE_TV_REVEAL){
            if (Device.isVipMode(mActivity)) {    
		...
		if(event.getKeyCode() == MKeyEvent.KEYCODE_MSTAR_UPDATE){
		   Intent intent = new Intent(Intent.ACTION_MAIN);
		...
		if(event.getKeyCode() == MKeyEvent.KEYCODE_SUBTITLE){
		   if(!Device.isVipMode(mActivity)){
				return true;
		   }			
