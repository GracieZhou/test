...
import com.mstar.tv.IFactoryDesk.EN_VD_SIGNALTYPE;
import com.mstar.tv.IFactoryDesk.MAPI_AVD_VideoStandardType;
import com.mstar.tv.IFactoryDesk.ST_MAPI_VIDEO_WINDOW_INFO;
...
public class FactoryDeskImpl {
    ...
    public void setWbGainOffsetEx(ColorTemperatureExData data, EnumInputSource source,
            EnumColorTemperature temperature) {
        try {
            TvManager.getInstance().getDatabaseManager()
                    .setDatabaseDirtyByApplication(IFactoryDesk.T_USER_COLORTEMP_EX_IDX);
            TvManager
                    .getInstance()
                    .getFactoryManager()
                    .setWbGainOffsetEx(temperature, data.redGain, data.greenGain, data.blueGain,
                            data.redOffset, data.greenOffset, data.blueOffset, source);
        } catch (TvCommonException e) {
            e.printStackTrace();
        }
        factoryDB.updateFactoryColorTempExData(data, source.ordinal(), temperature.ordinal());
    }
