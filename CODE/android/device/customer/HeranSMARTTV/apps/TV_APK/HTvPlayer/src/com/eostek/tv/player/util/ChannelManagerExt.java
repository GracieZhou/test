...
public class ChannelManagerExt {
    ...
    private static final int DELAYCHANGECHANNEL = 300;
    ...
    public void getAllChannels(Context context, int inputSource) {
        ...
        if (mChannels.size() == 0) {
            Toast.makeText(context, R.string.noprogram_tip, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    public void sendMessage() {
        ...
        mChannelHandler.sendMessageDelayed(msg, 0);
    }
    ...
    public String getCurProgramName(ProgramInfo info) {
        TvEpgManager mTvEpgManager = TvEpgManager.getInstance();
        DtvEitInfo mDtvEitInfo = null;
        PresentFollowingEventInfo pfEvtInfo = mTvEpgManager.getEpgPresentFollowingEventInfo(info.serviceType,
                info.number, true, TvEpgManager.EPG_DETAIL_DESCRIPTION);
        ...
        if (null != pfEvtInfo) {
            mDtvEitInfo = mTvEpgManager.getEitInfo(true);
        } else {
            mDtvEitInfo = null;
        }
        if (mDtvEitInfo == null) {
            ...
            return mDtvEitInfo.eitCurrentEventPf.eventName;
        }
    }
    ...
    public void setNosignalTips(Context context, int source, SignalTipView mSignalTipView) {
        switch (source) {
            ...
            case TvCommonManager.INPUT_SOURCE_HDMI:
                mSignalTipView.setText("HDMI1 " + context.getResources().getString(R.string.nosignaltips));
                break;
            ...
            case TvCommonManager.INPUT_SOURCE_HDMI3:
                mSignalTipView.setText("HDMI3 " + context.getResources().getString(R.string.nosignaltips));
                break;
