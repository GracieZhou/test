...
public abstract class InfoControlView extends FrameLayout {
    ...
    protected void handleImage(List<AdInfo> adinfos) {
        ...
        if (PlayerActivity.sHasDiskCache) {
        ...
        for (AdInfo adinfo : adinfos) {
            ...
            // the size model is 400X500
            String[] size = adinfo.getSiz().split("X");
            int width = Integer.parseInt(size[0]);
            int height = Integer.parseInt(size[1]);
            ImageSize targetSize = new ImageSize(width, height);
//            ImageSize targetSize = new ImageSize(mContext.getResources().getInteger(R.integer.ad_target_image_width),
//                    mContext.getResources().getInteger(R.integer.ad_target_image_height));
    ...
    public void dismissInfoView() {
        hasShow = false;
        ObjectAnimator translationUp = ObjectAnimator.ofFloat(mOSDLayout, "Y",
    ...
    protected void show() {
        ...
        mOSDLayout.setX(mContext.getResources().getInteger(R.integer.ChannelInfo_details_animator_x));
        mOSDLayout.setY(mContext.getResources().getInteger(R.integer.ChannelInfo_details_animator_height));
