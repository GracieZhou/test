...
public class OrderReminderDialog extends AlertDialog {
    ...
    private com.heran.launcher.ui.FocusView focusView;

    private final int EPGTIMER_COUNTDOWN_LEADING_TIMES = 10;

    private final int EPGTIMER_RECORDER_LEADING_TIMES = 5;

    private int mChannelNum = -1; // EPG timer event type none/remider/recorder
    ...
    private void init() {
        ...
        focusView = (com.heran.launcher.ui.FocusView) findViewById(R.id.focus_selector);
        int count = 0;
        try {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (info.startTime > (int) ((System.currentTimeMillis() / 1000))
                    ...
                    if (EnumEventTimerType.values()[info.enTimerType] == EnumEventTimerType.EPG_EVENT_REMIDER) {
                        ((TextView) findViewById(R.id.reminder_title)).setText(R.string.order_remind);
                    } else if (EnumEventTimerType.values()[info.enTimerType] == EnumEventTimerType.EPG_EVENT_RECORDER) {
                        ((TextView) findViewById(R.id.reminder_title)).setText(R.string.order_record_remind);
                    }
                    ...
                    List<EpgEventInfo> mEpgEventInfo = mChannelManager.getEventInfo((short) info.serviceType,
                            info.serviceNumber, startTime, 1);
            ...
            public void onClick(View view) {
                ...
                try {
                    mTimerManager.cancelEpgTimerEvent((int) ((currTime.toMillis(true) / 1000)
                            + EPGTIMER_COUNTDOWN_LEADING_TIMES + EPGTIMER_RECORDER_LEADING_TIMES), false);
                } catch (TvCommonException e) {
                    e.printStackTrace();
                }
                dismiss();
