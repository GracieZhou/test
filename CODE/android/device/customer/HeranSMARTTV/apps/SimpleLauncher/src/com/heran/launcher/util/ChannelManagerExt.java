...
/**
 * @projectName： EosTvPlayer @moduleName： ChannelManagerExt.java
 * 
...
public class ChannelManagerExt {
    ...
    private int curSource = TvCommonManager.INPUT_SOURCE_NONE;
    ...
    public ProgramInfo getCurProgramInfo() {
        return TvChannelManager.getInstance().getCurrentProgramInfo();
    }
...
//        TvChannelManager.getInstance().getProgramInfoByIndex(programIndex);
        return pi;
    }
     ...
     * @param position
     */
    public void programNumSel(int position) {
        if (position >= mChannelsAll.size()) {
            return;
        }
        mLastPosition = mCurPosition;
        mCurPosition = mChannels.indexOf(mChannelsAll.get(position));
     ...
     * @param inputSource
     */
    public void getAllChannels(Context context, int inputSource) {
        ...
        switch (inputSource) {
            case TvCommonManager.INPUT_SOURCE_ATV: {
                ...
                channelconunt = TvChannelManager.getInstance().getProgramCount(TvChannelManager.PROGRAM_COUNT_ATV_DTV);
            ...
            case TvCommonManager.INPUT_SOURCE_DTV: {
                int dataCount = TvChannelManager.getInstance().getProgramCount(TvChannelManager.PROGRAM_COUNT_DTV_DATA);
                channelconunt = TvChannelManager.getInstance().getProgramCount(TvChannelManager.PROGRAM_COUNT_DTV)
                        - dataCount;
    ...
    public ProgramInfo getCurInfo() {
        if (curSource == TvCommonManager.INPUT_SOURCE_DTV && mChannels.size() <= 0) {
            return null;
        }
        return mCurInfo;
    }
    ...
    public void setProgramAttribute(EnumProgramAttribute enpa, int programNo, int programType, int programId,
        ...
        try {
            TvManager.getInstance().getChannelManager().setProgramAttribute(enpa, programNo, (short) programType,
                    programId, bv);
        } catch (TvCommonException e) {
            e.printStackTrace();
        }
    ...
    public void setNosignalTips(Context context, int source, SignalTipView mSignalTipView) {
        switch (source) {
            case TvCommonManager.INPUT_SOURCE_ATV:
     ...
     * @param context
     */
    public void setTVWindow(int curSourceType, Activity context) {
        // EnumVideoArcType zoomMode =
        // TvPictureManager.getInstance().getVideoArc();
        int zoomMode = TvPictureManager.getInstance().getVideoArcType();
