...
public class ScreenProtection {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case Constants.DELAY_SHOW_SIGNAL_MSG:
                    ...
                    try {
                        if (TvManager.getInstance() != null) {
                            currentInputSource = TvManager.getInstance().getCurrentInputSource().ordinal();
                        }
                ...
                case Constants.SHOW_SIGNAL_TIPS:
                    ...
                    try {
                        if (TvManager.getInstance() != null) {
                            curInputSource = TvManager.getInstance().getCurrentInputSource().ordinal();
                        }
                    } catch (TvCommonException e) {
                        e.printStackTrace();
                    }
    ...
    public void setSignalTipView(int currentInputSource) {
        Log.v(TAG, "current input source:" + currentInputSource);
    ...
    private class TvPlayerEventListener implements OnTvPlayerEventListener {
        ...
        public TvPlayerEventListener() {
            try {
                if (TvManager.getInstance() != null) {
                    mSource = TvManager.getInstance().getCurrentInputSource().ordinal();
                }
            } catch (TvCommonException e) {
                e.printStackTrace();
            }
