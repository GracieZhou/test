...
public class ProxyConfig implements Parcelable {
    ...
    /**
     * Constructs a {@link ProxyConfig} object that points at a Direct proxy
     * on the specified host and port.
     */
    public static ProxyConfig buildDirectProxy(String host, int port) {
     ...
     */
    public static ProxyConfig buildDirectProxy(String host, int port, List<String> exclList) {
        String[] array = exclList.toArray(new String[exclList.size()]);
        return new ProxyConfig(host, port, TextUtils.join(",", array), array);
    }

    /**
     * Construct a {@link ProxyConfig} that will download and run the PAC script
     * at the specified URL.
     */
    public static ProxyConfig buildPacProxy(Uri pacUri) {
        return new ProxyConfig(pacUri);
    }
     ...
     * @hide
     */
    public ProxyConfig(String host, int port, String exclList) {
        mHost = host;
        mPort = port;
     ...
     * @hide
     */
    public ProxyConfig(Uri pacFileUrl) {
        mHost = LOCAL_HOST;
        mPort = LOCAL_PORT;
     ...
     * @hide
     */
    public ProxyConfig(String pacFileUrl) {
        mHost = LOCAL_HOST;
        mPort = LOCAL_PORT;
        setExclusionList(LOCAL_EXCL_LIST);
     ...
     * @hide
     */
    public ProxyConfig(Uri pacFileUrl, int localProxyPort) {
        mHost = LOCAL_HOST;
    ...
    private ProxyConfig(String host, int port, String exclList, String[] parsedExclList) {
        mHost = host;
        mPort = port;
...
//    }
    ...
    public static final Creator<ProxyConfig> CREATOR =
        new Creator<ProxyConfig>() {
            public ProxyConfig createFromParcel(Parcel in) {
                ...
                if (in.readByte() != 0) {
                    ...
                    return new ProxyConfig(url, localPort);
                }
                if (in.readByte() != 0) {
                    host = in.readString();
                    port = in.readInt();
                ...
                // FXIME String[] parsedExclList = in.readStringArray();
                ProxyConfig proxyProperties =
                        new ProxyConfig(host, port, exclList, null);
