...
public class AccessPoint extends Preference {
    ...
    AccessPoint(Context context, WifiConfiguration config) {
        super(context);
        mContext = context;
        setLayoutResource(R.layout.wifi_custom_preference);
        loadConfig(config);
        refresh();
    }
    ...
    AccessPoint(Context context, Bundle savedState) {
        ...
        mContext = context;
        // setWidgetLayoutResource(R.layout.preference_widget_wifi_signal);
        setWidgetLayoutResource(R.layout.wifi_custom_preference);
    ...
    public void refresh() {
        ...
        if (mConfig != null && mConfig.status == WifiConfiguration.Status.DISABLED) {
            ...
            String stateStr = Summary.get(context, mState);
            setSummary(stateStr);
            if (mState == DetailedState.CONNECTING) {
//                setWifiStateText(context.getString(R.string.wifi_display_status_connecting));
            ...
            if (mConfig != null) { // Is saved network
                if (isPasswordSaved()) {
                    ...
                    // setWifiStateText(context.getString(R.string.wifi_configured));
                }
                // Log.i(TAG, mConfig.isValid() + " :: " + mConfig.status +
                // " :: " + mConfig.SSID + " :: "
                // + mConfig.disableReason);
            }
    ...
    public WifiConfiguration buildConfigFromResult(boolean isAutoGetIp) {
...
//        config.ipAssignment = isAutoGetIp ? IpAssignment.DHCP : IpAssignment.STATIC;
