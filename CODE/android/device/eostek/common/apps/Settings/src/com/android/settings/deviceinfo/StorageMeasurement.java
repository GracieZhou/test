...
public class StorageMeasurement {
    ...
    public static final ComponentName DEFAULT_CONTAINER_COMPONENT = new ComponentName(DEFAULT_CONTAINER_PACKAGE,
            "com.android.defcontainer.DefaultContainerService");

    /** Media types to measure on external storage. */
    private static final Set<String> sMeasureMediaTypes = Sets.newHashSet(Environment.DIRECTORY_DCIM,
            Environment.DIRECTORY_MOVIES, Environment.DIRECTORY_PICTURES, Environment.DIRECTORY_MUSIC,
            Environment.DIRECTORY_ALARMS, Environment.DIRECTORY_NOTIFICATIONS, Environment.DIRECTORY_RINGTONES,
            Environment.DIRECTORY_PODCASTS, Environment.DIRECTORY_DOWNLOADS, Environment.DIRECTORY_ANDROID);
    ...
    private static class StatsObserver extends IPackageStatsObserver.Stub {
        ...
        public StatsObserver(boolean isInternal, MeasurementDetails details, int currentUser, Message finished,
                int remaining) {
        ...
        private void addStatsLocked(PackageStats stats) {
            if (mIsInternal) {
                ...
                mDetails.appsSize += stats.externalCodeSize + stats.externalDataSize + stats.externalMediaSize
    ...
    private class MeasurementHandler extends Handler {
		...
		private void measureApproximateStorage(IMediaContainerService imcs) {
			final String dataPath = mVolume != null ? mVolume.getPath()
					: Environment.getDataDirectory().getPath();
			final String sdcardPath = Environment.getExternalStorageDirectory()
					.getPath();
			final String systemPath = "/system";
			final String cachePath = "/cache";
			try {
				final long[] dataStats = imcs.getFileSystemStats(dataPath);
				final long[] sdcardStats = imcs.getFileSystemStats(sdcardPath);
				final long[] systemStats = imcs.getFileSystemStats(systemPath);
				final long[] cacheStats = imcs.getFileSystemStats(cachePath);
				mTotalSize = dataStats[0] + sdcardStats[0]+systemStats[0]+cacheStats[0];
				mAvailSize = dataStats[1] + sdcardStats[1]+systemStats[1]+cacheStats[1];
			   final StorageVolume[] storageVolumes = StorageManager.from(
						mContext.get()).getVolumeList();
				if ((Build.DEVICE.equals("heran") || Build.DEVICE
						.equals("scifly_m202_1G"))) {
					for (StorageVolume volume : storageVolumes) {
						if (Environment.MEDIA_MOUNTED.equals(volume.getState())
								&& !volume.isRemovable()) {
							mTotalSize = dataStats[0];
							mAvailSize = dataStats[1];
						}
					}
				}
        ...
        private void measureExactStorage(IMediaContainerService imcs) {
            ...
            if (mIsInternal || mIsPrimary) {
                final List<ApplicationInfo> apps = pm.getInstalledApplications(PackageManager.GET_UNINSTALLED_PACKAGES
    ...
    private long measureMisc(IMediaContainerService imcs, File dir) {
        ...
        // sort the list of FileInfo objects collected above in descending order
        // of their sizes
    ...
    static class FileInfo implements Comparable<FileInfo> {
        ...
        public int compareTo(FileInfo that) {
            if (this == that || mSize == that.mSize)
                ...
                return (mSize < that.mSize) ? 1 : -1; // for descending sort
        }
