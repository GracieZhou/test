...
import com.android.settings.deviceinfo.MoreInfoActivity;
...
public class StorageVolumePreferenceCategory extends PreferenceCategory {
    ...
    public void init() {
        ...
        if (showDetails) {
            ...
            if (showUsers) {
                ...
                for (UserInfo info : otherUsers) {
                    ...
                    final StorageItemPreference userPref = new StorageItemPreference(getContext(), info.name, colorRes,
                            info.id);
        ...
        /*
         * final boolean isRemovable = mVolume != null ? mVolume.isRemovable() :
         * false; // Always create the preference since many code rely on it
         * existing mMountTogglePreference = new Preference(context); if
         * (isRemovable) { if(mVolume.getPath().indexOf("internal_sd")>-1){
         ...
         * }else{ mMountTogglePreference.setTitle(R.string.sd_eject);
         * mMountTogglePreference.setSummary(R.string.sd_eject_summary); }
         * addPreference(mMountTogglePreference); } final boolean allowFormat =
         * mVolume != null; if (allowFormat) { mFormatPreference = new
         * Preference(context); if(mVolume.getPath().indexOf("internal_sd")>-1){
         * mFormatPreference.setTitle(R.string.nand_flash_format);
         * mFormatPreference.setSummary(R.string.nand_flash_format_summary);
         * }else{ mFormatPreference.setTitle(R.string.sd_format);
...
//          )); mMountTogglePreference.setSummary(mResources.getString(R.string.
//          sd_eject_summary)); } } else { if
...
//          .setTitle(mResources.getString(R.string.nand_flash_mount));
//         mMountTogglePreference
//          .setSummary(mResources.getString(R.string.nand_flash_mount_summary));
//          }else{
//          mMountTogglePreference.setTitle(mResources.getString(R.string.sd_mount
//          )); mMountTogglePreference.setSummary(mResources.getString(R.string.
//          sd_mount_summary)); } } else {
...
//          mMountTogglePreference.setTitle(mResources.getString(R.string.sd_mount
//          )); mMountTogglePreference.setSummary(mResources.getString(R.string.
//          sd_insert_summary)); } } removePreference(mUsageBarPreference);
//          removePreference(mItemTotal); removePreference(mItemAvailable); if
//          (mFormatPreference != null) { removePreference(mFormatPreference); }
//          } if (mUsbConnected &&
//          (UsbManager.USB_FUNCTION_MTP.equals(mUsbFunction) ||
//          UsbManager.USB_FUNCTION_PTP.equals(mUsbFunction))) {
//          mMountTogglePreference.setEnabled(false); if
//          (Environment.MEDIA_MOUNTED.equals(state) ||
//          Environment.MEDIA_MOUNTED_READ_ONLY.equals(state)) {
//          mMountTogglePreference.setSummary(
//          mResources.getString(R.string.mtp_ptp_mode_summary)); } if
//          (mFormatPreference != null) { mFormatPreference.setEnabled(false);
//       mFormatPreference
//          .setSummary(mResources.getString(R.string.mtp_ptp_mode_summary)); } }
//         else if (mFormatPreference != null) {
//          mFormatPreference.setEnabled(true);
//          if(mVolume.getPath().indexOf("internal_sd")>-1){
//          mFormatPreference.setSummary
//          (mResources.getString(R.string.nand_flash_format_summary)); }else{
//          mFormatPreference
//          .setSummary(mResources.getString(R.string.sd_format_summary)); } }
         
          
    }

    public void updateApproximate(long totalSize, long availSize) {
//        mItemTotal.setSummary(Utils.formatStorageSize(mContext, totalSize));
    	 mItemTotal.setSummary(formatSize(totalSize));
//    	 mItemTotal.setSummary(getFlashSize());
    ...
    public void updateDetails(MeasurementDetails details) {
   	 ...
   	 if (Build.DEVICE.equals("heran")||Build.DEVICE.equals("scifly_m202_1G")){
   		 details.totalSize=2*details.totalSize;
   		 details.availSize=2*details.availSize;
     }
        // Count caches as available space, since system manages them
        mItemTotal.setSummary(Utils.formatStorageSize(mContext, details.totalSize));
//        mItemTotal.setSummary(Utils.formatStorageSize(mContext,details.totalSize));
