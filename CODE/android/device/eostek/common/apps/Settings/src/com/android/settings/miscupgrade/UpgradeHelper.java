...
public class UpgradeHelper {
	...
	public UpgradeResult upgrade(String externalFilePath, String systemFilePath) {
		if (TextUtils.isEmpty(externalFilePath)
				|| TextUtils.isEmpty(systemFilePath)) {
		...
		if (extFile.exists() && extFile.canRead()) {
			...
			if (sysFile.exists()) {
				...
				if (sysMd5.equals(extMd5)) {
					Log.i(TAG, String.format(
							"sys file(%s), ext file(%s), md5sum(%s)\n",
							systemFilePath, externalFilePath, sysMd5));
			...
			// upgrade now
			boolean isVideo = UpgradeConstants.USER_BOOTVIDEO_NAME
					.equals(extFile.getName());
			boolean bcopy = false;
			...
			if (isVideo) {
				...
				bcopy = mShell.exec("cp " + externalFilePath + " "
						+ systemFilePath);
			}
			...
			if (bcopy) {
				...
				if (isVideo) {
					int ret = FileUtils.setPermissions(sysFile, 0644, 1000,
							1000);

					if (0 != ret) {
						Log.e(TAG, "Set permissions failed with file :"
								+ systemFilePath);
					...
					if (!bcopy) {
						Log.e(TAG, "eshell chmod 644 " + systemFilePath
								+ " failed.");
						return UpgradeResult.UPGRADE_SET_PERMISSION_FAILED;
					}
				}
				...
				Log.e(TAG, "Copy file " + externalFilePath + " to "
						+ systemFilePath + " failed.");
	...
	public UpgradeResult upgrade(String extLauncherPath) {
		...
		if (extFile.exists()) {
			...
			if (null == userLauncherPath) {
				...
				// check md5sum first
				String userLauncherMd5 = Util.getFileMD5(new File(
						userLauncherPath));
				String extMd5 = Util.getFileMD5(extFile);
	...
	private UpgradeResult installUserLauncher(String codePath) {
		...
		mPm = IPackageManager.Stub.asInterface(ServiceManager
				.getService("package"));
		// } else {
		// mPm =new EmptyPackageManagerService();
		// }
		if (mPm == null) {
		...
		// if(sysVersion<=21){
		// mPm.installPackageWithVerificationAndEncryption(apkURI, obs,
		// installFlags, null, verificationParams, null);
		// }
		// for 828
		// String abi="- ";
	...
	public static String getUserLauncherPath(Context context) {
		...
		Log.i(TAG, "getUserLauncher total spent " + String.valueOf(spent)
				+ " ms");
