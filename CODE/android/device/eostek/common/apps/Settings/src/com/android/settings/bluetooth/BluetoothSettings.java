...
public class BluetoothSettings extends DeviceListPreferenceFragment {
    ...
    private static final String TAG_CONFIRM_UNPAIR = "confirmUnpair";
    ...
    private Preference mScanDevicesPreference;
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothAdapter.ACTION_LOCAL_NAME_CHANGED)) {
                ...
                mScanDevicesPreference.setSummary(getString(R.string.bluetooth_searching_for_devices));
            } else if (action.equals(BluetoothAdapter.ACTION_DISCOVERY_FINISHED)) {
                mScanDevicesPreference.setSummary(getString(R.string.bluetooth_searching_finished));
            }
        }
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mActivityStarted = (savedInstanceState == null); // don't auto start
                                                         // scan after rotation
        ...
        insertCheckbox();
        if (mBluetoothCheckBox.isChecked()) {
            addScanDevicesPreference();
        }
    }
    ...
    public void onResume() {
        ...
        // get
        // any onDeviceAdded() callbacks before setting up view in
        // updateContent()
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        if (mLocalAdapter == null)
            return;
        // If the user is not allowed to configure bluetooth, do not show the
        // menu.
        ...
        menu.add(Menu.NONE, MENU_ID_SCAN, 0, textId).setEnabled(bluetoothIsEnabled && !isDiscovering)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_IF_ROOM);
        menu.add(Menu.NONE, MENU_ID_RENAME_DEVICE, 0, R.string.bluetooth_rename_device).setEnabled(bluetoothIsEnabled)
                .setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
        menu.add(Menu.NONE, MENU_ID_VISIBILITY_TIMEOUT, 0, R.string.bluetooth_visibility_timeout)
                .setEnabled(bluetoothIsEnabled).setShowAsAction(MenuItem.SHOW_AS_ACTION_NEVER);
        menu.add(Menu.NONE, MENU_ID_SHOW_RECEIVED, 0, R.string.bluetooth_show_received_files).setShowAsAction(
                MenuItem.SHOW_AS_ACTION_NEVER);
        // MStar Android Patch Begin
        // porting from brcm
        // com.broadcom.bt.app.settings.AdvancedSettings.addAdvancedSettingsMenu(getActivity(),menu,bluetoothIsEnabled);
        com.android.settings.bluetooth.AdvancedSettings
                .addAdvancedSettingsMenu(getActivity(), menu, bluetoothIsEnabled);
        // MStar Android Patch End
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        // porting from brcm
        // if
        // (com.broadcom.bt.app.settings.AdvancedSettings.handleOptionsItemSelected(getActivity(),
        // item)) {
        // return true;
        // }
        // MStar Android Patch End
        if (com.android.settings.bluetooth.AdvancedSettings.handleOptionsItemSelected(getActivity(), item)) {
            return true;
        }
    ...
    private void addDeviceCategory(PreferenceGroup preferenceGroup, int titleId, BluetoothDeviceFilter.Filter filter) {
        ...
        switch (bluetoothState) {
            case BluetoothAdapter.STATE_ON:
                ...
                if (getResources().getBoolean(com.android.internal.R.bool.config_voice_capable)) {
                    mMyDevicePreference.setIcon(R.drawable.ic_bt_cellphone); // for
                                                                             // phones
                } else {
                    mMyDevicePreference.setIcon(R.drawable.ic_bt_laptop); // for
                                                                          // tablets,
                                                                          // etc.
                }
                ...
                if (mDiscoverableEnabler == null) {
                    mDiscoverableEnabler = new BluetoothDiscoverableEnabler(getActivity(), mLocalAdapter,
                            mMyDevicePreference);
                    mDiscoverableEnabler.resume();
    ...
    private final View.OnClickListener mDeviceProfilesListener = new View.OnClickListener() {
        public void onClick(View v) {
            ...
            if (v.getTag() instanceof CachedBluetoothDevice) {
                ...
                ((PreferenceActivity) getActivity()).startPreferencePanel(DeviceProfilesSettings.class.getName(), args,
                        R.string.bluetooth_device_advanced_title, null, null, 0);
            } else {
                Log.w(TAG, "onClick() called for other View: " + v); // TODO
                                                                     // remove
            }
        }
    };
    ...
    private void insertCheckbox() {// add by ljh for adding a checkbox switch
        if (mBluetoothCheckBox == null) {
            ...
            mBluetoothCheckBox.setLayoutResource(R.layout.bluetooth_preference_check_layout);
            mBluetoothCheckBox.setWidgetLayoutResource(R.layout.bluetooth_enable_checkbox);
            mBluetoothCheckBox.setKey("switch");
            mBluetoothCheckBox.setTitle(R.string.bluetooth);
    ...
    private void addScanDevicesPreference() {
        if (mScanDevicesPreference == null) {
            mScanDevicesPreference = new Preference(getActivity());
            mScanDevicesPreference.setKey("switch_scan");
            mScanDevicesPreference.setIcon(R.drawable.ic_bt_search); // search
                                                                     // icon
            mScanDevicesPreference.setTitle(R.string.bluetooth_search_for_devices);
            mScanDevicesPreference.setSummary(getString(R.string.bluetooth_search_summary));
            mScanDevicesPreference.setPersistent(false);
            mScanDevicesPreference.setEnabled(true);
        }

        if (getPreferenceScreen().findPreference("switch_scan") == null) {
            getPreferenceScreen().addPreference(mScanDevicesPreference);
        }
    }

    private void removeAllPreferences() {// add by ljh for adding a checkbox
                                         // switch
    ...
    void onOtherPreferenceClick(PreferenceScreen preferenceScreen, Preference preference) {// add
                                                                                           // by
                                                                                           // ljh
                                                                                           // for
                                                                                           // adding
                                                                                           ...
                                                                                           // checkbox
                                                                                           // switch
        if (preference == mBluetoothCheckBox) {// sync bluetooth switch ,add by
                                               // ljh
            mLocalAdapter.setBluetoothEnabled(mBluetoothCheckBox.isChecked());
        }
        if (preference == mScanDevicesPreference) {
            if (mLocalAdapter.getBluetoothState() == BluetoothAdapter.STATE_ON) {
                startScanning();
            }
        }
