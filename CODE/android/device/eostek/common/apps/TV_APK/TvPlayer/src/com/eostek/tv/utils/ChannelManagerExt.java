...
public class ChannelManagerExt {
     ...
     */
    public List<ProgramInfo> mChannels = new ArrayList<ProgramInfo>();
    ...
    private static final int DELAYCHANGEMUTE = 500;
    ...
    private Handler mChannelHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            }else if(DELAYCHANGEMUTE == msg.what){
            	boolean value = false;
            	
            	ProgramInfo pinfo = getCurProgramInfo();
        		if (pinfo.isLock)
        			value = true;
    ...
    public boolean programSel(int number, int serviceType) {
        ...
        mChannelHandler.sendMessageDelayed(mChannelHandler.obtainMessage(DELAYCHANGEMUTE), 500);
    ...
    public static boolean isMasterMute() {
    	...
    	Context context = TvApplication.getInstance();
        try {
    ...
    public void programNumSel(int position) {
        ...
        if (mChannelsAll.get(position).isSkip) {
            position += 1;
            position %= mChannelsAll.size();
            programNumSel(position);
            return;
        }
	...
	public boolean unlockChannel() {
		try {
			...
			boolean ret = DtvManager.getDvbPlayerManager().unlockChannel();
			setMute(false);
