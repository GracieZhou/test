...
public class PackageInstallerActivity extends Activity implements OnCancelListener, OnClickListener {
    ...
    //Eostek Path Begin
    public PakageInstallerHolder mHolder;
    public PakageInstallerLogic mLogic;
    //Eostek Path End
    ...
    protected void onCreate(Bundle icicle) {
        ...
        Log.i(TAG, "SciflyPackageInstaller, Version:1.0.0 Date:2015-08-14, Publisher:Shirley.jiang REV:39605");
    ...
    public void initData(Intent intent){
        ...
        mHolder = new PakageInstallerHolder(this);
        mLogic = new PakageInstallerLogic(this);
        mHolder.setmLogic(mLogic);
        mLogic.setmHolder(mHolder);
        
        Log.d(TAG, "intent : " + intent.toString());
        Log.d(TAG, "uri : " + mPackageURI.getPath());
        switch (isBatchInstall()) {
            case 1: //is batch install
                mHolder.mInstallCategory = PakageInstallerHolder.BATCH_INSTALL;
                mHolder.what = PakageInstallerHolder.CATEGORY_SELECT;

                try {
                    mEntriesNames = ZipUtils.getEntriesNames(mStr);
                    Log.d(TAG, "entriesNames : " + mEntriesNames.toString());
            ...
            case 0:  //is signal install
                if (!mLogic.getSingleInstallApk(mPackageURI.getPath())) {
                    finish();
                    break;
                }
                mHolder.mInstallCategory = PakageInstallerHolder.SINGLE_INSTALL;
                mHolder.what = PakageInstallerHolder.CATEGORY_SINGLE_INSTALL_CONFIRM;
                mHolder.findViews();
                mHolder.regisertListener();
                mHolder.refreshUI();
                break;
            case 2: //is signal install
                if (!mLogic.getSingleInstallApk(mStr)) {
                    finish();
                    break;
                }
                mHolder.mInstallCategory = PakageInstallerHolder.SINGLE_INSTALL;
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            switch(keyCode) {
                case KeyEvent.KEYCODE_DPAD_DOWN:
                    if (mHolder.mInstallCategory == PakageInstallerHolder.BATCH_INSTALL && mHolder.what == PakageInstallerHolder.CATEGORY_SELECT) {
                        if (mHolder.getmListView().isFocused()) {
                            mHolder.getSelect_all().requestFocus();
                        } else if (mHolder.getSelect_all().isFocused()) {
                            mHolder.getConfirm_install_btn().requestFocus();
                        }
                    }
                    break;
                case KeyEvent.KEYCODE_DPAD_UP:
                    if (mHolder.mInstallCategory == PakageInstallerHolder.BATCH_INSTALL && mHolder.what == PakageInstallerHolder.CATEGORY_SELECT) {
                        if (mHolder.getConfirm_install_btn().isFocused() || mHolder.getCancle_install_btn().isFocused()) {
                            mHolder.getSelect_all().requestFocus();
                        } else if (mHolder.getSelect_all().isFocused()) {
                            mHolder.getmListView().requestFocus();
                        }
                    }
                    break;
    ...
    @Override
    public void finish() {
        //调用finish的时候将解压的文件全部删除
        String unzipPath = null;
        try {
            unzipPath = ZipUtils.getDestPath("", PackageInstallerActivity.this);
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (unzipPath != null) {
            ...
            if (files != null && files.length != 0) {
                for (File apkFile : files) {
                    apkFile.delete();
                    Log.e(TAG, "apk delete " + apkFile.getPath());
                }
            }
        }
