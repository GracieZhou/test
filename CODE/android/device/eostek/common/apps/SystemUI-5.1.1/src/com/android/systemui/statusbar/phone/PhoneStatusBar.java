...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode,
    ...
    // EosTek Patch Begin
    int mSystemUiVisibility = View.SYSTEM_UI_FLAG_LOW_PROFILE;
    ...
    public void start() {
        // EosTek Patch Begin
        Log.d(TAG, "Name : SystemUI, Version : 4.1.0, Date : 2016-02-01, Publisher : Felix.Hu, Revision : 50435");
        // EosTek Patch End
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        // EosTekk Patch Begin
        filter.addAction(USERDATA_RECOVER);
        ...
        // EosTek Patch Begin
        String isRecovery = SciflyStore.Global.getString(context.getContentResolver(), Global.RECOVERY_LABEL);
        if (TextUtils.isEmpty(isRecovery)) {
            Intent i = new Intent(USERDATA_RECOVER);
    ...
    public int getStatusBarHeight() {
        if (mNaturalBarHeight < 0) {
            ...
            // EosTek Patch Begin
            mNaturalBarHeight = res.getDimensionPixelSize(R.dimen.status_bar_height);
            // EosTek Patch End
        }
    ...
    public void addIcon(String slot, int index, int viewIndex, StatusBarIcon icon) {
        if (SPEW) Log.d(TAG, "addIcon slot=" + slot + " index=" + index + " viewIndex=" + viewIndex
                + " icon=" + icon);
        // EosTek Patch Begin
        if (slot.equals("message_state")) {
            // IM is online
            if (icon.iconId == com.android.internal.R.drawable.status_sys_im_online
                    || icon.iconId == com.android.internal.R.drawable.status_sys_im_online_message) {
                mNetworkController.updateNetworkStatusIcons(true);
            }
            // IM is offline
            if (icon.iconId == com.android.internal.R.drawable.status_sys_im_offline
                    || icon.iconId == com.android.internal.R.drawable.status_sys_im_offline_message) {
                mNetworkController.updateNetworkStatusIcons(false);
            }
            // IM is connected
            if (icon.iconId == com.android.internal.R.drawable.status_sys_im_connected
                    || icon.iconId == com.android.internal.R.drawable.status_sys_im_connected_message) {
                mNetworkController.updateNetworkStatusIcons(true);      
            }
        }
        // do not show ime status icon
        if ("ime".equals(slot)) {
            Log.d(TAG, "addIcon, ime status icon visible : " + icon.visible);
            icon.visible = false;
        }
        Log.d(TAG, "addIcon slot=" + slot + " index=" + index + " viewIndex=" + viewIndex + " icon=" + icon);
        // EosTek Patch End
    ...
    public void updateIcon(String slot, int index, int viewIndex,
        ...
        // do not show ime status icon
        if ("ime".equals(slot)) {
            Log.d(TAG, "updateIcon, ignore ime status icon.");
            return;
        }
        // EosTek Patch End
    ...
    private void updateNotificationIcons() {
        // EosTek Patch Begin
        final LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(mIconSize, mIconSize);
        final Resources res = mContext.getResources();
        params.leftMargin = res.getDimensionPixelSize(R.dimen.notification_icon_margin_right);
        Log.d(TAG, "params.leftMargin : " + params.leftMargin + ", params.rightMargin : " + params.rightMargin);
        // EosTek Patch End
        ...
        for (int i = 0; i < N; i++) {
            ...
            // EosTek Patch Begin
            if (ent.notification.getNotification().icon == R.drawable.status_sys_wifi_signal_0
                    || ent.notification.getNotification().icon == R.drawable.status_sys_wifi_signal_4
                    || ent.notification.getNotification().icon == R.drawable.status_sys_ethernet_connected
                    || ent.notification.getNotification().icon == R.drawable.status_sys_ethernet_no_network
                    || ent.notification.getNotification().icon == com.android.internal.R.drawable.ic_notification_ime_default) {
                continue;
            }
            // EosTek Patch End
            toShow.add(ent.icon);
    ...
    public void showClock(boolean show) {
        // EosTek Patch Begin
        // comment : fix mantis 0029789; invisible clock icon
        show = false;
        // EosTek Patch End
        ...
        // EosTek Patch Begin
        View line = mStatusBarView.findViewById(R.id.vertical_line);
        if (line != null) {
    ...
    public void setSystemUiVisibility(int vis, int mask) {
        // EosTek Patch Begin
        if (mask != 0xffffeeee) {
            Log.w(TAG, String.format("vis : %s, wrong mask : %s", Integer.toHexString(vis), Integer.toHexString(mask)));
            return;
        }
        Log.d(TAG, String.format("vis : %s, mask : %s", Integer.toHexString(vis), Integer.toHexString(mask)));
        // vis must be View.SYSTEM_UI_FLAG_LOW_PROFILE or View.SYSTEM_UI_FLAG_VISIBLE
        if (vis == View.SYSTEM_UI_FLAG_LOW_PROFILE) {
            mStatusBarContents.setVisibility(View.INVISIBLE);
            mStatusBarContents.startAnimation(loadAnim(com.android.internal.R.anim.push_down_out, null));
        } else if (vis == View.SYSTEM_UI_FLAG_VISIBLE) {
            mStatusBarContents.setVisibility(View.VISIBLE);
            mStatusBarContents.startAnimation(loadAnim(com.android.internal.R.anim.push_up_in, null));
        } else {
            Log.w(TAG, "invalid vis : " + Integer.toHexString(vis));
            return;
        }
        // EosTek Patch End
    ...
    private class MyTicker extends Ticker {
        ...
        public void tickerHalting() {
            if (!mTickerEnabled) return;
            // EosTek Patch Begin
            if (mStatusBarContents.getVisibility() != View.VISIBLE && areLightsOn()) {
            // EosTek Patch End
                mStatusBarContents.setVisibility(View.VISIBLE);
                mStatusBarContents
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            // EosTek Patch Begin
                ...
                final int visible = adjustClockVisibility();
                showClock(visible == View.VISIBLE);
            }
            // EosTek Patch End
        }
    };
    ...
    protected void loadDimens() {
        ...
        int newIconSize = res.getDimensionPixelSize(R.dimen.status_bar_icon_size);
        // EosTek Patch End
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    // EosTek Patch Begin
    private int adjustClockVisibility() {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy");
        final String mSystemTime = sdf.format(Calendar.getInstance().getTime());
        int mYear = 2014;
        try {
            mYear = Integer.parseInt(mSystemTime);
        } catch (NumberFormatException e) {
        }
        if (mYear <= 2014) {
            if (DEBUG) {
                Log.d(TAG, "current system time : " + mSystemTime);
            }
            return View.GONE;
        }
    ...
    private boolean areInTv() {
        ActivityManager am = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
        List<RunningTaskInfo> runningTask = am.getRunningTasks(1);
        ...
        // FIXME package of TV Player
        if ("com.eostek.tv.player".equals(taskInfo.topActivity.getPackageName())) {
            return true;
        }
