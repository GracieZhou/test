...
public class PhoneStatusBarPolicy {
    ...
    private final void updateVolumeZen() {
        ...
        if (volumeVisible) {
            // EosTek Patch Begin
            mService.setIcon(SLOT_VOLUME, R.drawable.status_sys_ringer_silent, 0, volumeDescription);
            // EosTek Patch Begin
        }
    ...
    private final void updateBluetooth() {
        ...
        // EosTek Patch Begin
        int iconId = R.drawable.status_sys_bluetooth_enabled;
        // EosTek Patch End
        ...
        if (adapter != null) {
            ...
            if (adapter.getConnectionState() == BluetoothAdapter.STATE_CONNECTED) {
                // EosTek Patch Begin
                iconId = R.drawable.status_sys_bluetooth_connected;
                // EosTek Patch End
    ...
    private final void updateVolume(Intent intent) {
        final boolean mute = intent.getBooleanExtra(AudioManager.EXTRA_MASTER_VOLUME_MUTED, false);
        final int iconId;
        String contentDescription = null;
        if (mute) {
            iconId = R.drawable.status_sys_ringer_silent;
            contentDescription = mContext.getString(R.string.accessibility_ringer_silent);
        }

        if (mute) {
            mService.setIcon("volume", R.drawable.status_sys_ringer_silent, 0, contentDescription);
        }
        if (mute != mVolumeVisible) {
            mService.setIconVisibility("volume", mute);
            mVolumeVisible = mute;
        }
    }
    // EosTek Patch End
