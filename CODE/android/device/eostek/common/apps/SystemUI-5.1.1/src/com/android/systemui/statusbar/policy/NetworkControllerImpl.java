...
public class NetworkControllerImpl extends BroadcastReceiver
    ...
    private int mPPPOEIconId = 0;
    private int mLastPPPOEIconId;
    // EosTek Patch End
 // MStar Android Patch Begin
    private final static String PPPOE_STATE_ACTION = "com.mstar.android.pppoe.PPPOE_STATE_ACTION";
    private final static String PPPOE_STATE_STATUE = "PppoeStatus";
    private static final String PPPOE_STATE_CONNECT = "connect";
    private static final String PPPOE_STATE_DISCONNECT = "disconnect";
    private static final String PPPOE_STATE_AUTHFAILED = "authfailed";
    private static final String PPPOE_STATE_FAILED = "failed";
    ...
    NetworkControllerImpl(Context context, ConnectivityManager connectivityManager,
        ...
        // EosTek Patch Begin
        mEthernetSignalController = new EthernetSignalController(mContext, mHasMobileDataFeature,
                mSignalsChangedCallbacks, mSignalClusters, this);
        // EosTek Patch End
    ...
    private void registerListeners() {
        ...
        filter.addAction(PPPOE_STATE_ACTION);
    ...
    public void onReceive(Context context, Intent intent) {
       ...
       if (action.equals(PPPOE_STATE_ACTION)) {
            ...
            Log.d(TAG, "networkInfo : " + networkInfo != null ? networkInfo.toString() : null);
            if (networkInfo != null && networkInfo.getType() == ConnectivityManager.TYPE_ETHERNET) {
               if (!mPPPOEConnected){
                   mEthernetSignalController.handleBroadcast(intent);
               }
            }
            // EosTek Patch End
    ...
    private void pushConnectivityToSignals() {
        ...
        // EosTek Patch Begin
        mEthernetSignalController.setInetCondition(mEthernetConnected ? 1 : 0);
        // EosTek Patch End
    }
    ...
    static class WifiSignalController extends
        ...
        private Notification mWifiNotification;
        // EosTek Patch End
        ...
        public void notifyListeners() {
            ...
            for (int i = 0; i < signalClustersLength; i++) {
                // EosTek Patch Begin
                mSignalClusters.get(i).setWifiIndicators(mCurrentState.connected, getCurrentIconId(),
                        contentDescription, mCurrentState.ssid, mCurrentState.rssi);
                // EosTek Patch End
            }
        }
        ...
        public void handleBroadcast(Intent intent) {
            ...
            // EosTek Patch Begin
            if (mLastState.connected ^ mCurrentState.connected) {
                setWifiNotification(true, mCurrentState.connected);
            }
            // EosTek Patch End
            notifyListenersIfNecessary();
        }
        // EosTek Patch Begin
        private void setWifiNotification(boolean visible, boolean connected) {
            final int notificationId = R.drawable.status_sys_wifi_signal_4;
            // Null check
            if (mNotificationManager == null) {
                return;
            }
            // cancel last icon
            if (mWifiNotification != null && visible) {
                mNotificationManager.cancel(notificationId);
            }

            // notify new notification
            if (visible) {
                if (mWifiNotification == null) {
                    mWifiNotification = new Notification();
                    mWifiNotification.when = 0;
                }
                mWifiNotification.defaults = Notification.DEFAULT_SOUND;
                mWifiNotification.flags = Notification.FLAG_AUTO_CANCEL;

                Resources r = mContext.getResources();
                CharSequence title;
                int icon;
                if (connected) {
                    title = r.getText(R.string.wifi_network_connected);
                    icon = R.drawable.status_sys_wifi_signal_4;
                } else {
                    title = r.getText(R.string.wifi_network_disconnect);
                    icon = R.drawable.status_sys_wifi_signal_0;
                }
                mWifiNotification.tickerText = title;
                mWifiNotification.icon = icon;

                Intent intent = new Intent();
                PendingIntent pi = PendingIntent.getBroadcastAsUser(mContext, 0, intent, 0, UserHandle.CURRENT);
                mWifiNotification.setLatestEventInfo(mContext, title, title, pi);
            }
            ...
            if (visible) {
                mNotificationManager.notify(null, notificationId, mWifiNotification);
            } else {
                mNotificationManager.cancel(notificationId);
            }
        }
    ...
    static abstract class SignalController<T extends SignalController.State,
        ...
        public void updateConnectivity(boolean online) {
            mCurrentState.mConnectivity = online;
            notifyListenersIfNecessary();
        }
        // EosTek Patch End
        ...
        public int getCurrentIconId() {
            ...
            switch (mTransportType) {
                case NetworkCapabilities.TRANSPORT_WIFI:
                    if (mCurrentState.connected) {
                        if (mCurrentState.mConnectivity) {
                            return getIcons().mSbIcons[0][mCurrentState.level];
                        } else {
                ...
                case NetworkCapabilities.TRANSPORT_ETHERNET:
                    int iconId = R.drawable.status_sys_ethernet_no_network;
                    if (mCurrentState.mConnectivity) {
                        iconId = R.drawable.status_sys_ethernet_connected;
                    } else {
                        iconId = R.drawable.status_sys_ethernet_no_network;
                    }
                    return iconId;
                default:
                    break;
        ...
        static class State {
            ...
            public void copyFrom(State state) {
                ...
                time = state.time;
                // EosTek Patch Begin
                mConnectivity = state.mConnectivity;
                // EosTek Patch End
            }
            ...
            public boolean equals(Object o) {
                ...
                return other.connected == connected
                        ...
                        && other.rssi == rssi
                        // EosTek Patch Begin
                        && other.mConnectivity == mConnectivity
                        // EosTek Patch End
                        ;
            }
        }
    ...
    public interface SignalCluster {
        ...
        // EosTek Patch Begin
        void setWifiIndicators(boolean visible, int strengthIcon, String contentDescription, String ssid, int strength);
        void setEthernetIndicators(boolean visible, int icon);
        // EosTek Patch End
     // EosTek Patch Begin
    ...
    public void updateNetworkStatusIcons(boolean online) {
        Log.d(TAG, "IM " + (online ? "on" : "off") + "line");
        mWifiSignalController.updateConnectivity(online);
        mEthernetSignalController.updateConnectivity(online);
    }

    static class EthernetSignalController extends
            SignalController<EthernetSignalController.EthernetState, SignalController.IconGroup> {
        private NotificationManager mNotificationManager;

        private Notification mEthernetNotification;

        public EthernetSignalController(Context context, boolean hasMobileData,
                List<NetworkSignalChangedCallback> signalCallbacks, List<SignalCluster> signalClusters,
                NetworkControllerImpl networkController) {
            super("EthernetSignalController", context, NetworkCapabilities.TRANSPORT_ETHERNET, signalCallbacks,
        ...
        @Override
        public void notifyListeners() {
            // only show ethernet in the cluster if connected or if
            // ethernet-only
            int signalClustersLength = mSignalClusters.size();
            for (int i = 0; i < signalClustersLength; i++) {
                mSignalClusters.get(i).setEthernetIndicators(mCurrentState.connected, getCurrentIconId());
            }
        }

        @Override
        protected EthernetState cleanState() {
            return new EthernetState();
        }

        /**
         * Extract Ethernet state directly from broadcasts about changes in
         * Ethernet state.
         */
        public void handleBroadcast(Intent intent) {
            final String action = intent.getAction();
            if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION_IMMEDIATE)) {
                final NetworkInfo networkInfo = (NetworkInfo) intent
                        .getParcelableExtra(ConnectivityManager.EXTRA_NETWORK_INFO);
                if (networkInfo != null) {
                    mCurrentState.enabled = networkInfo.isAvailable();
                    mCurrentState.connected = networkInfo.isConnected();
                }
            }
            if (DEBUG) {
                Log.d(TAG, "action : " + action + " mCurrentState.enabled : " + mCurrentState.enabled
                        + " mCurrentState.connected : " + mCurrentState.connected);
            }

            // send notification
            boolean visible = mCurrentState.enabled && mCurrentState.connected;
            if (mLastState.connected ^ mCurrentState.connected) {
                setEthernetNotification(true, mCurrentState.connected);
            }
            // notify all listeners
            notifyListenersIfNecessary();
        }

        private void setEthernetNotification(boolean visible, boolean connected) {
            final int notificationId = R.drawable.status_sys_ethernet_connected;
            // Null check
            if (mNotificationManager == null) {
                return;
            }
            // cancel last icon
            if (mEthernetNotification != null && visible) {
                mNotificationManager.cancel(notificationId);
            }

            // notify new notification
            if (visible) {
                if (mEthernetNotification == null) {
                    mEthernetNotification = new Notification();
                    mEthernetNotification.when = 0;
                }
                mEthernetNotification.defaults = Notification.DEFAULT_SOUND;
                mEthernetNotification.flags = Notification.FLAG_AUTO_CANCEL;

                Resources r = mContext.getResources();
                CharSequence title;
                int icon;
                if (connected) {
                    title = r.getText(R.string.ethernet_network_connected);
                    icon = R.drawable.status_sys_ethernet_connected;
                } else {
                    title = r.getText(R.string.ethernet_network_disconnect);
                    icon = R.drawable.status_sys_ethernet_no_network;
                }
                mEthernetNotification.tickerText = title;
                mEthernetNotification.icon = icon;

                Intent intent = new Intent();
                PendingIntent pi = PendingIntent.getBroadcastAsUser(mContext, 0, intent, 0, UserHandle.CURRENT);
                mEthernetNotification.setLatestEventInfo(mContext, title, title, pi);
            }
            ...
            if (visible) {
                mNotificationManager.notify(null, notificationId, mEthernetNotification);
            } else {
                mNotificationManager.cancel(notificationId);
            }
        }

        static class EthernetState extends SignalController.State {
    ...
    // EosTek Patch End
   // EosTek Patch Begin
    private void updatePPPoEState(Intent intent) {
        final String action = intent.getAction();
        if (action.equals(PPPOE_STATE_ACTION)) {
            int iconId = R.drawable.status_sys_ethernet_connected;
            String pppoeState = intent.getStringExtra(PPPOE_STATE_STATUE);
            if (pppoeState.equals(PPPOE_STATE_CONNECT)) {
                mPPPOEConnected = true;
                iconId = R.drawable.status_sys_ethernet_connected;
                mEthernetSignalController.setEthernetNotification(true, mPPPOEConnected);
            } else if (pppoeState.equals(PPPOE_STATE_DISCONNECT)
                    || pppoeState.equals(PPPOE_STATE_AUTHFAILED)
                    || pppoeState.equals(PPPOE_STATE_FAILED)) {
            ...
            for (SignalCluster cluster : mSignalClusters) {
                cluster.setEthernetIndicators(mPPPOEConnected, iconId);
                cluster.setPppoeIndicators(mPPPOEConnected,mPPPOEIconId);
            }
        }
    }
    private void updatePppoeIcon() {
        if (mPPPOEConnected) {
            mPPPOEIconId = R.drawable.stat_sys_pppoe_on;
        } else {
            mPPPOEIconId = 0;
        }
    }
