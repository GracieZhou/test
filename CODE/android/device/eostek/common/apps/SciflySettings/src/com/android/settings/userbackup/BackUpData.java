...
public class BackUpData extends BroadcastReceiver {
    ...
    public static boolean updateData(String strType, String strName, String value) {
        ...
        /**
         * TODO : 当document为null时，是因为内存不足引起的，内存为850M时空间不足，
         * 无法再继续备份，备份文件中的数据被清除了，加载的是空文件，当需要备份数据时就报NullPointerException，
         * 导致设置停止运行。为了防止设置停止运行，当document为null时，直接返回，
         * 相应的用户数据也将无法继续备份，此处需要以后再做进一步处理。
         */
        if (document == null)
            return false;
        docPrint(document);
    ...
    public static synchronized boolean createXML() {
        ...
        if (isDirExist) {
            try {
                ...
                Comment cmt2 = document.createComment("the inputmethod Info.");
                root.appendChild(cmt2);

                Element inputMethod = document.createElement("inputmethod");
                root.appendChild(inputMethod);
                inputMethod.setAttribute("name", "InputMethodConfig");

                Element inputMethodId = document.createElement("inputmethod_id");
                inputMethodId.appendChild(document.createTextNode("none"));
                inputMethod.appendChild(inputMethodId);

                Comment cmtDeviceName = document.createComment("the Device Info.");
                root.appendChild(cmtDeviceName);
