...
final class Utils {
    ...
    // Create (or recycle existing) and show unPair dialog.
    static AlertDialog showUnPairDialog(Context context,
            AlertDialog dialog,
            DialogInterface.OnClickListener unPairListener,
            CharSequence title, CharSequence message) {
        if (dialog == null) {
            dialog = new AlertDialog.Builder(context)
                    .setPositiveButton(android.R.string.ok, unPairListener)
                    .setNegativeButton(android.R.string.cancel, null)
                    .create();
        } else {
            if (dialog.isShowing()) {
