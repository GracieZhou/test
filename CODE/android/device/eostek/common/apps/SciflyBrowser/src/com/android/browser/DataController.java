...
import com.eostek.scifly.browser.settool.SetToolLogic;
import com.mstar.android.tvapi.dtv.vo.CaServiceEntitle;
...
public class DataController {
    ...
    private static final int HISTORY_QUERY = 204;
    private static final int HISTORY_REMOVE_BY_URL = 205;
    ...
    public static interface OnQueryHistory {
        void onQueryHistory(Cursor cursor);
    }
        ...
        mCbHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case HISTORY_QUERY:
                        OnQueryHistory cb = (OnQueryHistory) cc.replyTo;
                        Cursor cursor = (Cursor) cc.args[0];
                        cb.onQueryHistory(cursor);
                        break;
                }
            }
        };
    ...
    public void queryHistory(OnQueryHistory replyTo) {
        mDataHandler.sendMessage(HISTORY_QUERY, replyTo);
    }

    public void deleteHistoryByUrl (String url) {
        mDataHandler.sendMessage(HISTORY_REMOVE_BY_URL, url);
    }
    ...
    class DataControllerHandler extends Thread {
        ...
        private void handleMessage(DCMessage msg) {
            ...
            case TAB_SAVE_THUMBNAIL:
                ...
                break;
            case HISTORY_QUERY:
                doQueryHistory(msg.obj);
                break;

            case HISTORY_REMOVE_BY_URL:
                deleteHistoryByUrl(mContext.getContentResolver(), (String)msg.obj);
                break;
            }
        }
        ...
        private void doQueryHistory(Object replyTo) {
            Cursor cursor = mContext.getContentResolver().query(Combined.CONTENT_URI,
                    SetToolLogic.HistoryQuery.PROJECTION, Combined.VISITS + " > 0", null,
                    Combined.DATE_LAST_VISITED + " DESC");
            CallbackContainer cc = new CallbackContainer();
            cc.replyTo = replyTo;
            cc.args = new Object[] {cursor};
            mCbHandler.obtainMessage(HISTORY_QUERY, cc).sendToTarget();
        }

        private void deleteHistoryByUrl(ContentResolver cr, String url) {
            final WebIconDatabase iconDb = WebIconDatabase.getInstance();
            iconDb.releaseIconForPageUrl(url);

            cr.delete(History.CONTENT_URI, History.URL + "='" + url + "'", null);
        }
    }
}
