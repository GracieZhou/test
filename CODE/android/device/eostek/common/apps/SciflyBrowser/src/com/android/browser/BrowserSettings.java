...
public class BrowserSettings implements OnSharedPreferenceChangeListener,
    ...
    private static final String GINGERBREAD_USERAGENT = "Mozilla/5.0 (Linux; U; " +
        "Android 2.3.6; en-us; Nexus S Build/GRK39F) AppleWebKit/533.1 " +
        "(KHTML, like Gecko) Version/4.0 Mobile Safari/533.1";

    private static final String ICS_USERAGENT = "Mozilla/5.0 (Linux; U; " +
    ...
    public static final boolean MEM_OPTIMIZED=false;
...
//        mContext = context.getApplicationContext();
    ...
    private void syncSetting(WebSettings settings) {
        ...
        // android 5.1 VERSION.SDK_INT==22
        if (VERSION.SDK_INT != 22) {
            settings.setInLineVideoEnabled(enableInlineVideo());
        }
    ...
    private void initUseragentmap() {
        ...
        useragentMap.put("Nexus_S", GINGERBREAD_USERAGENT);
        useragentMap.put("Galaxy_Nexus", ICS_USERAGENT);
...
//            }
//        }
        WebView current = WebViewHelper.getInstance((BrowserActivity)mContext).getCurrentWebView();
        if (current != null) {
            current.clearCache(true);
        }
    }
...
//        return mPrefs.getString(PREF_SEARCH_ENGINE, SearchEngine.GOOGLE);
    ...
    public String getUserAgentName() {
        return mPrefs.getString(PREF_USERAGENT_STATE, "ipad");
    }
    // MStar Android Patch End
...
// STA extended end.
    ...
    public boolean enableInlineVideo() {
        return mPrefs.getBoolean(PREF_INLINE_VIDEO, false);
    }
    ...
    public boolean isClearHistory() {
        return mPrefs.getBoolean(PREF_CLEAR_HISTORY, false);
    }

    public void setClearHistory(boolean value) {
        mPrefs.edit().putBoolean(PREF_CLEAR_HISTORY, value).apply();
    }

    public boolean isClearCache() {
        return mPrefs.getBoolean(PREF_CLEAR_CACHE, true);
    }

    public void setClearCache(boolean value) {
        mPrefs.edit().putBoolean(PREF_CLEAR_CACHE, value).apply();
    }

    public boolean isClearFormData() {
        return mPrefs.getBoolean(PREF_CLEAR_FORM_DATA, true);
    }

    public void setClearFormData(boolean value) {
        mPrefs.edit().putBoolean(PREF_CLEAR_FORM_DATA, value).apply();
    }

    public boolean isClearPassword() {
        return mPrefs.getBoolean(PREF_CLEAR_PASSWORD, true);
    }

    public void setClearPassword(boolean value) {
        mPrefs.edit().putBoolean(PREF_CLEAR_PASSWORD, value).apply();
    }

    public boolean isClearCookie() {
        return mPrefs.getBoolean(PREF_CLEAR_COOKIE, true);
    }

    public void setClearCookie(boolean value) {
        mPrefs.edit().putBoolean(PREF_CLEAR_COOKIE, value).apply();
    }

    public boolean isCancleLocatPermission() {
        return mPrefs.getBoolean(PREF_CANCLE_LOCAT_PERMISSION, true);
    }

    public void setCancleLocatPermission(boolean value) {
        mPrefs.edit().putBoolean(PREF_CANCLE_LOCAT_PERMISSION, value).apply();
    }

    public void saveSuggestEtag(String eTag, String language) {
        mPrefs.edit().putString(PREF_SUGGEST_ETAG + language, eTag).apply();
    }

    public String getSuggestEtag(String language) {
        return mPrefs.getString(PREF_SUGGEST_ETAG + language, "");
    }
}
