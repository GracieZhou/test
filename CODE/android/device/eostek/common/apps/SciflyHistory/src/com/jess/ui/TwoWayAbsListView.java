...
public abstract class TwoWayAbsListView extends TwoWayAdapterView<ListAdapter> implements
     ...
     * mFastScrollEnabled; } protected boolean isVerticalScrollBarHidden() {
     ...
     * issue, you need to disable this property. When smooth scrollbar is
     * disabled, the position and size of the scrollbar thumb is based solely on
     * the number of items in the adapter and the position of the visible items
     * inside the adapter. This provides a stable scrollbar as the user
     ...
     * default, the drawing cache is enabled but this will use more memory. When
     * the scrolling cache is enabled, the caches are kept after the first
    ...
    static class SavedState extends BaseSavedState {
        ...
        public String toString() {
            return "TwoWayAbsListView.SavedState{" + Integer.toHexString(System.identityHashCode(this))
                    + " selectedId=" + selectedId + " firstId=" + firstId + " viewTop=" + viewTop + " position="
     ...
     * final boolean changed = super.setFrame(left, top, right, bottom); if
     * (changed) { // Reposition the popup when the frame has changed. This
     ...
     * != null && mPopup.isShowing()) { positionPopup(); } } return changed; }
     */
    ...
    private boolean performLongPress(final View child, final int longPressPosition, final long longPressId) {
        ...
        if (mOnItemLongClickListener != null) {
            handled = mOnItemLongClickListener.onItemLongClick(TwoWayAbsListView.this, child, longPressPosition,
                    longPressId);
        }
        if (!handled) {
        ...
        if (checkLayout) {
            ...
            for (int i = 0; i < count; i++) {
                if (activeViews[i] != null) {
                    result = false;
                    Log.d("Consistency", "AbsListView " + this + " has a view in its active recycler: "
                            + activeViews[i]);
                }
            }
                ...
                case MotionEvent.ACTION_DOWN: {
                    ...
                    if (!mDataChanged) {
                        if ((mTouchMode != TOUCH_MODE_FLING) && (motionPosition >= 0)
                            ...
                            // stopping a fling). It might be a
                ...
                if (mCheckFlywheel == null) {
                    mCheckFlywheel = new Runnable() {
                        public void run() {
                            ...
                            if (Math.abs(yvel) >= mMinimumVelocity && isScrollingInDirection(0, yvel)) {
                ...
                if (mCheckFlywheel == null) {
                    mCheckFlywheel = new Runnable() {
                        public void run() {
                            ...
                            if (Math.abs(xvel) >= mMinimumVelocity && isScrollingInDirection(0, xvel)) {
                                // Keep the fling alive a little longer
                                postDelayed(this, FLYWHEEL_TIMEOUT);
                            } else {
                                endFling();
