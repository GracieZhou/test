...
public class LeftNavView extends LinearLayout {
    ...
    public void addFocusables(ArrayList<View> views, int direction, int focusableMode) {
        if (direction == FOCUS_FORWARD) {
            // When setting the initial focus for the window, all views should
            // be considered
            // focusable.
            super.addFocusables(views, direction, focusableMode);
            if (!hasFocus()) {
                onDescendantFocusChanged(false);
            }
            return;
        }
        if (direction != FOCUS_LEFT && !hasFocus()) {
            // We don't want to gain focus unless it's coming from the right or
            // we already have
            // focus.
            return;
        }
    ...
    /*
     * @Override protected void onDescendantFocusChanged(boolean hasFocus) {
     * super.onDescendantFocusChanged(hasFocus); if (has(mDisplayOptions,
     * ActionBar.DISPLAY_AUTO_EXPAND)) { setExpanded(hasFocus); } }
     */
    ...
    private void setExpanded(final boolean expanded, boolean animated) {
        ...
        if (animated) {
            ...
            mWidthAnimator = ValueAnimator.ofInt(getLayoutParams().width, // Starting
                                                                          // value.
    ...
    /**
     * Returns the "steady state" width for the view, taking into account all
     * shadowing effects.
     */
    ...
    public void requestChildFocus(View child, View focused) {
        if (!hasFocus()) {
            onDescendantFocusChanged(true);
        }
        super.requestChildFocus(child, focused);
    }

    /**
     * Wrapper around custom views to allow them to use the layout parameters
     * defined in the ActionBar class and still be displayed within this view
     * group.
     */
    private static final class CustomViewWrapper extends ViewGroup {
        ...
        /**
         * Locates the top bound of the space available for custom views,
         * expressed in the coordinate system of {@code parent}.
         */
        ...
        /**
         * Locates the bottom bound of the space available for custom views,
         * expressed in the coordinate system of {@code parent}.
         */
        ...
        /**
         * Should be called after the rest of the left nav has been measured, so
         * that custom views can be sized according to the space left and the
         * desired gravity.
         */
        void onPostMeasure(LeftNavView parent) {
            ...
            // This is used when attempting to vertically center a custom view
            // which has its height
            // as "match parent": in this case its size will be limited by the
            // smallest of the two
            // spaces.
        ...
        /**
         * Should be called as the last layout step to properly position the
         * custom view.
         */
        void onPostLayout(LeftNavView parent) {
            ...
            int xPosition = 0;
            // Horizontal alignment is always performed within the available
            // space.
            ...
            int vGravity = params.gravity & Gravity.VERTICAL_GRAVITY_MASK;
            // For "center vertical" the view gets centered within the parent,
            // not within the
            // available space.
            /*
             * int superContainerHeight = parent.mBottom - parent.mTop -
             * parent.mPaddingTop - parent.mPaddingBottom;
             */
