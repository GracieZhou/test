...
public abstract class TwoWayAdapterView<T extends Adapter> extends ViewGroup {
    ...
    class AdapterDataSetObserver extends DataSetObserver {
        ...
        public void onChanged() {
            ...
            if (TwoWayAdapterView.this.getAdapter().hasStableIds() && mInstanceState != null && mOldItemCount == 0
                    && mItemCount > 0) {
     ...
     * event.setEventType(AccessibilityEvent.TYPE_VIEW_SELECTED); } // we send
     * selection events only from AdapterView to avoid // generation of such
     * event for each child View selectedView = getSelectedView(); if
     * (selectedView != null) { populated =
     * selectedView.dispatchPopulateAccessibilityEvent(event); } if (!populated)
     * { if (selectedView != null) { event.setEnabled(selectedView.isEnabled());
     * } event.setItemCount(getCount());
     * event.setCurrentItemIndex(getSelectedItemPosition()); } return populated;
     * }
     */
