...
public class ProximityInfo {
    ...
    class Key {
        ...
        public int squaredDistanceToEdge(int centerX, int centerY) {
            return 0;
        }
    ...
    ProximityInfo(final String localeStr, final int gridWidth, final int gridHeight, final int minWidth,
            final int height, final int mostCommonKeyWidth, final int mostCommonKeyHeight, final Key[] keys,
    ...
    public ProximityInfo() {
        ...
        mCellWidth = 60;
        mCellHeight = 31;
        mKeyboardMinWidth = 1920;
        mKeyboardHeight = 496;
        mMostCommonKeyHeight = 126;
        mMostCommonKeyWidth = 192;
        mKeys = new Key[34];
        ...
        mGridNeighbors = new Key[mGridSize][];
        computeNearestNeighbors();
        mNativeProximityInfo = createNativeProximityInfo(new TouchPositionCorrection());
    }
    ...
    private static native long setProximityInfoNative(String locale, int displayWidth, int displayHeight,
            int gridWidth, int gridHeight, int mostCommonKeyWidth, int mostCommonKeyHeight, int[] proximityCharsArray,
            int keyCount, int[] keyXCoordinates, int[] keyYCoordinates, int[] keyWidths, int[] keyHeights,
            int[] keyCharCodes, float[] sweetSpotCenterXs, float[] sweetSpotCenterYs, float[] sweetSpotRadii);
    ...
    private long createNativeProximityInfo(final TouchPositionCorrection touchPositionCorrection) {
        ...
        if (touchPositionCorrection != null && touchPositionCorrection.isValid()) {
            ...
            for (int infoIndex = 0, keyIndex = 0; keyIndex < keys.length; keyIndex++) {
                ...
                if (DEBUG) {
                    Log.d(TAG, String.format("  [%2d] row=%d x/y/r=%7.2f/%7.2f/%5.2f %s code=%s", infoIndex, row,
                            sweetSpotCenterXs[infoIndex], sweetSpotCenterYs[infoIndex], sweetSpotRadii[infoIndex],
                            (row < rows ? "correct" : "default"), Constants.printableCode(key.getCode())));
                }
                infoIndex++;
            }
        } else {
        ...
        // TODO: Stop passing proximityCharsArray
        return setProximityInfoNative(mLocaleStr, mKeyboardMinWidth, mKeyboardHeight, mGridWidth, mGridHeight,
                mMostCommonKeyWidth, mMostCommonKeyHeight, proximityCharsArray, keyCount, keyXCoordinates,
                keyYCoordinates, keyWidths, keyHeights, keyCharCodes, sweetSpotCenterXs, sweetSpotCenterYs,
                sweetSpotRadii);
    }
    ...
    private void computeNearestNeighbors() {
        ...
        // For large layouts, 'neighborsFlatBuffer' is about 80k of memory:
        // gridSize is usually 512,
        // keycount is about 40 and a pointer to a Key is 4 bytes. This
        // contains, for each cell,
        // enough space for as many keys as there are on the keyboard. Hence,
        // every
        // keycount'th element is the start of a new cell, and each of these
        // virtual subarrays
        // start empty with keycount spaces available. This fills up gradually
        // in the loop below.
        // Since in the practice each cell does not have a lot of neighbors,
        ...
        for (final Key key : mKeys) {
            ...
            /*
             * HOW WE PRE-SELECT THE CELLS (iterate over only the relevant
             * cells, instead of all of them) We want to compute the distance
             * for keys that are in the cells that are close enough to the key
             * border, as this method is performance-critical. These keys are
             * represented with 'star' background on the diagram below. Let's
             * consider the Y case first. We want to select the cells which
             * center falls between the top of the key minus the threshold, and
             * the bottom of the key plus the threshold. topPixelWithinThreshold
             * is key.mY - threshold, and bottomPixelWithinThreshold is key.mY +
             * key.mHeight + threshold. Then we need to compute the center of
             * the top row that we need to evaluate, as we'll iterate from
             * there. (0,0)----> x |
             * .-------------------------------------------. | | | | | | | | | |
             * | | | | |---+---+---+---+---+---+---+---+---+---+---| .- top of
             * top cell (aligned on the grid) | | | | | | | | | | | | | | |
             * |-----------+---+---+---+---+---+---+---+---|---' v | | | |
             * |***|***|*_________________________ topPixelWithinThreshold |
             * yDeltaToGrid | |---+---+---+-----^-+-|-+---+---+---+---+---| ^ |
             * | | | |***|*|*|*|*|***|***| | | |
             ...
             * y |---+---+---+---+-v-+-|-+---+---+---+---+---| | thresholdBase
             * and get the top pixel | | | |***|**########-------------------
             * key.mY | within the threshold. We align that on
             * |---+---+---+---+--#+---+-#-+---+---+---+---| | the grid by
             * computing the delta to the | | | |***|**#|***|*#*|***| | | | |
             * grid, and get the top of the top cell.
             * |---+---+---+---+--#+---+-#-+---+---+---+---| | | | |
             * |***|**########*|***| | | | | Adding half the cell height to the
             * top |---+---+---+---+---+-|-+---+---+---+---+---| | of the top
             * cell, we get the middle of | | | |***|***|*|*|***|***| | | | |
             * the top cell (yMiddleOfTopCell).
             * |---+---+---+---+---+-|-+---+---+---+---+---| | | | |
             * |***|***|*|*|***|***| | | | |
             * |---+---+---+---+---+-|________________________ yEnd | Since we
             * only want to add the key to | | | | | | |
             * (bottomPixelWithinThreshold) | the proximity if it's close enough
             * to |---+---+---+---+---+---+---+---+---+---+---| | the center of
             * the cell, we only need | | | | | | | | | | | | | to compute for
             * these cells where '---'---'---'---'---'---'---'---'---'---'---' |
             * topPixelWithinThreshold is above the (positive x,y) | center of
             * the cell. This is the case | when yDeltaToGrid is less than half
             * [Zoomed in diagram] | the height of the cell.
             * +-------+-------+-------+-------+-------+ | | | | | | | | On the
             * zoomed in diagram, on the right | | | | | | | the
             * topPixelWithinThreshold (represented | | | | | | top of | with an
             * = sign) is below and we can skip
             * +-------+-------+-------+--v----+-------+ .. top cell | this
             * cell, while on the left it's above | | = topPixelWT | |
             * yDeltaToGrid | and we need to compute for this cell.
             * |..yStart.|.....|.......|..|....|.......|... y middle | Thus, if
             * yDeltaToGrid is more than half | (left)| | | ^ = | | of top cell
             * | the height of the cell, we start the
             * +-------+-|-----+-------+----|--+-------+ | iteration one cell
             * below the top cell, | | | | | | | | | else we start it on the top
             * cell. This |.......|.|.....|.......|....|..|.....yStart (right) |
             * is stored in yStart. Since we only want to go up to
             * bottomPixelWithinThreshold, and we only iterate on the center of
             * the keys, we can stop as soon as the y value exceeds
             * bottomPixelThreshold, so we don't have to align this on the
             * center of the key. Hence, we don't need a separate value for
            ...
            final int yStart = Math.max(halfCellHeight, yMiddleOfTopCell
                    + (yDeltaToGrid <= halfCellHeight ? 0 : mCellHeight));
            ...
            final int xStart = Math.max(halfCellWidth, xMiddleOfLeftCell
                    + (xDeltaToGrid <= halfCellWidth ? 0 : mCellWidth));
    ...
    public void fillArrayWithNearestKeyCodes(final int x, final int y, final int primaryKeyCode, final int[] dest) {
