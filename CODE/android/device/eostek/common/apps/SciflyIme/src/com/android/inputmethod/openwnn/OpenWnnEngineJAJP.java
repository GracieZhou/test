...
public class OpenWnnEngineJAJP implements WnnEngine {
    ...
    public OpenWnnEngineJAJP(String writableDictionaryName) {
        ...
        mDictionaryJP = new OpenWnnDictionaryImpl(
                "/system/lib/libWnnJpnDic_scifly.so",
     ...
     */
    private int setSearchKey(String text, int maxLen) {
        String input = text;
...
//        mInputRomaji = text.toString(ComposingText.LAYER0);
    ...
    private int setSearchKey(ComposingText text, int maxLen) {
        ...
        Log.d("raymond", "setSearchKey input---------"+input);
        if (0 <= maxLen && maxLen <= input.length()) {
            input = input.substring(0, maxLen);
            mExactMatchMode = true;
        } else {
        ...
        mInputHiragana = input;
        Log.d("raymond", "mInputHiragana---------"+mInputHiragana);
        mInputRomaji = text.toString(ComposingText.LAYER0);
        Log.d("raymond", "mInputRomaji---------"+mInputRomaji);
        Log.d("raymond", "setSearchKey input.length---------"+input.length());
        return input.length();
    }
    ...
    public int predict(String text, int minLen, int maxLen) {
        clearCandidates();
    ...
    public WnnWord getNextCandidate() {
        ...
        Log.d("raymond", "mOutputNum--------"+mOutputNum);
        return word;
    }
