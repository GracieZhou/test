 ...
 * {@link VSession}s.
 */
public abstract class AbstractSession implements VSession {
    ...
    public void sign(HttpRequest request) {
        ...
        if (useWeiboToken) {
            ...
            Log.d("weibo_access_Token", "Weibo " + Signature.getWeiboHeader(this.appKeyPair, this.weiboAccessToken));
        } else {
            // 使用微盘Token进行认证
    ...
    /**
     * 使用微博的Token进行认证 Use Weibo token for authentication
     */
    ...
    /**
     * 取消使用微博的Token进行认证 Use VDisk token for authentication
     */
    ...
    /**
     * 使用微博的Token进行认证,并设置微博Token Use Weibo token for authentication, and set
     * Weibo token
     */
    ...
    /**
     * 此方法可以用来设置或者更新微博的Token This method is used to set or update Weibo token
     * 
     * @param weiboToken
     */
