...
/**
 * 登录认证时弹出的登录对话框 Pop a dialog when there is a authenticating.
 * 
 * @author sina
 */
public class VDiskDialog extends Dialog {
            ...
            switch (msg.what) {
                case SUCCESS:
                    ...
                    if (error == null && error_code == null) {
                        ...
                        // 用户或授权服务器拒绝授予数据访问权限
                        // User or authorization server refuses to grant data
                        // access
                        // permission
                        mListener.onCancel();
                    } else {
                        mListener.onVDiskException(new VDiskServerException(error, Integer.parseInt(error_code)));
                    }
                    VDiskDialog.this.dismiss();
                    break;
                case FAILED:
                    VDiskException e = (VDiskException) values.getSerializable("error");
    ...
    public VDiskDialog(final VDiskAuthSession session, Context context, String url, VDiskDialogListener listener) {
    ...
    private class WeiboWebViewClient extends WebViewClient {
        ...
        public void onReceivedError(WebView view, int errorCode, String description, String failingUrl) {
            ...
            mListener.onError(new VDiskDialogError(description, errorCode, failingUrl));
            VDiskDialog.this.dismiss();
        }
        ...
        if (code != null) {
            new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        mOAuth2AccessToken = mSession.getOAuth2AccessToken(code, getContext());
                    ...
                    values.putSerializable(VDiskAuthSession.OAUTH2_TOKEN, mOAuth2AccessToken);
                    msg.setData(values);
                    msg.what = SUCCESS;
                    handler.sendMessage(msg);
                };
