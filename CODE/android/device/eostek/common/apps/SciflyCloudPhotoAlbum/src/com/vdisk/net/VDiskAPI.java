...
/**
 * Location of the VDisk API functions. The class is parameterized with the type
 * of session it uses. This will be the same as the type of session you pass
 * into the constructor.
 */
public class VDiskAPI<SESS_T extends VSession> {
    ...
    public static class VDiskUploadFileInfo implements Serializable {
        ...
        protected VDiskUploadFileInfo(Map<String, Object> map, int segmentNum, String s3Host, long segmentLength,
                String sha1, String srcPath, String desPath) throws VDiskException {
            this.s3Host = s3Host;
            ...
            if (uploadKey == null || uploadId == null) {
                ...
                try {
                    for (int i = 1; i <= segmentNum; i++) {
                        @SuppressWarnings("unchecked")
                        Map<String, Object> section = (Map<String, Object>) sectionMap.get(String.valueOf(i));
                        String number = getFromMapAsString(section, "part_number");
                        String uri = getFromMapAsString(section, "uri");
                        partSigns.put(Integer.parseInt(number), uri);
                    }

                    expireTime = System.currentTimeMillis() + 2 * 24 * 60 * 60 * 1000;
    ...
    public static final class VDiskFileInfo implements Serializable {
        ...
        private VDiskFileInfo(HttpResponse response, Entry metadata, String location) throws VDiskException {
        ...
        public static Entry parseXVDiskMetadata(HttpResponse response) {
            ...
            Header xVDiskMetadataHeader = response.getFirstHeader("X-VDisk-Metadata");
    ...
    public static class VDiskInputStream extends FilterInputStream {
        ...
        public VDiskInputStream(HttpUriRequest request, HttpResponse response, Entry metadata, String location)
         ...
         * @param listener an optional {@link ProgressListener} to receive
         *            progress updates as the stream is copied, or null.
         * @throws VDiskPartialFileException if only part of the input stream was
         *             copied.
         * @throws VDiskIOException for network-related errors.
         * @throws VDiskLocalStorageFullException if there is no more room to
         *             write to the output stream.
         * @throws VDiskException for any other unknown errors. This is also a
         *             superclass of all other VDisk exceptions, so you may want
         *             to only catch this exception which signals that some kind
         *             of error occurred.
         */
        public void copyStreamToOutput(OutputStream os, File targetFile, ProgressListener listener, boolean isThumnail)
                throws VDiskIOException, VDiskPartialFileException, VDiskLocalStorageFullException {
            BufferedOutputStream bos = null;
            long totalRead = getRange(targetFile);
            long lastListened = 0;
            long length = isThumnail ? info.getContentLength() : info.getFileSize();
    ...
    public interface UploadRequest {
         ...
         * @throws VDiskPartialFileException if the request was canceled before
         *             completion.
         * @throws VDiskServerException if the server responds with an error
         *             code. See the constants in {@link VDiskServerException}
         *             for the meaning of each error code. The most common error
         *             codes you can expect from this call are 404 (path to
         *             upload not found), 507 (user over quota), and 400
         *             (unexpected parent rev).
    ...
    protected static final class BasicUploadRequest implements UploadRequest {
        ...
        private final VSession session;

        public BasicUploadRequest(HttpUriRequest request, VSession session) {
            this.request = request;
            this.session = session;
        }
    ...
    protected static final class ComplexUploadRequest implements UploadRequest {
        ...
        public ComplexUploadRequest(HttpUriRequest request, VSession session) {
            this.request = request;
            this.session = session;
        }
    ...
    public static class VDiskLink {
         ...
         * 
         * @param map the parsed parameters returned from VDisk
    ...
    /**
     * Returns the {@link VSession} that this API is using.
     */
     ...
     * @throws VDiskException
     */
    public String doOAuth2Authorization(AppKeyPair appKeyPair, String code, String redirectUrl, Context ctx)
            throws VDiskException {

        String[] params = {
                "client_id", appKeyPair.key, "client_secret", appKeyPair.secret, "grant_type", "authorization_code",
                "code", code, "redirect_uri", redirectUrl
        };
        return RESTUtility.parseResponse(session, VDiskAuthSession.URL_OAUTH2_ACCESS_TOKEN, params, ctx);
    }
     ...
     * @throws VDiskException
     */
    public String doOAuth2Password(String usrname, String password, AppKeyPair appKeyPair, Context ctx)
            throws VDiskException {

        String[] params = {
                "username", usrname, "password", password, "client_id ", appKeyPair.key, "client_secret",
     ...
     * @throws VDiskException
     */
    public String doOAuth2Refresh(AppKeyPair appKeyPair, String refreshToken, Context ctx) throws VDiskException {

        String[] params = {
                "client_id", appKeyPair.key, "client_secret", appKeyPair.secret, "grant_type", "refresh_token",
    ...
    /**
     * Returns the {@link Account} associated with the current {@link VSession}.
     * 
     * @return the current session's {@link Account}.
     * @throws VDiskUnlinkedException if you have not set an access token pair on
     *             the session, or if the user has revoked access.
     * @throws VDiskServerException if the server responds with an error code.
     *             See the constants in {@link VDiskServerException} for the
     *             meaning of each error code.
     * @throws VDiskIOException if any network-related error occurs.
     * @throws VDiskException for any other unknown errors. This is also a
     *             superclass of all other VDisk exceptions, so you may want to
    ...
    public Account accountInfo() throws VDiskException {
        ...
        Map<String, Object> accountInfo = (Map<String, Object>) RESTUtility.request(RequestMethod.GET,
                session.getAPIServer(), "/account/info", VERSION, null, session);
     ...
     * @throws VDiskServerException if the server responds with an error code.
     *             See the constants in {@link VDiskServerException} for the
     *             meaning of each error code. The most common error codes you
     *             can expect from this call are 404 (path not found) and 400
     *             (bad rev).
     * @throws VDiskPartialFileException if a network error occurs during the
     *             download.
     * @throws VDiskIOException for some network-related errors.
     * @throws VDiskException for any other unknown errors. This is also a
     *             superclass of all other VDisk exceptions, so you may want to
     *             only catch this exception which signals that some kind of
     *             error occurred.
     */
    public VDiskFileInfo getFile(String path, String rev, OutputStream os, File targetFile, ProgressListener listener)
            throws VDiskException {
     ...
     * @throws VDiskException
     */
    public VDiskFileInfo getFileLink(String path, String rev) throws VDiskException {
     ...
     * when you're done with it to release all resources. You can also cancel
     * the download by closing the returned {@link VDiskInputStream} at any
     * time.
     * 
     * @param path the VDisk path to the file.
     * @param rev the revision (from the file's metadata) of the file to
    ...
    public VDiskInputStream getFileStream(String path, String rev, File targetFile) throws VDiskException {
        ...
        if (fileInfo != null) {
            ...
            String filename = targetFile.getName().replace(DOWNLOAD_TEMP_FILE_SUFFIX, "");
            ...
            RequestAndResponse rp = RESTUtility.streamRequestAndResponse(RequestMethod.GET, session, location, null,
     ...
     * @throws VDiskException
     */
    public VDiskFileInfo getVDiskFileInfo(String path, String rev) throws VDiskException {
        assertAuthenticated();
        ...
        RequestAndResponse r = RESTUtility.streamRequest(RequestMethod.GET, session.getAPIServer(), url, VERSION, args,
                session);

        int statusCode = r.response.getStatusLine().getStatusCode();
        if (statusCode == VDiskServerException._302_FOUND) {
            String location = VDiskServerException.getHeader(r.response, "location");
     ...
     * 
     * @param path the full VDisk path where to put the file, including
     ...
     * @param parentRev the rev of the file at which the user started editing it
     ...
     * @throws VDiskUnlinkedException if you have not set an access token pair
     *             on the session, or if the user has revoked access.
     * @throws VDiskFileSizeException if the file is bigger than the maximum
     *             allowed by the API. See {@code VDiskAPI.MAX_UPLOAD_SIZE}.
     * @throws VDiskServerException if the server responds with an error code.
     *             See the constants in {@link VDiskServerException} for the
     *             meaning of each error code. The most common error codes you
    ...
    public Entry putFile(String path, InputStream is, long length, String parentRev, ProgressListener listener)
            throws VDiskException {
        UploadRequest request = putFileRequest(path, is, length, parentRev, listener);
    ...
    public Entry putFileOverwrite(String path, InputStream is, long length, ProgressListener listener)
            throws VDiskException {
        UploadRequest request = putFileOverwriteRequest(path, is, length, listener);
        return request.upload();
    }
     ...
     * 
     * @param srcPath the local path of the file.
     * @param desPath the full VDisk path where to put the file, including
     *            directories and filename.
     * @param length the length of the local file.
     * @param handler the handler to control the upload session.
     * @throws VDiskException
     */
    public void putLargeFileOverwriteRequest(String srcPath, String desPath, long length, ComplexUploadHandler handler)
            throws VDiskException {
     ...
     *            directories and filename.
     * @param length the length of the local file.
     * @param segmentLength the length of a segment after you cutting the local
     *            file, default length is 4Mb.
     ...
     *            files will be overwritten only if the {@code parentRev}
     *            matches the current rev on the server or otherwise a
     *            conflicted copy of the file will be created and you will get
     *            the new file's metadata {@link Entry}.
     * @param parentRev the rev of the file at which the user started editing it
     *            (obtained from a metadata call), or null if this is a new
     *            upload. If null, or if it does not match the latest rev on the
     *            server, a copy of the file will be created and you'll receive
     ...
     * @throws VDiskException
     */
    public void putLargeFileRequest(String srcPath, String desPath, long length, long segmentLength, boolean overwrite,
            String parentRev, ComplexUploadHandler handler) throws VDiskException {
        assertAuthenticated();
        ...
        VDiskUploadFileInfo fileInfo = handler.readUploadFileInfo(srcPath, desPath);

        if (fileInfo == null || fileInfo.expireTime < System.currentTimeMillis()) {
            // 计算sha1 // Compute sha1
            ...
            fileInfo = startComplexUploadMultipartInit(srcPath, desPath, segmentLength, length, sha1,
                    SINA_STORAGE_SERVICE_HOST, overwrite, parentRev);
     ...
     * 
     * @param fileInfo the file info of the uploading file.
     * @param length the length of the local file.
     * @param handler the handler to control the upload session.
     * @param overwrite whether to overwrite the file if it already exists. If
     *            true, any existing file will always be overwritten. If false,
     ...
     * @throws VDiskException
     */
    private Entry startComplexUpload(VDiskUploadFileInfo fileInfo, long length, ComplexUploadHandler handler,
            boolean overwrite, String parentRev) throws VDiskException {
        int startPoint = fileInfo.point;

        Log.d(TAG, "startPoint-->" + startPoint + ";segmentNum-->" + fileInfo.segmentNum);
        ...
        for (int i = startPoint; i < fileInfo.segmentNum; i++) {
            ...
            String url = "http://" + fileInfo.s3Host + fileInfo.partSigns.get(i + 1);
            ...
            try {
                isEntity = new ComplexUploadInputStreamEntity(fileInfo.srcPath, startPos, readLength);
                Log.i(TAG, "startPos-->" + startPos + ";readLength-->" + readLength);
            } catch (IOException e) {
                throw new VDiskIOException(e);
            }
            ...
            ComplexUploadRequest uploadRequest = new ComplexUploadRequest(req, session);
     ...
     * @throws VDiskException
     */
    private Entry startComplexUploadMerge(VDiskUploadFileInfo fileInfo, boolean overwrite, String parentRev)
            throws VDiskException {
        assertAuthenticated();
        ...
        String[] params = new String[] {
                "root", session.getAccessType().toString(), "path", fileInfo.desPath, "s3host", fileInfo.s3Host,
                "upload_id", fileInfo.uploadId, "upload_key", fileInfo.uploadKey, "sha1", fileInfo.sha1, "md5_list",
                fileInfo.md5s, "overwrite", String.valueOf(overwrite), "parent_rev", parentRev
        };

        @SuppressWarnings("unchecked")
        Map<String, Object> entry = (Map<String, Object>) RESTUtility.request(RequestMethod.POST,
                session.getAPIServer(), target, VERSION, params, session, UPLOAD_MERGE_TIMEOUT_MS);
     ...
     * @param segmentLength the length of a segment after you cutting the local
     *            file, default length is 4Mb.
     * @param sha1 the sha1 of the upload file.
     * @param overwrite whether to overwrite the file if it already exists. If
     *            true, any existing file will always be overwritten. If false,
     *            files will be overwritten only if the {@code parentRev}
     *            matches the current rev on the server or otherwise a
     ...
     * @throws VDiskException
     */
    public VDiskUploadFileInfo startComplexUploadMultipartInit(String srcPath, String desPath, long segmentLength,
            long length, String sha1, String s3Host, boolean overwrite, String parentRev) throws VDiskException {
        assertAuthenticated();
        ...
        String[] params = new String[] {
                "root", session.getAccessType().toString(), "path", desPath, "s3host", s3Host, "part_total",
                String.valueOf(segmentNum), "size", String.valueOf(length), "sha1", sha1, "overwrite",
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> setcionInfo = (Map<String, Object>) RESTUtility.request(RequestMethod.POST,
                session.getAPIServer(), target, VERSION, params, session);

        VDiskUploadFileInfo fileInfo = new VDiskUploadFileInfo(setcionInfo, segmentNum, s3Host, segmentLength, sha1,
                srcPath, desPath);
     ...
     * 
     * @param path the VDisk path to the file for which you want to get a
     *            thumbnail.
     ...
     *             and 500 (internal error while creating thumbnail).
     * @throws VDiskPartialFileException if a network error occurs during the
     *             download.
     * @throws VDiskIOException for some network-related errors.
     * @throws VDiskException for any other unknown errors. This is also a
     ...
     *             error occurred.
     */
    public VDiskFileInfo getThumbnail(String path, OutputStream os, ThumbSize size, ProgressListener listener)
            throws VDiskException {
     ...
     * stream when you're done with it to release all resources. You can also
     * cancel the thumbnail download by closing the returned
     ...
     *             error occurred.
     */
    public VDiskInputStream getThumbnailStream(String path, ThumbSize size) throws VDiskException {
     ...
     * @throws VDiskException
     */
    public VDiskFileInfo getThumbnailInfo(String path, ThumbSize size) throws VDiskException {
        assertAuthenticated();
        ...
        RequestAndResponse r = RESTUtility.streamRequest(RequestMethod.GET, session.getAPIServer(), target, VERSION,
                params, session);
     ...
     * immediate children.
     * 
     * @param path the VDisk path to the file or directory for which to get
     *            metadata.
     ...
     *            for files.
     * @param includeDeleted optionally gets metadata for a file at a prior rev
     *            (does not apply to folders). Use false for the latest
     *            metadata.
     ...
     *             error occurred.
     */
    public Entry metadata(String path, String hash, boolean list, boolean includeDeleted) throws VDiskException {
        assertAuthenticated();
        ...
        String[] params = {
                "hash", hash, "list", String.valueOf(list), "include_deleted", String.valueOf(includeDeleted)
        };

        String url_path = "/metadata/" + session.getAccessType() + path;

        @SuppressWarnings("unchecked")
        Map<String, Object> dirinfo = (Map<String, Object>) RESTUtility.request(RequestMethod.GET,
     ...
     * 
     * @param path the VDisk path to the file for which to get revisions
     *            (directories are not supported).
     * @param revLimit the maximum number of revisions to return. Default is 10
     ...
     * @throws VDiskServerException if the server responds with an error code.
     *             See the constants in {@link VDiskServerException} for the
     *             meaning of each error code. The most common error code you
     ...
     */
    @SuppressWarnings("unchecked")
    public List<Entry> revisions(String path, int revLimit) throws VDiskException {
        assertAuthenticated();
        ...
        JSONArray revs = (JSONArray) RESTUtility.request(RequestMethod.GET, session.getAPIServer(), url_path, VERSION,
                params, session);
     ...
     * 
     * @param path the VDisk directory to search in.
     * @param query the query to search for (minimum 3 characters).
     * @param fileLimit the maximum number of file entries to return. Default is
     *            1,000 if you pass in 0 or less, and 1,000 is the most that
     *            will ever be returned.
     * @param includeDeleted whether to include deleted files in search results.
     ...
     *             error occurred.
     */
    public List<Entry> search(String path, String query, int fileLimit, boolean includeDeleted) throws VDiskException {
        assertAuthenticated();
        ...
        String[] params = {
                "query", query, "file_limit", String.valueOf(fileLimit), "include_deleted",
                String.valueOf(includeDeleted)
        };

        Object response = RESTUtility.request(RequestMethod.GET, session.getAPIServer(), target, VERSION, params,
                session);
     ...
     * to another.
     * 
     * @param fromPath the VDisk path to move from.
     * @param toPath the full VDisk path to move to (not just a directory).
     * @return a metadata {@link Entry}.
     * @throws VDiskUnlinkedException if you have not set an access token pair on
     *             the session, or if the user has revoked access.
     * @throws VDiskServerException if the server responds with an error code.
     ...
     *             404 (path not found), and 507 (user over quota).
     * @throws VDiskIOException if any network-related error occurs.
     * @throws VDiskException for any other unknown errors. This is also a
     *             superclass of all other VDisk exceptions, so you may want to
     *             only catch this exception which signals that some kind of
    ...
    public Entry move(String fromPath, String toPath) throws VDiskException {
        ...
        String[] params = {
                "root", session.getAccessType().toString(), "from_path", fromPath, "to_path", toPath
        };

        @SuppressWarnings("unchecked")
        Map<String, Object> resp = (Map<String, Object>) RESTUtility.request(RequestMethod.POST,
                session.getAPIServer(), "/fileops/move", VERSION, params, session);
     ...
     * to another.
     * 
     * @param fromPath the VDisk path to copy from.
     * @param toPath the full VDisk path to copy to (not just a directory).
     * @return a metadata {@link Entry}.
     * @throws VDiskUnlinkedException if you have not set an access token pair on
     *             the session, or if the user has revoked access.
     * @throws VDiskServerException if the server responds with an error code.
    ...
    public Entry copy(String fromPath, String toPath) throws VDiskException {
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> resp = (Map<String, Object>) RESTUtility.request(RequestMethod.POST,
                session.getAPIServer(), "/fileops/copy", VERSION, params, session);
     ...
     * 
     * @param path the VDisk path to the new folder.
     ...
     *             at that path), 404 (path not found), and 507 (user over
     *             quota).
     * @throws VDiskIOException if any network-related error occurs.
     * @throws VDiskException for any other unknown errors. This is also a
     *             superclass of all other VDisk exceptions, so you may want to
    ...
    public Entry createFolder(String path) throws VDiskException {
        ...
        String[] params = {
                "root", session.getAccessType().toString(), "path", path
        };

        @SuppressWarnings("unchecked")
        Map<String, Object> resp = (Map<String, Object>) RESTUtility.request(RequestMethod.POST,
                session.getAPIServer(), "/fileops/create_folder", VERSION, params, session);
     ...
     * be set to {@code true}.
     * 
     * @param path the VDisk path to delete.
     * @throws VDiskUnlinkedException if you have not set an access token pair on
     *             the session, or if the user has revoked access.
     * @throws VDiskServerException if the server responds with an error code.
     *             See the constants in {@link VDiskServerException} for the
    ...
    public Entry delete(String path) throws VDiskException {
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> dirinfo = (Map<String, Object>) RESTUtility.request(RequestMethod.POST,
                session.getAPIServer(), "/fileops/delete", VERSION, params, session);
     ...
     * 
     * @param path the VDisk path to the file to restore.
    ...
    public Entry restore(String path, String rev) throws VDiskException {
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> metadata = (Map<String, Object>) RESTUtility.request(RequestMethod.POST,
                session.getAPIServer(), target, VERSION, params, session);
     ...
     * 
     * @param path the VDisk path of the file for which to get a streaming link.
     * @param ssl whether the streaming URL is https or http. Some Android and
     ...
     * 
     * @param sourceCopyRef file's copy Reference
     * @return
     * @throws VDiskException
     */
    public VDiskLink getLinkByCopyRef(String sourceCopyRef) throws VDiskException {
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> map = (Map<String, Object>) RESTUtility.request(RequestMethod.GET, session.getAPIServer(),
                target, VERSION, params, session);

        return new VDiskLink(map, true);
    }

    /**
     * Generates a Link for sharing the specified file. Notice: the file'size
     * need to be less than 200MB; the file should be in the following types:
     ...
     * 
     * @param path the VDisk file path to share.
    ...
    public String share(String path) throws VDiskException {
        ...
        @SuppressWarnings("unchecked")
        Map<String, Object> map = (Map<String, Object>) RESTUtility.request(RequestMethod.POST, session.getAPIServer(),
                target, VERSION, null, session);
     ...
     * 
     * @param path the VDisk path to cancel share.
     ...
     *            progress updates, or null.
     * @return an {@link UploadRequest}.
     * @throws IllegalArgumentException if {@code newFilename} is null or empty.
     * @throws VDiskUnlinkedException if you have not set an access token pair
     *             on the session, or if the user has revoked access.
     * @throws VDiskFileSizeException if the file is bigger than the maximum
     *             allowed by the API. See {@code VDiskAPI.MAX_UPLOAD_SIZE}.
    ...
    private UploadRequest putFileRequest(String path, InputStream is, long length, boolean overwrite, String parentRev,
        ...
        String[] params = new String[] {
                "overwrite", String.valueOf(overwrite), "parent_rev", parentRev
        };

        String url = RESTUtility.buildURL(session.getUploadServer(), VERSION, target, params);
        ...
        if (Logger.DEBUG_MODE) {
            ...
            if (session.getWeiboAccessToken() != null) {
                ...
                String token = Signature.getWeiboHeader(session.getAppKeyPair(),
                ...
                curlHeader = "Authorization:OAuth2 " + session.getAccessToken().mAccessToken;
            }

            Log.i(TAG, "curl -H \"" + curlHeader + "\" " + "-H \"Host:" + session.getUploadServer() + "\" " + "\""
                    + url + "\" -k -v");
            Logger.writeHeader(session.getContext());
            Logger.writeToFile("curl -H \"" + curlHeader + "\" " + "-H \"Host:" + session.getUploadServer() + "\" "
                    + "\"" + url + "\" -k -v");
        }

        SimpleUploadInputStreamEntity isEntity = new SimpleUploadInputStreamEntity(is, length);
     ...
     * 
     * @param cursor On the first call, you should pass in <code>null</code>. On
    ...
    public DeltaPage<Entry> delta(String cursor) throws VDiskException {
        ...
        Object json = RESTUtility.request(RequestMethod.POST, session.getAPIServer(), path, VERSION, params, session);
        try {
            return DeltaPage.extractFromJson(new JsonThing(json), Entry.JsonExtractor);
        } catch (JsonExtractionException ex) {
            throw new VDiskParseException("Error parsing /delta results: " + ex.getMessage());
        }
    }
    ...
    public static final class DeltaPage<MD> {
        ...
        public static <MD> DeltaPage<MD> extractFromJson(JsonThing j, JsonExtractor<MD> entryExtractor)
    ...
    public static final class DeltaEntry<MD> {
         ...
         * </p>
         * <ul>
         * <li>If the path refers to parent folders that don't exist yet in your
         * local state, create those parent folders in your local state.</li>
         * <li>If the metadata is for a file, replace whatever your local state
         * has at that path with the new entry.</li>
         * <li>If the metadata is for a folder, check what your local state has
         * at the path. If it's a file, replace it with the new entry. If it's a
        ...
        public static final class JsonExtractor<MD> extends com.vdisk.net.jsonextract.JsonExtractor<DeltaEntry<MD>> {
            ...
            public JsonExtractor(com.vdisk.net.jsonextract.JsonExtractor<MD> mdExtractor) {
            ...
            public static <MD> DeltaEntry<MD> extract(JsonThing j,
                    com.vdisk.net.jsonextract.JsonExtractor<MD> mdExtractor) throws JsonExtractionException {
     ...
     *             error occurred.
     */
    public CreatedCopyRef createCopyRef(String sourcePath) throws VDiskException {
        assertAuthenticated();
        ...
        Object result = RESTUtility.request(RequestMethod.POST, session.getAPIServer(), url_path, VERSION, null,
                session);

        try {
            return CreatedCopyRef.extractFromJson(new JsonThing(result));
        } catch (JsonExtractionException ex) {
            throw new VDiskParseException("Error parsing /copy_ref results: " + ex.getMessage());
        }
    }
     ...
     * 
     * @param sourceCopyRef The copy-ref to use as the source of the file data
     *            (comes from {@link CreatedCopyRef#copyRef
     *            CreatedCopyRef.copyRef}, which is created through
     *            {@link #createCopyRef createCopyRef()}).
     ...
     *             error occurred.
     */
    public Entry addFromCopyRef(String sourceCopyRef, String targetPath) throws VDiskException {
        assertAuthenticated();
        ...
        String[] params = {
                "root", session.getAccessType().toString(), "from_copy_ref", sourceCopyRef, "to_path", targetPath
        };
    ...
    /**
     * 递归列出某目录下全部子目录。目前仅支持 root 为 basic。 Use recursion to list all
     * sub-directories under the specified directory. Currently only supports
     * root of basic.
     * 
     * @param targetPath 想要获取所有相册的目标路径 Target path of all albums you want to get
     * @param cursor 表示已返回的列表在操作记录中的位置 The location of the returned list in
     *            operation log
     */
    public AlbumPage<Entry> getAlbums(String targetPath, String cursor) throws VDiskException {
        assertAuthenticated();
        ...
        String[] params = {
                "root", session.getAccessType().toString(), "path", targetPath, "cursor", cursor
        };

        Object json = RESTUtility.request(RequestMethod.GET, session.getAPIServer(), "/folder/recur", VERSION, params,
                session);

        try {
            return AlbumPage.extractFromJson(new JsonThing(json), Entry.JsonExtractor);
        } catch (JsonExtractionException ex) {
            throw new VDiskParseException("Error parsing /delta results: " + ex.getMessage());
        }
    ...
    public static final class AlbumPage<MD> {
        ...
        public static <MD> AlbumPage<MD> extractFromJson(JsonThing j, JsonExtractor<MD> entryExtractor)
                throws JsonExtractionException {
            JsonMap m = j.expectMap();
            String cursor = m.get("cursor").expectString();
            boolean hasMore = m.get("has_more").expectBoolean();
            List<Entry> entries = m.get("data").expectList().extract(Entry.JsonExtractor);
    ...
    /**
     * 带筛选的目录列表，按文件类型列出目录下的全部文件。目前仅支持 root 为 basic。 A directory list with
     * filter, listing all files under the specified directory by file
     * type.Currently only supports root of basic.
     * 
     * @param targetPath 要获取列表的目录路径 Directory path of the list you want to get
     * @param size 缩略图尺寸 Size of thumbnail
     * @return
     * @throws VDiskException
     */
    @SuppressWarnings("unchecked")
    public List<Entry> getPhotos(String targetPath, ThumbSize size) throws VDiskException {
        assertAuthenticated();

        String[] params = {
                "root", session.getAccessType().toString(), "path", targetPath, "file_type", "img", "dimension",
                size.toAPISize()
        };

        JSONArray photos = (JSONArray) RESTUtility.request(RequestMethod.GET, session.getAPIServer(), "/folder/filter",
    ...
    /**
     * 调用微盘的其他API Call VDisk's other APIs.
     * 
     * @param params 调用API所用到的参数数组 Params array
     * @param apiName API路径, 例如"/metadata/" API's path, such as "/metadata/"
     * @param method 请求方法, 例如GET, POST Request method, such as GET or POST
     * @return JSONArray或者JSONObject Returns JSONArray or JSONObject
     * @throws VDiskException
     */
    public Object callOtherAPI(String[] params, String apiName, RequestMethod method) throws VDiskException {
        assertAuthenticated();

        return RESTUtility.request(method, session.getAPIServer(), apiName, VERSION, params, session);
    }
     ...
     * @return the value, with false as a default if no parameter set
     */
    protected static String getFromMapAsString(Map<String, Object> map, String name) {
        Object val = map.get(name);
        String ret = null;
        if (val != null) {
            if (val instanceof Number) {
