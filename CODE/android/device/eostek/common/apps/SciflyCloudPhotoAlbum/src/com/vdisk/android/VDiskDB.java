 ...
 * +上传文件的微盘目标路径)，用来标识待上传的文件; file_obj表示文件对象序列化后的形式 There are two columns in this
 * table in database: file_id & file_obj. file_id = MD5(Local path of file to
 * upload + target VDisk path of file to upload), to identify the file to
 * upload; file_obj, the form of serialized file object.
 * 
 * @author Kevin
 */
public class VDiskDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String sql = "CREATE TABLE IF NOT EXISTS " + UPLOAD_TABLE + " (" + "_id"
                + " Integer primary key autoincrement, " + UPLOAD_FILE_ID + " TEXT UNIQUE, " + UPLOAD_FILE_OBJECT
                + " TEXT )";
        db.execSQL(sql);
    }
    ...
    /**
     * 更新上传文件分段信息 Update segment information of upload file.
     * 
     * @param fileId
     * @param serStr
     * @return
    ...
    public boolean updateUploadFileInfo(String fileId, String serStr) {
        if (readUploadFileInfo(fileId) == null) {
            ...
            db.update(UPLOAD_TABLE, c, UPLOAD_FILE_ID + " =? ", new String[] {
                fileId
            });
            return true;
        }
    ...
    /**
     * 删除上传文件分段信息 Delete segment information of upload file.
     * 
     * @param fileId
     */
    ...
    /**
     * 读取上传文件分段信息 Read segment information of upload file.
     * 
     * @param fileId
     * @return
     */
    public String readUploadFileInfo(String fileId) {
        Cursor cursor = db.query(UPLOAD_TABLE, new String[] {
            UPLOAD_FILE_OBJECT
        }, UPLOAD_FILE_ID + " = ?", new String[] {
            fileId
        }, null, null, null);
