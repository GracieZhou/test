...
public class VDiskServerException extends VDiskException {
    ...
    public String toString() {
        return "VDiskServerException (" + server + "): " + error + " " + reason + " (" + body.error + ")";
    }
    ...
    public static boolean isValidWithNullBody(HttpResponse response) {
        ...
        if (code == _302_FOUND) {
            /*
             * String location = getHeader(response, "location"); if (location
             * != null) { int loc = location.indexOf("://"); if (loc > -1) {
             * location = location.substring(loc+3); loc =
