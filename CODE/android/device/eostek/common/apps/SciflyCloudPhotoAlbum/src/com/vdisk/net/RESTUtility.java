...
import com.vdisk.net.session.VSession.ProxyInfo;
...
public class RESTUtility {
     ...
     * @param host the hostname to use. Should be either api server, content
     *            server, or web server.
     * @param path the URL path, starting with a '/'.
     * @param apiVersion the API version to use. This should almost always be
     *            set to {@code VDiskAPI.VERSION}.
     * @param params the URL params in an array, with the even numbered elements
     *            the parameter names and odd numbered elements the values, e.g.
     *            <code>new String[] {"path", "/Public", "locale",
     *         "en"}</code>.
     * @param session the {@link VSession} to use for this request.
     ...
     * @throws VDiskUnlinkedException if the user has revoked access.
     * @throws VDiskParseException if a malformed or unknown response was
    ...
    static public Object request(RequestMethod method, String host, String path, int apiVersion, String[] params,
            VSession session) throws VDiskException {
        HttpResponse resp = streamRequest(method, host, path, apiVersion, params, session).response;
        return parseAsJSON(resp);
    }

    static public Object request(RequestMethod method, String host, String path, int apiVersion, String[] params,
            VSession session, int socketTimeoutOverrideMs) throws VDiskException {
        HttpResponse resp = streamRequest(method, host, path, apiVersion, params, session, true,
                socketTimeoutOverrideMs).response;
        return parseAsJSON(resp);
    }
    ...
    static public RequestAndResponse streamRequest(RequestMethod method, String host, String path, int apiVersion,
            String params[], VSession session) throws VDiskException {
        return streamRequest(method, host, path, apiVersion, params, session, true, -1);
    }

    static public RequestAndResponse streamRequest(RequestMethod method, String host, String path, int apiVersion,
            String params[], VSession session, boolean needSign, int socketTimeoutOverrideMs) throws VDiskException {
        String target = null;
        ...
        return streamRequestAndResponse(method, session, target, params, 0, null, needSign, socketTimeoutOverrideMs);
    }

    static public RequestAndResponse streamRequestAndResponse(RequestMethod method, VSession session, String target,
            String params[]) throws VDiskException {
        return streamRequestAndResponse(method, session, target, params, 0, null, false, -1);
    }

    static public RequestAndResponse streamRequestAndResponse(RequestMethod method, VSession session, String target,
            String params[], long range, String md5, boolean needSign, int socketTimeoutOverrideMs,
            boolean handleRedirect) throws VDiskException {
        ...
        if (method == RequestMethod.GET) {
            ...
            if (range > 0 && md5 != null) {
                req.setHeader("Range", "bytes=" + range + "-");// 设置下载范围 // Set
                                                               // download range
                req.setHeader("If-Range", "\"" + md5 + "\"");
            }

            if (Logger.DEBUG_MODE) {
                Log.i(TAG, "curl -H \"" + curlHeader + "\" \"" + target + "\" -k -v");
                Logger.writeHeader(session.getContext());
                Logger.writeToFile("curl -H \"" + curlHeader + "\" \"" + target + "\" -k -v");
            }
        } else {
            ...
            if (params != null && params.length >= 2) {
                ...
                for (int i = 0; i < params.length; i += 2) {
                    if (params[i + 1] != null) {
                        ...
                        postParams.append(params[i]).append("=").append(params[i + 1]).append("&");
                    }
                }
                ...
                if (Logger.DEBUG_MODE) {
                    ...
                    Log.i(TAG, "curl -H \"" + curlHeader + "\" -d \"" + postParams.toString() + "\" \"" + target
                            + "\" -k -v");
                    Logger.writeHeader(session.getContext());
                    Logger.writeToFile("curl -H \"" + curlHeader + "\" -d \"" + postParams.toString() + "\" \""
                            + target + "\" -k -v");
                }
    ...
    static public RequestAndResponse streamRequestAndResponse(RequestMethod method, VSession session, String target,
        ...
        return streamRequestAndResponse(method, session, target, params, range, md5, needSign, socketTimeoutOverrideMs,
                false);
    }
     ...
     *             See the constants in {@link VDiskServerException} for the
     *             meaning of each error code.
     * @throws VDiskIOException if any network-related error occurs while
     ...
     *             error occurred.
     */
    public static Object parseAsJSON(HttpResponse response) throws VDiskException {
        Object result = null;
        ...
        if (Logger.DEBUG_MODE) {
            ...
            for (Header header : allHeaders) {
                Log.d(TAG, "header name:" + header.getName() + ";header value:" + header.getValue());
        ...
        if (statusCode != VDiskServerException._200_OK) {
            if (statusCode == VDiskServerException._401_UNAUTHORIZED) {
                if (Logger.DEBUG_MODE) {
                    Log.i(TAG, "error_body-->" + result + ";statusCode-->" + statusCode);
        ...
        if (Logger.DEBUG_MODE) {
            Logger.writeToFile("statusCode-->" + statusCode + "\n" + "error_body-->" + result);
            Log.i(TAG, "error_body-->" + result.toString() + ";statusCode-->" + statusCode);
        }
    ...
    /**
     * Executes an {@link HttpUriRequest} with the given {@link VSession} and
     ...
     *             error occurred.
     */
    public static HttpResponse execute(VSession session, HttpUriRequest req) throws VDiskException {
     ...
     * @param session the session to use.
     * @param req the request to execute.
     * @param socketTimeoutOverrideMs if >= 0, the socket timeout to set on this
     *            request. Does nothing if set to a negative number.
     * @return an {@link HttpResponse}.
     * @throws VDiskServerException if the server responds with an error code.
     *             See the constants in {@link VDiskServerException} for the
     *             meaning of each error code.
     ...
     *             error occurred.
     */
    public static HttpResponse execute(VSession session, HttpUriRequest req, int socketTimeoutOverrideMs)
            throws VDiskException {
        ...
        HttpRequestRetryHandler retryHandler = ((DefaultHttpClient) client).getHttpRequestRetryHandler();
        ...
        while (retry) {
            try {
                ...
                if (response == null) {
                    ...
                    if (code == VDiskServerException._206_OK || code == VDiskServerException._302_FOUND) {
                        return response;
                    }
                ...
                retry = retryHandler.retryRequest(cause, ++executionCount, localContext);
     ...
     * 
     * @param host the VDisk host (i.e., api server, content server, or web
     *            server).
     ...
     * @param params any URL params in an array, with the even numbered elements
     *            the parameter names and odd numbered elements the values, e.g.
     *            <code>new String[] {"path", "/Public", "locale",
     *         "en"}</code>.
     ...
     */
    private static synchronized HttpClient updatedHttpClient(VSession session) {
     ...
     */
    private static void updateClientProxy(HttpClient client, VSession session) {
    ...
    static public HttpUriRequest streamHttpUriRequest(String url, String[] params) {
        HttpUriRequest req = null;
        HttpPost post = new HttpPost(url);
        post.setHeader("Content-Type", "application/x-www-form-urlencoded");// 必须进行encode处理，否则报错
                                                                            // //
                                                                            // Encode
                                                                            // setting
                                                                            // must
    ...
    static public String parseResponse(VSession session, String url, String[] params, Context ctx)
            throws VDiskException {
