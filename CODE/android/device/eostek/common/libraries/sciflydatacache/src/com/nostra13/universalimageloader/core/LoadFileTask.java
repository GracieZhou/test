...
final class LoadFileTask implements Runnable, IoUtils.CopyListener {
    ...
    public LoadFileTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo, Handler handler) {
        this.engine = engine;
        ...
        postfix = DataCacheManager.getPostfix(options);
    }
    ...
    public void run() {
        ...
        try {
            ...
            object = configuration.memoryCache.get(memoryCacheKey);

            if (object == null || !options.isCacheInMemory()) {
                L.d("Object not in memory, try loading [%s]", memoryCacheKey);
                object = tryLoadFile();
                if (object == null) return; // listener callback already was fired
                ...
                if (object != null && options.isCacheInMemory()) {
                    ...
                    configuration.memoryCache.put(memoryCacheKey, object);
                }
            } else {
        ...
        DisplayObjectTask displayObjectTask = new DisplayObjectTask(object,
                imageLoadingInfo, engine, loadedFrom, syncLoading, handler);
        runTask(displayObjectTask, syncLoading, handler, engine);
    }
    ...
    private Object tryLoadFile() throws TaskCancelledException {
        Object object = null;
        try {
            File file = configuration.diskCache.get(uri, postfix);
            if (file != null && file.exists()) {
            ...
            if (object == null) {
                ...
                if (options.isCacheOnDisk() && tryCacheFileOnDisk()) {
                    file = configuration.diskCache.get(uri, postfix);
                    if (file != null && file.exists()) {
                        if (options.isCacheInMemory()) {
                            object = fileToString(file);
                        } else {
                    ...
                    L.d("Fail to cache on disk [%s], try to get from network!", memoryCacheKey);
                    InputStream is = getDownloader().getStream(uri, options.getExtraForDownloader());
                    if (options.isCacheInMemory()) {
                        L.d("Return String to caller [%s]", memoryCacheKey);
                        object = streamToString(is);
                    }
                }
                checkTaskNotActual();
    ...
    private boolean downloadFile() throws IOException {
        InputStream is = getDownloader().getStream(uri, options.getExtraForDownloader());
        return configuration.diskCache.save(uri, is, postfix, this);
    }
