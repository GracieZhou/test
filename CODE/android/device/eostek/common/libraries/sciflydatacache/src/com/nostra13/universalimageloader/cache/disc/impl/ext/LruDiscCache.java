...
public class LruDiscCache implements DiskCache {
    ...
    public LruDiscCache(File cacheDir, FileNameGenerator fileNameGenerator, long cacheMaxSize) throws IOException {
        this(cacheDir, null, fileNameGenerator, cacheMaxSize, 0, 0);
    }
    ...
    public LruDiscCache(File cacheDir, File reserveCacheDir, FileNameGenerator fileNameGenerator, long cacheMaxSize,
        ...
        this(cacheDir, null, fileNameGenerator, cacheMaxSize, cacheMaxFileCount, 0);
    }

    public LruDiscCache(File cacheDir, File reserveCacheDir, FileNameGenerator fileNameGenerator, long cacheMaxSize,
            int cacheMaxFileCount, int singleDiskSize) throws IOException {
        if (cacheDir == null) {
    ...
    public File get(String imageUri, String postfix) {
        ...
        try {
            snapshot = cache.get(getKey(imageUri, postfix));
    ...
    public boolean save(String imageUri, InputStream imageStream,
        ...
        L.d("singleDiskSize=[%d], and inputStream size is [%d]",singleDiskSize, size);
        if (singleDiskSize > 0 && size > singleDiskSize) {
            L.w("size > singleDiskSize, inputStream should not be cached");
            return false;
        }
        DiskLruCache.Editor editor = cache.edit(getKey(imageUri, postfix));
        if (editor == null) {
            return false;
        }
    ...
    public boolean save(String imageUri, Bitmap bitmap, String postfix) throws IOException {
        ...
        L.d("singleDiskSize=[%d], and bitmap size is [%d]",singleDiskSize, size);
        if (singleDiskSize > 0 && size > singleDiskSize) {
            L.w("size > singleDiskSize, bitmap should not be cached");
            return false;
        }
        DiskLruCache.Editor editor = cache.edit(getKey(imageUri, postfix));
        if (editor == null) {
    ...
    public boolean remove(String imageUri, String postfix) {
        try {
            return cache.remove(getKey(imageUri, postfix));
        } catch (IOException e) {
            L.e(e);
            return false;
        }
    ...
    private String getKey(String imageUri, String postfix) {
        if (postfix != null && !postfix.isEmpty() && postfix.startsWith(".")) {
            return fileNameGenerator.generate(imageUri).concat(postfix);
        }
