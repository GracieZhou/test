...
public class ImageLoader {
    ...
    public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options,
        ...
        Bitmap bmp = (Bitmap)configuration.memoryCache.get(memoryCacheKey);
        if (bmp != null && !bmp.isRecycled() && options.isCacheInMemory()) {
            ...
            if (options.shouldPostProcess()) {
                ...
                if (options.shouldCheckIfModified()) {
                    ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
                            options, listener, progressListener, engine.getLockForUri(uri));
                    CheckAndDisplayImageTask checkTask = new CheckAndDisplayImageTask(engine, imageLoadingInfo,
                            defineHandler(options));
                    if (options.isSyncLoading()) {
                        checkTask.run();
                    } else {
    ...
    public void loadImage(String uri, ImageSize targetImageSize, DisplayImageOptions options,
            ImageLoadingListener listener, ImageLoadingProgressListener progressListener) {
        loadImage(DataCacheManager.DATA_CACHE_TYPE_IMAGE, uri, targetImageSize, options, listener, progressListener);
    }
     ...
     * <b>NOTE:</b> {@link #init(ImageLoaderConfiguration)} method must be called before this method call
     *
     * @param type             Cache type {@link DataCacheManager.DATA_CACHE_TYPE_IMAGE}
     *                         {@link DataCacheManager.DATA_CACHE_TYPE_TXT}
     *                         {@link DataCacheManager.DATA_CACHE_TYPE_FILE}
     * @param uri              Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
     * @param targetImageSize  Minimal size for {@link Bitmap} which will be returned in
     *                         {@linkplain ImageLoadingListener#onLoadingComplete(String, android.view.View,
     *                         android.graphics.Bitmap)} callback}. Downloaded image will be decoded
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public void loadImage(int type, String uri, ImageSize targetImageSize, DisplayImageOptions options,
            ImageLoadingListener listener, ImageLoadingProgressListener progressListener) {
        checkConfiguration();
        ...
        if (type == DataCacheManager.DATA_CACHE_TYPE_IMAGE) {
            displayImage(uri, imageAware, options, listener, progressListener);
        } else {
            loadFile(uri, imageAware, options, listener, progressListener);
        }
    }

    /**
     * Adds load File task to execution pool. File will be returned with
     * {@link ImageLoadingListener#onLoadingComplete(String, android.view.View, java.lang.Object)} callback}.
     * <br />
     * <b>NOTE:</b> {@link #init(ImageLoaderConfiguration)} method must be called before this method call
     *
     * @param uri              File URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
     * @param imageAware       {@linkplain com.nostra13.universalimageloader.core.imageaware.ImageAware Image aware view}
     *                         which should display image
     * @param options          {@linkplain com.nostra13.universalimageloader.core.DisplayImageOptions Options} for image
     *                         decoding and displaying. If <b>null</b> - default display image options
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public void loadFile(String uri, ImageAware imageAware, DisplayImageOptions options,
            ImageLoadingListener listener, ImageLoadingProgressListener progressListener) {
        checkConfiguration();
        if (imageAware == null) {
            throw new IllegalArgumentException(ERROR_WRONG_ARGUMENTS);
        ...
        Object object = configuration.memoryCache.get(memoryCacheKey);
        if (object != null && !options.isCacheInMemory()) {
            L.d(LOG_LOAD_IMAGE_FROM_MEMORY_CACHE, memoryCacheKey);
            listener.onLoadingComplete(uri, imageAware.getWrappedView(), object);
            // check if file modified from network when file in memory
            if (options.shouldCheckIfModified()) {
                ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
                        options, listener, progressListener, engine.getLockForUri(uri));
                CheckFileTask loadTask = new CheckFileTask(engine, imageLoadingInfo,
                        defineHandler(options));
                if (options.isSyncLoading()) {
                    loadTask.run();
                } else {
                    engine.submit(loadTask);
                }
            }
        } else {
            if (options.shouldShowImageOnLoading()) {
            ...
            ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageAware, targetSize, memoryCacheKey,
                    options, listener, progressListener, engine.getLockForUri(uri));
            LoadFileTask loadTask = new LoadFileTask(engine, imageLoadingInfo,
                    defineHandler(options));
            if (options.isSyncLoading()) {
                loadTask.run();
            } else {
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public Object loadImageSync(String uri) {
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public Object loadImageSync(int type, String uri) {
        DisplayImageOptions options = new DisplayImageOptions.Builder()
                .cacheOnDisk(true)
                .build();
        return loadImageSync(type, uri, null, options);
    }
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public Object loadImageSync(String uri, DisplayImageOptions options) {
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public Object loadImageSync(String uri, ImageSize targetImageSize) {
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public Object loadImageSync(String uri, ImageSize targetImageSize, DisplayImageOptions options) {
        return loadImageSync(DataCacheManager.DATA_CACHE_TYPE_IMAGE, uri, targetImageSize, options);
    }
     ...
     * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
     */
    public Object loadImageSync(int type, String uri, ImageSize targetImageSize, DisplayImageOptions options) {
        if (options == null) {
            options = configuration.defaultDisplayImageOptions;
        }
        ...
        loadImage(type, uri, targetImageSize, options, listener, null);
    ...
    public Object clearMemoryCache(String uri) {
        checkConfiguration();
        String memoryCacheKey = MemoryCacheUtils.generateKey(uri, configuration.getMaxImageSize());
        return configuration.memoryCache.remove(memoryCacheKey);
    }

    public Object clearMemoryCache(ImageView imageView) {
        checkConfiguration();
        String memoryCacheKey = getLoadingUriForView(imageView);
        return configuration.memoryCache.remove(memoryCacheKey);
    }
    ...
    public void clearDiskCache() {
        ...
        configuration.downloader.clear();
    }
    ...
    public boolean clearDiskCache(String uri, String postfix) {
        checkConfiguration();
        configuration.downloader.clear(uri);
        return configuration.diskCache.remove(uri, postfix);
    }
    ...
    /**
     * Cancel the task of loading and displaying image for passed uri
     *
     * @param uri for which display task will be cancelled
     */
    public void cancelDisplayTask(String uri) {
        String memoryCacheKey = MemoryCacheUtils.generateKey(uri, configuration.getMaxImageSize());
        engine.cancelDisplayTaskFor(memoryCacheKey);
    }
