 ...
 * @since 1.3.1
 */
final class CheckFileTask implements Runnable, IoUtils.CopyListener {
    ...
    public CheckFileTask(ImageLoaderEngine engine, ImageLoadingInfo imageLoadingInfo,
            Handler handler) {
        this.engine = engine;
    ...
    public void run() {
        ...
        DisplayObjectTask displayObjectTask = new DisplayObjectTask(object, imageLoadingInfo,
                engine, LoadedFrom.LOCAL_NETWORK, syncLoading, handler);
    ...
    private boolean downloadFile() throws IOException {
        InputStream is = getDownloader().checkStreamFromNetwork(uri, options.getExtraForDownloader());
        if (is == null) {
            // file not modified
            return false;
        }
