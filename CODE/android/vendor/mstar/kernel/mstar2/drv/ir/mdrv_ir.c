...
#include "IR_KONKA.h"
// EosTek Patch Begin
#elif (IR_TYPE_SEL == IR_TYPE_HERAN)
#include "IR_HERAN_DTV.h"
// EosTek Patch End
...
static U32 EosIRHeaderCode = 0x00;//xxh
...
//static U32  _u32IRCount=0;
// EosTek Patch Begin
#if (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_SEL == IR_TYPE_MSTAR_DTV || IR_TYPE_SEL == IR_TYPE_CHANGHONG || IR_TYPE_SEL == IR_TYPE_HERAN)
// EosTek Patch End
...
int key_dispatch(void *p) {
    ...
    DEBUG_IR_INPUT_DEVICE(printk("key_dispatch thread start\n"));
    ...
    while(1) {
        ...
        if ((prev_scancode != 0xFFFFUL) && (scancode == 0xFFFFUL)) {
           ...
           if ((scancode != prev_scancode)) {
	        ...
	        rc_keydown_eos_xxh_notimeout(ir->dev, scancode,EosIRHeaderCode,0);
           }
        } else if ((prev_scancode == 0xFFFFUL) && (scancode != 0xFFFFUL)) {
    ...
    DEBUG_IR_INPUT_DEVICE(printk("key_dispatch thread end\n"));
    return 0;
}
...
static irqreturn_t  MDrv_IR_ISRParseKey(void)
{
    ...
    // EosTek Patch Begin
    #elif (IR_TYPE_SEL == IR_TYPE_HISENSE || IR_TYPE_SEL == IR_TYPE_MSTAR_DTV || IR_TYPE_SEL == IR_TYPE_CHANGHONG || IR_TYPE_SEL == IR_TYPE_HERAN)
    // EosTek Patch End
    static unsigned long ulPreTime;
    U32 tmp;
	...
	for (j=0; j<IR_RAW_DATA_NUM; j++)
	{
		...
		if(_u8IRRawModeCount == IR_RAW_DATA_NUM)
		{
			//_u8IRRawModeCount = 0;
				...
				{
					EosIRHeaderCode = _u8IRRawModeBuf[0]<<8;
					EosIRHeaderCode |= _u8IRRawModeBuf[1];
