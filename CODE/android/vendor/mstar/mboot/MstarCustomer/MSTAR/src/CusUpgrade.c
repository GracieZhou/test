...
#define AP_BIN_PATH             getenv("ForceUpgradePath")==NULL?"/usbflash/638Upgrade.bin":getenv("ForceUpgradePath")
#define OAD_BIN_PATH            AP_BIN_PATH
#define NET_BIN_PATH            AP_BIN_PATH
#define PQ_BIN_PATH             "/usbflash/PQUpgrade.bin"
...
#define EOSTEK_PQUPGRADE_BEGIN_STR      "EOSTEK_PQUPGRADE_BEGIN"
#define EOSTEK_PQUPGRADE_END_STR        "EOSTEK_PQUPGRADE_END"
...
#define UPGRADE_STATE_PQ_BIN              (BIT4)
//============for ISP Upgrade============
...
static int check_bin_flag(void)
{
    ...
    if(check_file_partition(PQ_BIN_PATH,NULL,NULL)==0)
    {
      stage |= UPGRADE_STATE_PQ_BIN;
      UBOOT_DEBUG("has PQ upgrade bin\n");
    }
...
// EosTek Patch End
    ...
    else if(state&UPGRADE_STATE_PQ_BIN)
    {
        char *tmp;
        console_init();
        printf("\r\nUART_115200\r\n");
        printf("\r\n"EOSTEK_PQUPGRADE_BEGIN_STR"\r\n");
        memset(cmd, 0, 128);
        snprintf(cmd, sizeof(cmd)-1, "ustar %s",PQ_BIN_PATH);
        UBOOT_DEBUG("cmd=%s\n",cmd);
        if(-1 == run_command(cmd, 0))
        {
            printf("Upgrade PQ error!\n");
        }
        else
        {
            UBOOT_INFO("PQ upgrade OK!~\n");
        }
        setenv(ENV_FORCEUPGRADE,NULL);
        setenv ("sync_mmap", "1");
        saveenv();
        
        printf("\r\n"EOSTEK_PQUPGRADE_END_STR"\r\n");
