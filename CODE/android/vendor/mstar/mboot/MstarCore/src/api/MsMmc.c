...
static u32 do_mmc_empty_check(const void *buf, u32 len, u32 empty_flag)
{
    int i;
// EosTek Patch Begin
#if ENABLE_LED_FLICKER_WHEN_SW_UPDATE
        {
...
//                    print_part(mmc_dev);
        ...
        // MStar End
            ...
            } else if(!strcmp("erase",argv[1])) {             //erase all blocks in user area partiiton

                if(argc==3 && !strcmp("customer",argv[2]))
		   {
    ...
    if( strcmp(argv[1], "clonepart") == 0 )
    {
        ...
        for (i=1; i<=EMMC_RESERVED_FOR_MAP_V2; i++)
        {
            ...
            if( strcmp((const char *)mpart[i-1].name, argv[2]) == 0 )
            {
                src_start_blk = mpart[i-1].start_block;
                src_blks = mpart[i-1].block_count;
            }

            if( strcmp((const char *)mpart[i-1].name, argv[3]) == 0 )
            {
...
#define RELIABLE_TEST_SIZE          (mmc->block_dev.lba-0x8000-RELIABLE_START_OFFSET)
